...
## Økt 2026-09-08 (sen) — oppsummering
- Annonse-film: Storskrift skjules med visibility etter toning (ingen dobbel adresse), Arkhode glir inn. Kart: kontinuerlig kamera (settes → driver → zoom expo-out → fortsetter inn i arket), nål lander med skygge + to pust, VELG 920 ms. Bug fikset: kart-<img> ble klemt av Tailwind `max-width:100%` → `maxWidth:'none'`. Egen mobilvariant `public/v4/annonse/bergen-kart-m.svg` (2× strek).
- ProduktSeksjon: kapittelbytte toner ut (260 ms) → monterer → inn; høyde holdes (holdH/slippH); ingress min-h 6em; alle kapitler forhåndslastes; scroll bare hvis flaten ikke står på plass. Kapittelpille: én glidende markør (left/width) i stedet for hoppende bakgrunn.
- /forvaltning bygget om: hero (løfte venstre / ForvaltningScene i ramme høyre), «Én måned. 31 ting / 1 beslutning» (Maanedsbrett), InnsynSeksjon (eierens oversikt, gjenbrukbar via props), kompakt TeamSeksjon.
- /utleiemeglere NY: MeglerV4 (hero «Hundre eiere. Én rolig morgen.» + MeglerScene: Bergen-kart m/ 72 portefølje-punkter, tellere, dagens ledger, «Venter på dere»), SalgSeksjon (pipeline-brett Ny→Kontaktet→Befaring→Tilbud sendt→Signert, reisende kort, Tilbudsark som bygger seg opp: cover, personlig hilsen, svart kort brutto→honorar→netto, «Aksepter → BankID» → «Signert»), ProduktSeksjon (4 kapitler), SammenligningSeksjon («Én dag. Tre stoler.»), OppgjorSeksjon (4 stasjoner, teller, linje, 86 eier-streker), InnsynSeksjon (eierportal), KontrollSeksjon, Steg, FAQ (inkl. «Konkurrerer DigiHome med oss?»), Avslutning. Nav: 'Boligeiere','Eiendomsselskaper','Utleiemeglere','Forvaltning','Priser'. Footer + sitemap oppdatert.
- Produkt-repo (github martinkviteberg1/DigiHome, branch newdesignuiux, klonet til /tmp/digihome-prod — ephemeral) kartlagt: huseier-CRM (stadier, temperatur, SLA 4/7 d, EDR, estimat SSB+egen portefølje, tilbudsutkast/versjoner/sporing, Posten BankID, provisjonering, eier-onboarding, landingssider per tenant), fakturamotor, purring→§4-18, oppgjørsrapporter, klientkonto (PowerOffice), honorar-netting, eierportal/brief, Cases v2, ops-senter, autopilot, Channex. Alt på /utleiemeglere er grunnet i dette.
- Hero (HeroStage): veggtekst filmatisk fade (v4-ord-fade/-ut, pause 640 ms), nye tekster (hele setninger), telefonstrøm = moderne chat-bobler (Emma/eier/DigiHome, skriveindikator, bildeboble m/ 3 thumbnails, kvitteringer, hårlinje fra telefonen beholdt, glød fjernet), video hentes straks og fortellingen venter på `playing` (fallback 2,8 s). AdresseFelt: ingen Start-knapp — valg går rett videre.
- Kom-i-gang-velger: Huseier-kort bruker eier-hjemme-1920.webp (samme mann som heroen).
- SEO: scripts/lag-og.py lager 1200×630 OG-bilder (egen mal, sidens fonter) → public/og/<slug>.jpg for 19 nøkkelsider; alle disse har openGraph.images + twitter summary_large_image. Kjør scriptet på nytt ved tekstendringer. Forsiden bruker fortsatt public/og/forside.jpg.
- Ikke brukerbekreftet: alt over.

## Økt 2026-09-08 (kveld) — oppsummering
- PROD-BILDER: digihome.no manglet alle /v4-, /og-, /brand-filer (standalone-prod har ikke /public; fallback → /api/media → objektlagring, og filene var aldri lastet opp). Kjørte `node scripts/upload_public_to_storage.mjs` (327 filer, 0 feil) → 200 på prod. Regel: nye filer i /public må lastes opp (`--only v4` osv.) før deploy.
- Cache-felle tettet: sti-regelen for bilder i next.config `headers()` ga også 404-svar `public, max-age=86400` (Cloudflare normaliserte, nettlesere husket brutte bilder et døgn). Regelen fjernet — serveMedia eier headerne (200 → 1 år immutable, 404 → no-store). Krever deploy.
- AdresseFelt: pil/knapp fjernet — bare feltet (px-5), Enter/listevalg går videre, teksten dempes mens siden bytter.
- /ledige-boliger viste «Ingen ledige» på prod mens forsiden viste 7 ledige: siden var ISR (revalidate 120) og ble bygget i CI uten DB → tom versjon i cache til første regenerering etter hver deploy. Nå `dynamic='force-dynamic'`; `getPublishedListingsResult()` skiller DB-feil (ok:false → nøytral tekst i side + ListingsGrid `dbOk`) fra «ingen ledige». Samme for app/sitemap.js (prod-sitemap hadde 0 boligsider). Metadata-feil rettet (twitter lå inni openGraph).
- Avtale om selvforvaltning skrevet om (lib/avtale-selvforvaltning.js), 21 punkter, versjon `selvforvaltning-2026-09` (plattformen lagrer versjonsstrengen fritt): partene/inngåelse, ordforklaringer, hva DigiHome gjør / hva eier gjør, fullmakt, annonsering + ny utleie via DigiHome, leiekontrakt, husleie/klientkonto/utbetaling (frigjørende utbetaling, motregning), purring/inkasso (eiers beslutning), depositum, honorar (5 % av innbetalt leie, ingen minstepris, forsinkelsesrente), eiers ansvar (lovlig utleie, forsikring, skatt, diskrimineringsvern, skadesløsholdelse), konto/portalbruk/suspensjon/e-kommunikasjon, tredjeparter/AI, IP/anonymiserte data/90 d nedlasting, personvern + databehandleravtale (art. 28), ansvarsbegrensning (12 mnd honorar, ikke ved grov uaktsomhet), varighet/oppsigelse (ut måneden), angrerett (14 d, start straks), endringer/overdragelse, lovvalg/Bergen tingrett. Kvitterings-e-post (lead-emails.js) lenker nå til avtalen + angrerett/angreskjema for privatpersoner. Bridge `pdf_url` → /avtale/selvforvaltning (var /vilkar). MVA: «inkl. mva der avgiftspliktig» — avklar med eier om bedrifter skal ha mva i tillegg.
- HERO (endelig etter flere runder — bruker likte boblene ved telefonen, ikke veggtekst som skifter, ikke tom vegg): boblene står over skulderen hans som før, men som GLASS-kort (backdrop-blur, tre materialer: mørkt/lilla/hvitt, fjærende inn `v4-glass-inn`, ut `v4-glass-ut`, innhold 140 ms etter), «pakke» = lysprikk som reiser langs hårlinjen (offset-path) 520 ms før hver boble + ring fra skjermpunktet, bildene fanner ut (`v4-foto-fan`, --fra/--rot), haken tegnes (`v4-hake-tegn`), kolonnen svever (`v4-flyt`). Veggen: ÉN blokk i lyset (`Veggstatement`, left max(61%,…), top 27 %): meta-linje (adresse, Bergen · torsdag kl + fem kapittelstreker som fylles per slag) og statementet «Kvelden er din.» (display clamp(48px,5vw,96px), lilla punktum) — kommer én gang, står stille. Mobil: samme blokk nederst over gradient; kortstabelen forankret ved Y+18 så Emma-slaget får plass. useFortelling bruker STROM (5 slag) og HVILER på siste. Siste boble: «Alt i orden på {adresse}. Ingenting venter på deg — god kveld.» Forkastet underveis: samtale projisert på veggen (bruker: «likte mye bedre sånn det var»), statisk «Én beslutning var din.» på veggen (bruker: «ikke moderne»), caption i hjørnet alene (bruker: «litt tomt»).
- (historikk) HERO-forsøk: samtalen projiseres ut på veggen (bruker: «hva om animasjonen viste ute på veggen»). Telefonstrom: kolonne ~400 px sentrert på 78 % (min 58 %), bunn på 60 % av høyden, 15 px tekst / 72 px bilder; hårlinje 1 px fra telefonen (lys) til kolonnens nedre venstre hjørne (blekk-gradient); stille linje under (adresse · klokke · ● Alt i orden). Veggoverskriftene, undertekstene, tallene og fremdriftslinjen er FJERNET i direkte-modus (Veggfortelling → null). useFortelling bruker STROM (5 slag, 4,8/5,4/4,6/6,6 s) og HVILER på siste slag — ingen loop. Siste boble bærer poenget: «Alt i orden på {adresse}. Én beslutning var din i dag — rørleggerens pris. God kveld.» Mobil: som før (bobler oppe til høyre), ingen vegglinje.
- Investordeck-modul (DeckModul.js + API) fortsatt IKKE koblet inn i admin-ruteren — venter.
- Ikke brukerbekreftet: alt over.
- HERO-VEGG (nåværende): `Veggkort` — editorialt boligkort rett på veggen (ingen mockup): kartbånd tone-i-tone (bergen-kart-m.svg, maskede kanter, lilla nål i sentrum), adressen i display, «5015 Bergen · Leilighet 2», hårlinjerader Byggeår/Areal/Etasje/Leie/Leietaker. To stille endringer: Leietaker «Ledig fra 1. november» → Emma m/ bilde etter slag 1; Leie får «● betalt» etter slag 2. PERSPEKTIV (rettet etter bruker: «HELT feil» — veggen kommer MOT kamera på høyre side, gulvlisten faller mot høyre): `perspective(1000px) rotateY(-14deg)`, origin venstre kant, left 60 % / right 8 % / top 12 %; `mix-blend-mode: multiply` (desktop) så lyset i filmen går gjennom blekket; radene er et spesifikasjonsark: etikett i smal venstrekolonne (versaler 10.5 px, sperret 0.14em, dempet) + verdi 16 px venstrestilt, hårlinje under — kortet ligger på veggen som viker mot høyre. Mobil: samme kort nederst over gradient, uten perspektiv. Forkastet: EierApp-mockup (components/forside/v4/EierApp.js ligger igjen ubrukt — kan slettes), «Kvelden er din.», caption i hjørnet.
- ANNONSE-FILM scene 2–3 (AnnonseFilm.js): arket i full scenebredde (SW=VW, fasade 56 %), kartet ligger svakt under arket (opacity .26, sakte zoom) FUNNET→TRYKK_START og forsvinner ved FELT; faktaradene «slås opp» (lilla linje `v4-slaa-opp` langs bunnen, verdi lander, kilde m/ grønn prikk 300 ms etter, takt 420 ms); «Lag annonse»-knapp, peker i åpningen, dropzone og «hånd» er FJERNET (desktop + mobil). FELT: fasaden glir til mosaikkplassen; DRA: de fire flyr inn fra høyre og snapper (LANDING, 170 ms stagger, stue først). AUTO: FUNNET 3200, PEKER 700, HOVER/TRYKK_START 1 ms (tomme faser), FELT 1300, DRA 1900, SLIPP 500, BUNKE 300. Felt/HoldtBunke-funksjonene står ubrukt i filen.
- ANNONSE scene 2 (FUNNET) v3: arket er ETT oppslag i hele scenebredden — hode (adresse 26 px · sted · hake · KOORD «60.3878° N · 5.3252° Ø» ytterst til høyre), bredt fasadepanel 2.5:1 (`FasadePanel`, «utvikles» inn med skala 1.04→1 + lapp «Fasade · funnet fra adressen»), og `Spekk`: fem kolonner (Byggeår/Areal/Etasje/Kollektiv/Leie i området; grid 0.85/1/0.9/1.05/1.2fr) med etikett i versaler, verdi 17 px (proporsjonale tall — tabular-nums fjernet, Diatype tnum så ut som mono), kilde m/ grønn prikk; topp-hårlinje tegnes, kolonnene slås opp i takt 440 ms med lilla `v4-slaa-opp`. HODE_H 40. Fasade-flisen er usynlig under FUNNET og lander (1.06→1) på mosaikkplassen ved FELT mens panelet toner ut. Mobil (Kompakt) bruker fortsatt Boligfakta-rader + FasadeLapp.
- HERO Veggkort v2 (animasjon «neste nivå»): kartet avdukes fra nålen (`v4-kart-avduk` clip-path), nålen lander (`v4-naal-land`), ring én gang + langsom pust, adressen ord for ord (`v4-ord-fade`), hårlinjer tegnes (`v4-linje-tegn`) rad for rad, etikett/verdi trykkes (`v4-verdi-inn`); endringer lander idet boblen kommer (signert +1000 ms i slag 1, betalt +2000 ms i slag 2) via `Bytt` (gammel ut/ny inn + lilla sveip `v4-sveip`) og `v4-dot-pop`.
- /BEDRIFT HERO v2 (bruker: «forbedre alt med denne heroen»): PortefoljeScene skrevet om til «byen er dashbordet» — Bergen-kartet (samme SVG) tone-i-tone i panelet med vignett, hvert bygg en nål (POS i kart-koordinater, illustrativt) med etikett «adresse · enheter», anonyme småbygg for «+N bygg til». Dagen spilles som glasskort VED nålene (hendelser(): husleie m/ teller ved Nygårdsgaten — alle nåler pulser grønt i en bølge fra sentrum; saker ved Kong Oscars gate; kontrakt + låsbytte (Nora) ved Solheimsgaten; fasadevask ved Strandgaten = lilla, pulserende). Godkjenningskortet er mørkt glass ved Strandgaten-nålen (interaktivt; Ola etter 5 s), kvittering «Fasadevask bestilt · uke 46» etterpå. Bunnlinje: siste hendelse + «Spill igjen». Etiketten skjules når et kort står ved nålen. FASER lengre (rad 2,0–2,3 s). Venstre kolonne: segmentkontrollen erstattet av stille tekstlinje «Se dagen for en portefølje på 10–50 · 50–250 · 250+ enheter» (lilla understrek på valgt), «30 minutter»-linjen fjernet, CTA-rad rett under ingressen.


## Oppdatering 8. sep 2026 — forside-tråd, boligeiere, «Hele utleien», priser, systemseksjon
- **Telefonstrom** (heroens samtale) flyttet til `components/forside/v4/Telefonstrom.js`: ekte tråd (nye meldinger nederst, eldre skyves opp og tones bort i maske), skriveindikator som MORFER til boblen, ord-for-ord ut av uskarphet, eierens bobler sendes fra telefonen i lett bue, godkjenning INNE i samtalen (kostnad + Godkjenn → grønn Godkjent + kvittering), DigiHome-merke som avatar (ren lilla sirkel), bilder uten skall, ingen kanter på boblene, kun svakt glød ved telefonen når noe kommer inn. Ekte fjærkurver via `--dh-fjaer/--dh-land` (linear()) i globals.css.
- **/boligeiere**: ny hero i stage-layout + `boligeiere/AarScene.js` («ett leieår»: fasade kveld→morgen, tidslinje med 12 måneder, hendelser én om gangen, glass-godkjenningskort som stopper året). TillitStripe alltid på. Veiskille: metalinje + nummererte hårlinjerader. Sammenligning: lilla = din beslutning, grønn = skjer uten deg, nummererte rader, legende.
- **AltSamletSeksjon** («Hele utleien. Ett sted.»): register til venstre (velg med trykk — ikke hover), scene til høyre uten boks: fem flate produktfragmenter (samme leietaker Emma), auto-leselys med fremdriftslinje (4,2 s), forklaringslinje.
- **FaqSeksjon**: register med nummer, pluss→minus, første svar åpent, e-postlenke.
- **BoligerSeksjon**: statuslinje per bolig (grønn utleid / lilla ledig), parallakse i bildene, stegvis inntoning, ledige kort lenker til `/ledige-boliger/<slug>`.
...
## Oppdatering 9. sep 2026 (2) — prod-bilder fikset + «Hvorfor»-slide ombygd
- **Prod-bilder (digihome.no):** flere `/public`-assets (forsidens annonse-scene: v4/annonse/fasade-kveld-1920.webp, kjokken-1280.webp, soverom-useng-1000.webp, spisestue-1280.webp, stue-tom-1200.webp, bergen-kart.svg, leietaker-*.webp, v4/bolig-oslo-dempet-2000.webp, v4/kari.webp) 404-et fordi de aldri var lastet til objektlagringen. Objektlagring + EMERGENT_LLM_KEY fungerer på prod (digihome-icon-purple.svg gav 200). Fiks: kjørte `node scripts/upload_public_to_storage.mjs` → alle 327 filer lastet opp (0 feil). Serveres i runtime via /api/media, så kun hard refresh trengs – ingen ny deploy.
- **Hvorfor (slide 02) — «før→etter»-buen:** `Fragmentering` bygget om fra fannede «gule lapper»-kort til en FRAKOBLET konstellasjon — rolige, ensartede hvite noder (lilla ikon-chip) bundet av korte STIPLEDE «rekk» som aldri når frem (mellomrom + liten høyde-jitter), som ender i tomrom. Motsetningen til den sammenkoblede pipelinen i slide 04. Beholder tall-ankere 10/5. Ingen evig animasjon. Agent-testet (desktop 1920 + mobil 390), IKKE brukerbekreftet.

## Oppdatering 9. sep 2026 (3) — Hvorfor bygget om + ny «Løsningen»-slide (DigiHome-stil)
- **Hvorfor (02) fra scratch i DigiHome-stil:** varmt bolig-foto (v4/stue-2000.webp) med ett elegant «Autopilot AV»-overlay (toggle av, rødt AV, jobber som piller … manuelt). Kul animasjon: toggelen prøver å slå seg PÅ og faller tilbake til AV (loop, kun aktiv slide) + langsom Ken Burns-zoom. Profesjonell copy: H2 «Utleie mangler et system.», ingress (begge segmenter, AI), KOSTNADEN «Manuell drift skalerer med lønn.» Fjernet mørkt glass-panel/kaskade/verdikjede (bruker mislikte dem).
- **Ny slide «Løsningen» (03), motstykket:** lagt inn nytt kapittel i KAPITLER etter «hvorfor». Varmt foto (v4/video/eier-poster.webp) + «Autopilot PÅ»-overlay: toggle skrur seg PÅ og blir stående (animasjon), grønne haker på jobbene (… automatisk), grønn PÅ-badge. Copy: H2 «Boligdrift på autopilot.», ingress «ett AI-drevet system … automatisk», GEVINSTEN «Drift som skalerer uten bemanning.» (speiler KOSTNADEN). Ny narrativ bue: problem (autopilot av) → løsning (autopilot på, AI).
- Agent-testet (desktop 1920 + mobil 390, #hvorfor/#losning). IKKE brukerbekreftet.
- **KORRIGERT «Løsningen» (03):** bruker ba om et HELT EGET konsept (ikke foto+toggle-speil). Byttet til «AI-motoren»: de fire driftsområdene (Leietakere, Kontrakter, Husleie, Drift) som brikker strømmer via animerte stiplede lilla linjer (deck-los-stream) inn i én glødende, pulserende DigiHome AI-kjerne (Sparkles, deck-los-gloed). Pure SVG-linjer (viewBox 0 0 100 100, preserveAspectRatio none, non-scaling-stroke) + HTML-brikker på matchende %-koordinater = robust alignment. Responsivt: bue-inn på desktop (KjerneStage desktop), kompakt ring rundt kjernen på mobil (KjerneStage mobil). Fjernet toggle/foto-varianten og dens CSS. Agent-testet desktop+mobil, IKKE brukerbekreftet.
- **Hvorfor (02) løftet til samme nivå som Løsningen:** byttet foto+toggle (ManueltIDag, nå ubrukt) til `FragmentKjerne` — samme KjerneStage-diagram som Løsningen men variant="problem": grå brikker, brutte stiplede grå linjer (bane(d, 0.74) — stopper før midten), og et TOMT stiplet «Intet system» (Ban-ikon) i stedet for glødende AI-kjerne. Slide 02/03 er nå et bevisst før/etter-par i samme visuelle språk (fragmentering → ett AI-system). Agent-testet desktop+mobil, IKKE brukerbekreftet.

## Oppdatering 9. sep 2026 (4) — «Løsningen» (03): forsidens produktfilm i den hvite rammen, i høyre kolonne
- **Bruker-ønske:** vis forsidens nydelige produktanimasjon (annonse, drift osv.) på Løsningen-sliden, TILPASSET presentasjonen: animasjonen i HØYRE kolonne, uten bolig-bakgrunnsbildet.
- **Ny komponent `components/investor/LosningFilm.js`:** gjenbruker forsidens FRAMMEDE produktfilmer (AnnonseFilm/KontraktFilm/DriftFilm/OkonomiFilm → den hvite `Ramme`) + `Kapittelpille` (kompakt). Styrt av decket, IKKE skroll: `aktiv` (slide 03 fremme) er avspillingsporten — `spiller`/`synlig` = aktiv, så klokken (`useFilm`) går kun når sliden er fremme og PAUSER ellers (ytelse). Filmene lastes (dynamic) + monteres først ved første besøk («tent»). Kapitlene looper (onFerdig → neste), høyden holdes under bytte, manuelt valg via pillen.
- **Slide-layout:** `Todelt bredHoyre` (venstre 4/12 tekst, høyre 8/12 animasjon). Venstre: H2 «Boligdrift på autopilot.» + ingress + GEVINSTEN «Drift som skalerer uten bemanning.». Høyre: `LosningFilm` (kompakt pille + hvit ramme, ingen bakgrunn — står rett på decket). Mobil stabler (tekst over, ramme under; filmens kompakt-variant).
- Erstattet forrige «AI-motoren»-diagram (LosningKjerne nå ubrukt). Agent-testet desktop 1920 + mobil 390 (nav via kapittelmeny), kapittelbytte (Drift) bekreftet. IKKE brukerbekreftet ennå.

## Oppdatering 9. sep 2026 (5) — «Løsningen» (03) løftet til VERDENSKLASSE, bred 16:9-visning
- Bruker: animasjonen ble for smal/1:1 i høyre kolonne; på forsiden er den bred (16:9). Løft til verdensklasse.
- `components/investor/LosningFilm.js` bygget om til BRED hero: forsidens hvite ramme i full slide-bredde (~1360×660, ratio ~2.06 = forsidens proporsjon), med en HORISONTAL pipeline-indeks over (Annonse→Kontrakt→Drift→Økonomi). Koblingene mellom stegene fylles lilla i takt med filmens fremdrift (onFremdrift) — pipelinen «flyter» fra steg til steg og er samtidig navigasjonen (klikk = hopp). Topplinje: kapittelmarkør + H2 «Boligdrift på autopilot.» (ord-for-ord reveal) til venstre, «GEVINSTEN — Skalerer 1→1 000 enheter, uten nye ansatte» til høyre. Mobil: stablet, pipeline som nummer-badges (navn skjult), ingen overflow.
- Fortsatt deck-styrt avspilling (spiller kun når sliden er fremme; tent-gate for lazy mount). Agent-testet desktop 1920 + mobil 390, kapittelbytte (Drift) + sync bekreftet. IKKE brukerbekreftet ennå.

## Oppdatering 9. sep 2026 (6) — «Løsningen» (03): produktet som NYDELIG EDITORIAL full-bleed bakgrunn
- Bruker forkastet kort/boks-varianter (framed, 16:9, 1:1 stående) — «gjør animasjonen nydelig editorial på bakgrunnen».
- `components/investor/LosningFilm.js` bruker nå forsidens FULL-BLEED kino-filmer (AnnonseKino/KontraktKino/DriftKino/OkonomiKino) som editorial bakgrunn: foto fyller hele sliden, filmens egen editoriale tittel nede til venstre er helt (f.eks. «Vi ser det leietakere spør om.» + AI-etiketter). Oppå kun rolig deck-chrome: kicker «03 · Løsningen — Boligdrift på autopilot», tynn steg-pipeline (Annonse→Kontrakt→Drift→Økonomi) som fylles lilla i takt med filmen (onFremdrift) og er navigasjon, samt dempet gevinst «Skalerer 1→1 000 uten nye ansatte» (desktop). Farger/scrim følger scenens tone via onTema.
- Sliden er full-bleed (morkt), lagt til i morkSide. Deck-styrt avspilling beholdt (spiller kun når fremme; tent-gate). Agent-testet desktop 1920 + mobil 390. IKKE brukerbekreftet ennå.

## Oppdatering 9. sep 2026 (7) — «Løsningen» (03): SPLIT, animasjon frameless til høyre
- Bruker presiserte: split view — tekst venstre, animasjon HØYRE, UTEN hvit ramme (ikke full-bleed, ikke kort).
- `components/investor/LosningFilm.js`: venstre halvdel (46%) = lerret med investor-tekst + levende steg-pipeline (vertikal skinne, synk med filmen) + gevinst. Høyre halvdel (54%) = forsidens KINO-film som blør helt ut til kanten UTEN hvit ramme; filmens egen editoriale tittel er helt. Myk skjøt (canvas→transparent gradient) toner filmen inn i lerretet ved møtet. Mobil stabler: tekst+pipeline øverst (44%), film nederst (56%). Deck-styrt avspilling beholdt (spiller kun når fremme; tent-gate). Agent-testet desktop 1920 + mobil 390. IKKE brukerbekreftet ennå.

## Oppdatering 9. sep 2026 (8) — «Løsningen» (03): framede produktfilmer, START på adresse-steget, likt forsiden
- Bruker: animasjonen må STARTE med adresse-steget og være LIK forsiden (kino-versjonen startet feil, med foto-montasje).
- `components/investor/LosningFilm.js` bruker nå forsidens FRAMEDE produktfilmer (AnnonseFilm/KontraktFilm/DriftFilm/OkonomiFilm) i den hvite rammen — starter alltid på 'annonse' (adressesøk → kart → annonse), akkurat som forsiden. To-kolonne: venstre = levende steg-pipeline + tekst + gevinst; høyre = den framede filmen (lever naturlig innfelt, ikke kant-til-kant). Deck-styrt (spiller kun når fremme; tent-gate). Agent-testet desktop 1920 + mobil 390 — bekreftet start på adresse-steget. IKKE brukerbekreftet.

## Oppdatering 9. sep 2026 (9) — «Løsningen» (03) FERDIG i verdensklasse-retning (brukeren: «bedre», «fortsett å løfte»)
- **Endelig konsept (bruker-styrt):** forsidens EKTE produktfilmer (AnnonseFilm→KontraktFilm→DriftFilm→OkonomiFilm) rendres med `naken` (INGEN hvit ramme) rett på slidens bakgrunn. Lyse kapitler (Annonse/Kontrakt) gjøres gjennomsiktige via scoped CSS `!important` (kun innenfor `[data-testid="deck-losning-film"]`, rører ikke forsiden). Mørke kapitler (Drift/Økonomi) beholder kino-fotoet.
- **Layout (fits viewport, INGEN scroll):** desktop = flex-kolonne med fast høyde `calc(100svh-184px)`; `FilmSkala` skalerer filmen (designhøyde 660) presist til resten av høyden (render bredere w/k + `translate(-50%,-50%) scale(k)`, sentrert; k cappet til 1 → full størrelse på normale skjermer). Mobil = naturlig flyt (Kompakt-variant).
- **Chrome (minimalt, editorial):** kicker «• LØSNINGEN» + én linje «Ett system tar over hele driften.» (ikke stor dobbel-overskrift — bruker kalte den «jævlig»). TIDSLINJE-stegindeks: 4 noder på hårlinje (fylt = ferdig, glødende = aktiv, hul = kommende), lilla linje rykker frem med filmens `onFremdrift`. Klikkbar. Lit-stage: varm radial lysning bak, hvisken lilla bak filmen. Filmen har `rounded-[24px]`.
...
## Oppdatering 9. sep 2026 (13) — 06 + 07 løftet til EKTE konsept + moderne tekst
- Bruker ville ha «myyye bedre konsept», moderne tekst, og «svinghjul høres rart ut».
- 07 STRUKTUREN → «Motoren»: ny Svinghjul-komponent (funksjonsnavn), men ALL synlig tekst = «Motoren». Roterende hjul: ring tegner seg inn → steg 1–4 avsløres med klokka → nav (Digihome Tech ⇄ lisens nuller seg ut ⇄ Digihome AS) → glødende verdiprikk i bane rundt. Moderne steg: «Vi forvalter flere boliger» → «Hver bolig betaler og lærer opp plattformen» → «Plattformen blir bedre og billigere» → «Salget går lettere · inntekten gjør runden på nytt». H2 «To selskaper. Én motor.» Mobil = vertikal nummerert loop.
- 06 FOR HVEM → SPEKTER: «Du gjør det selv → Vi gjør alt». Tre stopp (Private=Gjør det selv / Eiendomsselskaper=Drift i skala / Forvaltning=Vi gjør alt) på en skinne som fylles (deck-spk-skinne scaleX), plattform-base under. H2 «Én plattform. Du bestemmer hvor mye du gjør selv.»
- CSS: deck-hjul-tegn/-rot/-chev/-orbit/-lisens + deck-spk-skinne; alle i reduced-motion-reset. Verifisert: kompilerer; desktop 1920 overflowY~3 begge; mobil 390 offenders=0 begge. IKKE brukerbekreftet.

## Oppdatering 9. sep 2026 (14) — 08 «Organisasjon» + 09 «Hvor vi står» kraftig løftet
- 08 → people-forward: erstattet boks-org-kartet med tre store redaksjonelle PortrettKort (Sarah/Martin/Kevin: foto + navn/rolle brent inn + drepende referanse), venstre «hvorfor oss»-kolonne (Operatør+AI+jus / Allerede bygget og i drift / Felles styre), og en felles-styre-stripe med 4 overlappende avatarer. H2 «Bygget av utleiere, for utleiere.» OrgKart-komponenten nå ubrukt (beholdt).
...
## Oppdatering 10. sep 2026 (22) — PowerOffice: miljø-riktige nøkler (demo+prod) + redigerbare tilkoblinger
- Bruker: «kan jeg redigere PO-tilkoblingene? + vi har andre PROD-nøkler + få det til å fungere på preview og prod».
- PROD-NØKLER lagt i .env: POWEROFFICE_APP_KEY_PROD=2c65396a-75d1-4605-80a1-d3dcba798565, POWEROFFICE_SUBSCRIPTION_KEY_PROD=a2262c088c644e2d9609d7022c2ebab5 (DigiHome Tech AS eier integrasjonen «DigiHome»). Demo-nøkler også speilet til _DEMO. Generiske POWEROFFICE_APP_KEY/SUBSCRIPTION_KEY beholdt som demo-fallback.
- lib/poweroffice.js: NY creds(env) velger app+abonnementsnøkkel etter selskapets miljø (demo/prod hvert sitt par); hentToken/poFetch bruker creds(env); poFellesKonfigurert(env?) sjekker per miljø eller «minst ett». Token-cache allerede env-scoped.
- VERIFISERT: demo virker fortsatt; POST env=production m/demo-klientnøkkel traff PROD-OAuth (invalid_client «client integration not found») = prod app/sub korrekt wired. Mangler kun ekte PROD KLIENTNØKKEL for DigiHome AS (Martin må legge «DigiHome»-utvidelsen i DigiHome AS' prod Go-klient og kopiere klientnøkkelen).
- RegnskapModul.js: la til REDIGER av tilkobling (Endre-knapp → inline: navn, ny klientnøkkel valgfri, miljø demo/prod → PUT tester tilkobling før lagring). presentasjon-modus skjuler fortsatt all admin i deck.
- DEPLOY-NOTAT: samme POWEROFFICE_*-env må finnes i Emergent prod-deploy. «demo/production» på selskapsnivå er PO-miljø (uavhengig av Emergent preview/prod) — et prod-selskap treffer ekte PO fra både preview og deploy.
