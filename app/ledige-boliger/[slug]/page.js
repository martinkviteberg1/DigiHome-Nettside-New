...
//
// Personvern: gatenavn (uten husnummer), bydel, areal, soverom og
// prisintervall. Aldri husnummer, eier, leietaker eller eksakt leiebeløp.
//
...
export default async function ListingPage({ params, searchParams }) {
  ...
  const ld = {
    ...
    about: {
      ...
      address: {
        '@type': 'PostalAddress',
        ...(listing.area ? { streetAddress: listing.area } : {}),
            ...
            {related.map((c) => (
              <Link key={c.id} href={`/ledige-boliger/${c.slug}`}
                ...
                <div className="p-5">
                  ...
                  <p className="mt-1.5 flex items-center gap-1.5 text-[13px] text-[#78726a]"><MapPin className="h-3.5 w-3.5 text-[#c9c3ba]" />{[c.area, c.district].filter(Boolean).join(', ')}</p>
                  <p className="mt-3 text-[14px] font-semibold text-[#0a0a0a] tabular-nums">{c.rentBand || 'Pris på forespørsel'}</p>
                </div>
              </Link>
            ))}
          </div>
