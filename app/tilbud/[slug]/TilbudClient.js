...
const D = { chip: 'rgba(255,255,255,0.07)', chipLine: 'rgba(255,255,255,0.1)', icon: 'rgba(255,255,255,0.92)', soft: 'rgba(255,255,255,0.62)', faint: 'rgba(255,255,255,0.42)' };
...
function Rad({ l, v, bold, muted, top, dark }) {
  return (
    <div className="flex items-baseline gap-3" style={top ? { paddingTop: 12, borderTop: `1px solid ${dark ? 'rgba(255,255,255,0.12)' : C.hairline}`, fontWeight: 600, fontSize: 14 } : undefined}>
      ...
      <span aria-hidden className="mb-[3px] flex-1 self-end border-b border-dotted" style={{ borderColor: dark ? 'rgba(255,255,255,0.2)' : 'rgba(10,10,10,0.16)' }} />
...
function MiniAnnonse({ variant, bilde, aiBilde, tittel, adresse, pris, netto }) {
  ...
  return (
    <div className="flex flex-col overflow-hidden rounded-[26px] transition-all duration-300 hover:-translate-y-1"
      style={{ backgroundColor: C.bg, border: dh ? `1.5px solid ${C.ink}` : `1px solid ${C.line}`, boxShadow: dh ? '0 18px 50px rgba(17,24,39,0.1)' : '0 6px 22px rgba(24,20,16,0.04)' }}>
...
export default function TilbudClient({ tilbud, slug }) {
  ...
  // Data er server-rendret — ingen spinner. Tomt tilbud håndteres av serverkomponenten.
                ...
                <Reveal key={t} delay={i * 0.08} className="h-full">
                  <div className="h-full pt-7" style={{ borderTop: `1px solid ${D.chipLine}` }}>
                    <div className="mb-7 flex items-baseline justify-between">
                      <span className="font-light tabular-nums tracking-[-0.03em]" style={{ ...head, fontSize: 42, color: 'rgba(255,255,255,0.2)' }}>{`0${i + 1}`}</span>
                      {i === 2 && (
