...
// ---------------------------------------------------------------------------
// Nav3 — helt clean topp. Kun logo før scroll. Hele høyre-siden (Logg inn +
// Bli utleier + hamburger) glir inn samtidig når man begynner å scrolle. Gir
// bildet full luft til å strekke seg helt opp til toppen av viewporten.
// ---------------------------------------------------------------------------
...
export default function Nav3() {
  ...
  const startOnboarding = () => {
    try { track('cta_click', { cta: 'nyest3_nav' }); } catch (e) { /* analyse skal aldri blokkere */ }
    setOpen(false);
    router.push('/bli-utleier/start');
  };

  return (
    <header className="fixed left-0 right-0 top-0 z-50" data-testid="nyest3-nav">
      <div
      ...
      >
        <div
          className={`flex w-full items-center justify-between gap-4 px-5 transition-all duration-500 sm:gap-6 sm:px-8 lg:px-12 ${
            scrolled ? 'h-[60px]' : 'h-[72px] sm:h-[76px]'
          }`}
        >
          <Link href="/" className="group flex shrink-0 items-center" data-testid="nyest3-nav-logo">
            <img
          ...
          {/* Hele høyre-siden gjemmes før scroll — bildet får hele toppen. */}
          <div
            className={`flex items-center gap-1.5 transition-all duration-500 sm:gap-3 ${
              scrolled ? 'translate-y-0 opacity-100' : 'pointer-events-none -translate-y-1.5 opacity-0'
            }`}
          >
            <Link
              href={site.loginUrl}
              data-testid="nyest3-nav-login"
        ...
        {/* Side-drawer — samme som /nyest og /nyest2. */}
        <div
