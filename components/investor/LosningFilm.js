...
/* ─────────────────────────────────────────────────────────────────────────────────────
   ...
   Dette ER forsidens produktanimasjon (samme filmer: Annonse → Kontrakt → Drift
   → Økonomi). Eneste forskjell fra forsiden: vi viser den IKKE inni den hvite
   rammen (Ramme/kort) — filmene rendres med `naken`, så animasjonen skjer rett
   på slidens bakgrunn.

   Deck-styrt: `aktiv` = sliden er fremme. Filmene spiller kun da (synlig+spiller),
   og pauser når man går videre — ytelse. Når et kapittel er ferdig, tar vi over
   (onFerdig → true) og bytter til neste; livssyklusen er en sirkel. Kapittel-
   indeksen øverst (som på forsiden) viser hvor vi er, og kan klikkes.
   ───────────────────────────────────────────────────────────────────────────────────── */

import React, { useCallback, useEffect, useRef, useState } from 'react';
import dynamic from 'next/dynamic';
import { T } from '@/components/forside/v4/tokens';

/* Filmene hentes som egne chunks — først når kapitlet skal vises. Samme komponenter som forsiden bruker. */
...
export default function LosningFilm({ aktiv = false, nr = 3 }) {
  ...
  const [frem, setFrem] = useState({ andel: 0, ms: 0 });    // fremdrift i aktivt kapittel (til den lilla linja)
  ...
  /* Samme props som forsiden gir filmene — men naken (ingen hvit ramme) og tema 'lys' (som forsidens standard). */
  ...
  return (
    <div className="w-full" data-testid="deck-losning-film" data-kapittel={kap}>
      {/* Kapittelindeksen — som på forsiden, men rett på slidens bakgrunn */}
      <div role="tablist" aria-label="Kapitler i løsningen" className="mb-6 flex items-center justify-center gap-6 sm:gap-10 lg:mb-8">
        {KAP.map((k, i) => {
          const er = k.id === kap;
          return (
            <button
      ...
      {/* Filmen — forsidens animasjon, uten ramme, rett på bakgrunnen. Mykt kryssbytte mellom kapitlene. */}
      <div key={kap} className="animate-in fade-in-0 duration-500" style={{ opacity: bytter ? 0 : 1, transition: 'opacity 340ms ease' }} data-testid="deck-losning-scene">
