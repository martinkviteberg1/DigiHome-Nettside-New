...
/* ═══════════════ Signering — modul for BankID-signeringsrunder ═══════════════
   Full oversikt over alle signeringsoppdrag (fra saker og frittstående),
   opplasting av nye dokumenter rett til signering, purring, kansellering og
   status i sanntid. Gjenbruker DokumentModal for selve handlingene —
...
import {
  ...
  XCircle, Ban, ChevronRight, Send, RefreshCw, Search,
} from 'lucide-react';
...
export default function SigneringOversikt({ apiKey, user }) {
  ...
  const [dokFil, setDokFil] = useState(null); // {fil, taskId} → DokumentModal
  ...
  const hentAlt = useCallback(async () => {
    try {
      const [rj, rd, ro] = await Promise.all([
        api('signering/jobber'), api('dokumenter'), api('signering/oppsett'),
      ]);
      const [jj, jd, jo] = await Promise.all([rj.json(), rd.json(), ro.json()]);
  ...
  const lastOpp = async (file) => {
    ...
    try {
      ...
      // Åpne dokumentpanelet direkte så signering kan startes med én gang
  ...
  return (
    <div className="mx-auto max-w-[1100px]" data-testid="signering-modul">
      ...
      <div
        ...
        className={`mt-4 rounded-2xl border-2 border-dashed p-6 text-center transition-colors sm:p-8 ${drar ? 'border-[#7c3aed]/60 bg-[#f8f5ff]' : 'border-black/[0.09] bg-white hover:border-black/[0.16]'}`}
        data-testid="signering-dropzone"
      >
        <input ref={filRef} type="file" accept=".pdf,application/pdf,.docx,.doc,image/*" className="hidden" onChange={(e) => lastOpp(e.target.files && e.target.files[0])} data-testid="signering-filinput" />
        ...
        <p className="mt-3 text-[14.5px] font-semibold text-[#0a0a0a]">Send et dokument til BankID-signering</p>
        <p className="mx-auto mt-1 max-w-[420px] text-[12.5px] leading-relaxed text-[#a8a29a]">Slipp en PDF her, eller velg fil — så velger du signatarer, rekkefølge og frist i neste steg</p>
        <button
          onClick={() => filRef.current && filRef.current.click()}
          disabled={lasterOpp}
          data-testid="signering-lastopp"
        ...
        >
          {lasterOpp ? <><Loader2 className="h-3.5 w-3.5 animate-spin" /> Laster opp … {prosent}%</> : <><Upload className="h-3.5 w-3.5" /> Velg dokument</>}
        </button>
      </div>

      {/* Frittstående dokumenter (uten aktiv jobb vises de her for videre handling) */}
      {dokumenter.length > 0 && (
        <div className="mt-6">
          <p className="text-[10.5px] font-bold uppercase tracking-[0.1em] text-[#a8a29a]">Opplastede dokumenter</p>
          <div className="mt-2 overflow-hidden rounded-2xl border border-black/[0.06] bg-white">
            {dokumenter.map((f, i) => (
              <button
                key={f.id}
                onClick={() => setDokFil({ id: f.id, name: f.name, type: f.type, size: f.size, taskId: 'DOKUMENTER' })}
                data-testid={`signering-dok-${f.id}`}
                className={`group flex w-full items-center gap-3 px-4 py-3 text-left transition-colors hover:bg-[#fbfaf9] ${i > 0 ? 'border-t border-black/[0.04]' : ''}`}
              >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block text-[11px] text-[#b0aca6]">{fmtStr(f.size)} · lastet opp {fmtDato(f.at)}{f.uploadedBy ? ` av ${f.uploadedBy}` : ''}</span>
                </span>
