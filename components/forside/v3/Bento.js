...
/* ---------------------------------------------------------------------------
   ...
   skala. Hvite celler på varm off-white; produktflaten inni er varm ivory
   slik appen selv er. Ingen ikon+tekst-celler. Hver celle: ett utsnitt,
   én tittel, én linje. Etiketter i setningsform — aldri versaler.
--------------------------------------------------------------------------- */
...
function Celle({ t, b, className = '', children, testid }) {
  ...
  return (
    <div ref={ref} className={`group relative flex flex-col overflow-hidden rounded-[16px] border border-[#0F0E10]/[0.08] bg-white transition-colors duration-300 hover:border-[#0F0E10]/[0.18] ${className}`} data-testid={testid}>
      <div className="px-6 pt-6">
        <p className="text-[19px] font-medium tracking-[-0.02em] text-[#0F0E10]">{t}</p>
        <p className="mt-1.5 text-[14.5px] leading-[1.55] text-[#0F0E10]/55">{b}</p>
      </div>
...
/* Utsnitt-ramme: varm produktflate som kuttes av cellekanten */
function Flate({ className = '', children }) {
  return (
    <div className={`mx-5 rounded-t-[12px] border border-b-0 border-[#0F0E10]/[0.06] bg-[#F3F0EA] p-5 ${className}`}>{children}</div>
  );
}

const Etk = ({ children }) => <p className="text-[11.5px] font-medium text-[#8A867F]">{children}</p>;
  ...
  return (
    <Flate className="h-[250px]">
      <div className="flex items-start justify-between gap-6">
        <div>
          <Etk>Utbetalt i år</Etk>
          <p className="mt-1.5 text-[34px] font-bold leading-none tracking-[-0.035em] text-[#111827]" style={heading}>{tall(55500)} <span className="text-[14px] font-normal text-[#a49e93]">kr</span></p>
          ...
          {MND.map((m, i) => {
            ...
            return (
              <div key={m} className="flex flex-1 flex-col items-center gap-1.5">
                <span className="w-[62%] origin-bottom rounded-[3px]" style={{ height: '64px', background: betalt ? '#111827' : 'transparent', boxShadow: betalt ? 'none' : 'inset 0 0 0 1px #D9D3C8', transform: betalt && !inne ? 'scaleY(0.06)' : 'none', transition: `transform 900ms ${EASE} ${i * 140}ms` }} />
      ...
      <div className="mt-5 overflow-hidden rounded-[14px] border border-[#E8E5DF] bg-white">
        {[
          ['Mars', 'Betalt 1. mars · KID'],
          ['Februar', 'Betalt 1. februar · KID'],
          ['Januar', 'Betalt 2. januar · KID'],
        ].map(([m, s], i) => (
          <div key={m} className={`flex items-center gap-3 px-4 py-2.5 ${i > 0 ? 'border-t border-black/[0.04]' : ''}`}>
            ...
            <span className="text-[12.5px] font-bold text-[#111827]" style={heading}>{tall(18500)} kr</span>
          </div>
        ))}
      </div>
    </Flate>
  ...
  return (
    <Flate className="h-[250px]">
      ...
      <div className="mt-3 space-y-2">
        <Inn vis={inne} delay={100} dy={8}><div className="max-w-[86%] rounded-[14px] rounded-bl-[5px] border border-[#E8E5DF] bg-white px-3.5 py-2.5 text-[12.5px] leading-[1.45] text-[#111827]">Hei! Varmtvannet er borte 😬</div></Inn>
        ...
        <Inn vis={inne} delay={1400} dy={8}><div className="max-w-[60%] rounded-[14px] rounded-bl-[5px] border border-[#E8E5DF] bg-white px-3.5 py-2.5 text-[12.5px] text-[#111827]">Perfekt, takk! 🙏</div></Inn>
      </div>
      <div className="mt-3 flex items-center gap-2 rounded-full border border-[#E8E5DF] bg-white px-4 py-2.5">
      ...
      <div className="overflow-hidden rounded-[14px] border border-[#E8E5DF] bg-white">
        {[
          ...
          <div key={t} className={`flex items-center gap-3 px-4 py-3 ${i > 0 ? 'border-t border-black/[0.04]' : ''}`}>
            <span className="flex h-9 w-9 shrink-0 items-center justify-center rounded-[10px] bg-[#F3F0EA]"><FileText className="h-4 w-4 text-[#57534e]" strokeWidth={1.7} /></span>
    ...
    <div className="mx-auto w-[236px] rounded-[34px] bg-[#0a0a0a] p-[6px] shadow-[0_44px_96px_-32px_rgba(23,18,12,0.42),0_0_0_1px_rgba(0,0,0,0.1)]" style={{ transform: 'translateY(6px)' }}>
      <div className="relative overflow-hidden rounded-[28px] bg-[#F7F5F1]">
        ...
        <div className="flex items-center justify-between px-5 pt-2.5">
          <span className="text-[9.5px] font-bold text-[#0a0a0a]">9:41</span>
        ...
        <div className="px-3.5 pb-3 pt-4">
          ...
          <div className="relative mt-3 overflow-hidden rounded-[16px] bg-[#0f0d0b] p-3">
            <div className="flex items-center justify-between">
              <p className="text-[8px] font-medium text-white/60">Neste husleie</p>
            ...
            <p className="mt-1.5 text-[20px] font-bold leading-none tracking-[-0.02em] text-white" style={heading}>{tall(18500)} <span className="text-[9px] font-medium text-white/40">kr</span></p>
            <p className="mt-1.5 text-[7.5px] text-white/55">Trekkes automatisk 1. april · KID</p>
          </div>
          <p className="mt-3 text-[8.5px] font-medium text-[#8d877d]">Min bolig</p>
