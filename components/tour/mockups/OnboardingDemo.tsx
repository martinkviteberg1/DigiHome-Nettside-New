...
// ---------------------------------------------------------------------------
// OnboardingDemo — super-minimalistisk, selvspillende demo uten ramme eller
// browser-chrome. Ett nydelig avrundet adressefelt flyter på bakgrunnen:
// adressen skrives inn tegn for tegn, forslag glir opp, boligdata toner inn —
// så morpher feltet til et kontaktskjema (første kontaktfelt ligger nøyaktig
// der adressefeltet sto, så overgangen leses som én form som skifter innhold).
// Starter når demoen er synlig, nullstilles utenfor viewport, og looper rolig.
// ---------------------------------------------------------------------------
...
export default function OnboardingDemo({ onKonto }: { onKonto?: (iKonto: boolean) => void }) {
  ...
  const glid = 'transition-all duration-[600ms] ease-[cubic-bezier(0.16,1,0.3,1)]';

  return (
    <div ref={rot} className="relative mx-auto w-full max-w-[520px]" data-testid="tour-onboarding-demo">
      {/* Spørsmålet over feltet — krysfader mellom fasene. */}
      <div className="relative mb-6 h-[26px]">
        <p
          className={`absolute inset-0 text-[17px] font-bold tracking-[-0.02em] ${glid} ${
            iKonto ? '-translate-y-2 opacity-0' : 'translate-y-0 opacity-100'
          }`}
        ...
        <p
          className={`absolute inset-0 text-[17px] font-bold tracking-[-0.02em] ${glid} ${
            iKonto ? 'translate-y-0 opacity-100' : 'translate-y-2 opacity-0'
          }`}
          style={{ color: INK, fontFamily: 'var(--font-heading), sans-serif' }}
        >
          Opprett kontoen din
      ...
      {/* Fast høyde så sliden står helt i ro mens innholdet morpher. */}
      <div className="relative h-[380px]">
        {/* --- Fase A: Adressen ------------------------------------------- */}
        <div
          className={`absolute inset-x-0 top-0 ${glid} ${
            iKonto ? 'pointer-events-none -translate-y-3 scale-[0.985] opacity-0' : 'translate-y-0 scale-100 opacity-100'
          }`}
        >
        ...
        <div
          className={`absolute inset-x-0 top-0 ${glid} ${
            iKonto ? 'translate-y-0 scale-100 opacity-100' : 'pointer-events-none translate-y-3 scale-[0.985] opacity-0'
          }`}
        >
