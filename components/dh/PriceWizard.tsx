  ...
  return (
    <div>
        ...
        {values.map((v, i) => {
          ...
          return (
            <div key={i} className="flex-1 flex flex-col justify-end h-full">
              <div
                className={`w-full rounded-[4px] transition-all duration-700 ${isSpike ? 'bg-gradient-to-t from-[#9b6cc4] to-[#cf97fc]' : 'bg-white/[0.13]'}`}
      ...
      <div className="flex gap-[5px] mt-1.5">
        {MONTHS.map((m, i) => (
          <span key={i} className={`flex-1 text-center text-[9px] ${hybrid && (i === 6 || i === 7) ? 'text-[#cf97fc] font-bold' : 'text-white/30'}`}>{m}</span>
        ))}
      </div>
    </div>
  );
    ...
    <header className="shrink-0 h-[64px] border-b border-[#eee9e1] bg-[#fdfcfb]/90 backdrop-blur-xl sticky top-0 z-50">
      ...
      {!done && (
        <div className="h-[2.5px] bg-[#f0ece6]">
          <div className="h-full bg-gradient-to-r from-[#cf97fc] to-[#9b6cc4] transition-all duration-700" style={{ width: `${Math.max(progressPct, 4)}%`, transitionTimingFunction: 'cubic-bezier(0.22,1,0.36,1)' }} />
        </div>
      )}
    </header>
  );
        ...
        <div className="flex-1 flex items-center justify-center px-6 py-14">
          <div className="max-w-[600px] w-full text-center dh-fade-up" data-testid="wizard-success">
            ...
            <div className="rounded-[22px] bg-[#0a0a0a] text-white p-6 mb-8 flex items-center justify-between gap-4 text-left relative overflow-hidden">
              <div className="absolute -top-16 -right-16 w-[220px] h-[220px] rounded-full pointer-events-none" style={{ background: 'radial-gradient(circle, rgba(207,151,252,0.18) 0%, transparent 65%)' }} />
    ...
    <div className="min-h-screen lg:h-screen flex flex-col" style={{ backgroundColor: '#fdfcfb' }}>
      ...
      <div className="flex-1 lg:overflow-hidden grid lg:grid-cols-[1fr_440px] xl:grid-cols-[1fr_480px]">
        ...
        <aside className="hidden lg:flex flex-col bg-[#0a0a0a] text-white relative overflow-y-auto" data-testid="wizard-price-panel">
          <div className="absolute top-0 right-0 w-[420px] h-[420px] rounded-full pointer-events-none" style={{ background: 'radial-gradient(circle, rgba(207,151,252,0.16) 0%, transparent 62%)', transform: 'translate(30%, -30%)' }} />
          ...
          <div className="relative flex-1 flex flex-col justify-center px-10 xl:px-12 py-10 min-h-0">
            <div className="flex items-center justify-between mb-8">
              <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#cf97fc]">Din pakke — live</p>
            ...
            <div className="rounded-2xl bg-white/[0.045] border border-white/[0.08] p-5">
              <div className="flex items-center justify-between mb-4">
                ...
                {uplift > 0 && <span className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-[#cf97fc]"><Sparkles className="w-3 h-3" /> +{Math.round(uplift * 100)} % med 10+2</span>}
              </div>
          ...
          <div className="relative shrink-0 px-10 xl:px-12 py-5 border-t border-white/[0.07] flex items-center gap-x-5 gap-y-1 flex-wrap">
            {[{ i: ShieldCheck, t: 'Gratis vurdering' }, { i: Clock, t: 'Svar innen 24 timer' }, { i: Check, t: 'Ingen binding' }].map(({ i: I, t }) => (
              <span key={t} className="inline-flex items-center gap-1.5 text-[11.5px] text-white/40"><I className="w-3 h-3 text-[#cf97fc]/70" /> {t}</span>
            ))}
          </div>
        </aside>
      </div>
