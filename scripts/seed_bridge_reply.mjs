...
const body = [
  ...
  '   - Preview-test NAA: https://bli-utleier-redesign.preview.emergentagent.com/api/webhooks/lead-status',
  ...
  '',
  '5) FORWARD-RETNING (viktig avvik): Markedssiden VIDERESENDER nye leads til DERES POST /api/leads via env DIGIHOME_API_URL. Vaar config peker i dag paa preview https://bli-utleier-redesign.preview.emergentagent.com og prod https://digihome.no. Dere oppga preview tenant-hub-210 og prod digihome-draft.emergent.host. Kan dere BEKREFTE hvilken URL som skal motta videresendte leads (preview + prod), og om /api/leads krever X-API-Key (vi sender dh_live_...)? Da synker vi begge retninger korrekt.',
].join('\n');

const data = {
  marketing_webhook_preview: 'https://bli-utleier-redesign.preview.emergentagent.com/api/webhooks/lead-status',
  ...
  current_marketing_forward_targets: { preview: 'https://bli-utleier-redesign.preview.emergentagent.com', prod: 'https://digihome.no' },
};
