...
(async()=>{
  ...
  for (const fmt of ['MOBILE_FEED_STANDARD','INSTAGRAM_STANDARD','INSTAGRAM_STORY','DESKTOP_FEED_STANDARD']) {
    const pv=await g(`${id}/previews`,{ad_format:fmt});
    if (pv.error) { console.log(fmt, 'ERR', pv.error.message); continue; }
    const body=(pv.data&&pv.data[0]&&pv.data[0].body)||'';
    const w=(body.match(/width="?(\d+)/)||[])[1];
    const h=(body.match(/height="?(\d+)/)||[])[1];
    const scroll=(body.match(/scrolling="?([a-z]+)/i)||[])[1];
    console.log(fmt.padEnd(24), 'w='+w, 'h='+h, 'scroll='+scroll);
  }
