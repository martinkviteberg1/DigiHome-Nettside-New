...
export function tilFellesForm(c) {
  return {
    ...
    aktiv: c.paused !== true,
    selskap: c.selskap === 'tech' ? 'tech' : 'digihome',
...
export async function upsertFellesKostnad(db, bEo = {}) {
  ...
  // Selskap settes kun når skuffen sender det — ellers beholdes eksisterende (nye poster: Digihome AS).
  if (bEo.selskap !== undefined) { doc.selskap = bEo.selskap === 'tech' ? 'tech' : 'digihome'; doc.selskapBekreftet = true; }
  await db.collection(COSTS_COLL).updateOne(
    { id },
    { $set: doc, $setOnInsert: { id, createdAt: now, vendor: '', source: 'datarom', note: '', ...(doc.selskap ? {} : { selskap: 'digihome' }) } },
    { upsert: true },
  );
  return { ok: true, id };
}
