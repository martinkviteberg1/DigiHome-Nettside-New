...
export const DEFAULT_SEO_CONFIG = {
  ...
  aeoQuestions: [
    ...
    'Hvilke selskaper tilbyr utleieforvaltning i Bergen?',
    'Hvem kan hjelpe meg med Airbnb-utleie av leiligheten min i Bergen?',
    'Hva koster det å bruke utleiemegler i Bergen?',
    'Hvordan kan jeg leie ut leiligheten min i Bergen uten å gjøre jobben selv?',
    'Er DigiHome i Bergen til å stole på for utleieforvaltning?',
  ],
  competitors: [
...
export async function getSeoConfig(db) {
  let doc = null;
  try { doc = await db.collection('settings').findOne({ key: 'seo_config' }); } catch (_) {}
  ...
  return {
    ...
    aeoQuestions: cleanList(v.aeoQuestions, MAX_QUESTIONS, 200).length ? cleanList(v.aeoQuestions, MAX_QUESTIONS, 200) : DEFAULT_SEO_CONFIG.aeoQuestions,
...
export async function runAeoCheck(db, { questions } = {}) {
  ...
  for (const question of list) {
    ...
    if (web.available) {
      const citedUs = (web.citations || []).some((c) => (c.url || '').includes(OUR_DOMAIN)) || mentionsUs(web.content);
      doc.web = {
        available: true,
        mentions: mentionsUs(web.content),
        cited: citedUs,
  ...
  const summary = {
    ...
    webMentionRate: withWeb.length ? Math.round((withWeb.filter((r) => r.web.cited).length / withWeb.length) * 100) : null,
  };
...
export async function getSeoOverview(db) {
  ...
  const aeoHistory = aeoRunIds.slice(0, 12).map((rid) => {
    ...
    return {
      ...
      webRate: withWeb.length ? Math.round((withWeb.filter((d) => d.web.cited).length / withWeb.length) * 100) : null,
    };
  }).reverse();
