...
// Offentlig indeks — kun publiserbare boliger. Skiller «databasen svarte ikke»
// (ok: false) fra «ingen ledige» (ok: true, tom liste), slik at siden aldri
// påstår at alt er utleid når den egentlig bare mangler data.
export async function getPublishedListingsResult() {
  try {
    const rows = await loadRaw({ visible: true });
    return { ok: true, listings: rows.filter((p) => listingGate(p).publishable).map(toListingCard).sort(sortListings) };
  } catch (e) { return { ok: false, listings: [] }; }
}

export async function getPublishedListings() {
  return (await getPublishedListingsResult()).listings;
}
