    ...
    if (Array.isArray(raa) && raa.length) {
      ...
      try {
        const [rU2, rC2] = await Promise.allSettled([
          hentJson(`${base}/api/units/export?limit=500`, key, 15000),
          hentJson(`${base}/api/contracts/export?status=all&limit=500`, key, 15000),
        ]);
        if (rU2.status === 'fulfilled' && rC2.status === 'fulfilled') {
          rows = berikMedReferanser(rows, rU2.value.units || [], rC2.value.contracts || []);
        }
