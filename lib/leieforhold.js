...
function normaliserPlattformRad(r) {
  ...
  return {
    ...
    bolig_type: String(r.bolig_type || (r.unit_kind === 'room' ? 'Rom' : '') || r.property_type || '').trim(),
    ...
    group,
    advertised: !!r.advertised || /annonsert/i.test(String(r.status_label || '')),
    move_in_date: String(r.move_in_date || '').slice(0, 10),
    move_out_date: String(r.move_out_date || r.end_date || '').slice(0, 10),
    lease_id: String(r.lease_id || r.contract_id || '').trim(),
    forvaltning_id: String(r.forvaltning_id || r.agreement_id || '').trim(),
    forvaltning_status: String(r.forvaltning_status || r.agreement_status || '').trim(),
    forvaltning_start: String(r.forvaltning_start || '').slice(0, 10),
    monthly_rent: rent,
    fee_percent: fp,
    ...
    // service_level: kanonisk todeling brukt av skjerm/Excel/deck. Plattformen
    // sender feltet som `service_tier` (stor forbokstav: «Selvbetjening» /
    // «Full forvaltning» / «—» for ikke satt). Case-UFØLSOMT oppslag — tidligere
    // ble det sammenlignet mot 'selvbetjening' (liten s) og traff aldri, så ALT
    // defaultet til «Full forvaltning». '—'/tomt → «Full forvaltning» (default).
    service_level: /selvbetj|selvforvalt/i.test(String(r.service_level || r.service_tier || '')) ? 'Selvbetjening' : 'Full forvaltning',
    // Rå verdi fra plattformen bevares slik at forbrukere kan skille «—» (ikke
    // klassifisert) fra reell «Full forvaltning» ved behov.
    service_tier: String(r.service_tier || r.service_level || '').trim(),
    vat_inclusive: !!vat,
  };
}
  ...
  for (const [bid, bUnits] of enheterPerBygg) {
    ...
    const lagRad = (overstyr) => {
      ...
      return {
        ...
        bolig_type: '',
        enhet_detalj: '',
        // Kontraktreferanser til detalj-skuffen (PDF-visning m.m.)
        lease_id: '',
        move_out_date: '',
        forvaltning_id: forv?.contract_id || '',
        forvaltning_status: forv?.status || '',
        forvaltning_start: String(forv?.start_date || '').slice(0, 10),
    ...
    for (const u of bUnits) {
      ...
      if (lease) {
        ...
        rows.push(lagRad({
          ...
          lease_id: String(lease.contract_id || ''),
          ...
          move_out_date: String(lease.end_date || '').slice(0, 10),
    ...
    if (romEnheter.length) {
      ...
      romLeases.forEach((l, i) => {
        ...
        rows.push(lagRad({
          ...
          lease_id: String(l.contract_id || ''),
          move_in_date: String(l.start_date || '').slice(0, 10),
          move_out_date: String(l.end_date || '').slice(0, 10),
        }));
      });
...
function avledFraKontrakter(contracts) {
  ...
  for (const e of perEiendom.values()) {
    ...
    rows.push({
      ...
      advertised: false,
      move_in_date: moveIn,
      move_out_date: group === 'vacant' ? '' : String(beste?.end_date || '').slice(0, 10),
      lease_id: group === 'vacant' ? '' : String(beste?.contract_id || ''),
      forvaltning_id: String(forv?.contract_id || ''),
      forvaltning_status: String(forv?.status || ''),
      forvaltning_start: String(forv?.start_date || '').slice(0, 10),
      monthly_rent: rent,
      fee_percent: sats,
      fee_amount: hon.fee_amount,
...
/* Berik lease-income-rader (plattformens fasit for PENGENE) med beskrivende
   referanser fra units/contracts: boligtype, etasje/m², inn-/utflytting for
   utleide og kontrakt-ID-er til PDF-visning. Tallene røres ALDRI. Feltene er
   også bestilt direkte i lease-income/export via broen — når de kommer, vinner
   plattformens verdier (normaliserPlattformRad leser dem først). */
function berikMedReferanser(rows, units, contracts) {
  ...
  for (const c of contracts || []) {
    ...
    if (!pid) continue;
    if (String(c.type || '').includes('forvaltning')) {
      const eks = forvPerBygg.get(pid);
      const rang = (x) => (x.status === 'active' ? 0 : x.status === 'signed' ? 1 : 2);
      if (!eks || rang(c) < rang(eks)) forvPerBygg.set(pid, c);
    } else {
      if (!leierPerBygg.has(pid)) leierPerBygg.set(pid, []);
      leierPerBygg.get(pid).push(c);
    }
  ...
  return rows.map((r) => {
    const u = unitById.get(String(r.enhet_id || '').split(':')[0]);
    if (!u) return r;
    const ut = { ...r };
    const erRom = r.unit_type === 'Rom i bofellesskap';
    if (!ut.bolig_type) ut.bolig_type = erRom ? 'Rom' : typeNavn(u.unitType);
    if (!ut.enhet_detalj) {
      if (erRom) {
        const m = String(r.unit_room || '').match(/Rom\s*\d+/i);
        ut.enhet_detalj = m ? m[0] : 'Rom';
      } else {
        ut.enhet_detalj = enhetsDetalj(u);
      }
    }
    const forv = forvPerBygg.get(u.buildingId);
    if (forv && !ut.forvaltning_id) {
      ut.forvaltning_id = String(forv.contract_id || '');
      ut.forvaltning_status = String(forv.status || '');
      ut.forvaltning_start = String(forv.start_date || '').slice(0, 10);
    }
    // Avtalt sats fra forvaltningsavtalen — lease-income sender først honorar
    // når avtalen fakturerer, men satsen ER avtalt. Konvensjon (bekreftet av
    // Martin): satsen avtales INKL. mva — regnes derfor om til eks. mva her.
    // Kontrakten lagrer prosent som brøk (0.1 = 10 %); defensiv mot begge.
    if (forv && !(ut.fee_percent > 0) && !(ut.fee_amount > 0)) {
      const pRaa = Number(forv.fee_percent);
      const pct = pRaa > 0 ? Math.round((pRaa <= 1 ? pRaa * 100 : pRaa) * 10) / 10 : 0;
      const fast = String(forv.fee_model || '') === 'fixed' && Number(forv.fee_fixed) > 0 ? Number(forv.fee_fixed) : 0;
      if (pct > 0 || fast > 0) {
        const inklBelop = fast || Math.round((ut.monthly_rent || 0) * (pct / 100));
        if ((ut.group === 'leased' || ut.group === 'future') && inklBelop > 0) {
          // Leiekontrakten er signert → honoraret regnes som REELT (ikke ~)
          // og telles i honorar-KPI, sum-rad og budsjettets sikret-lag.
          if (pct > 0) ut.fee_percent = pct;
          ut.vat_inclusive = true;
          ut.fee_amount = Math.round(inklBelop / 1.25); // eks. mva
          // Plattformen har ennå ikke trukket honoraret fra netto — juster
          // kun når netto fortsatt står lik brutto leie (eller mangler).
          if (ut.net_to_owner == null || ut.net_to_owner === ut.monthly_rent) {
            ut.net_to_owner = Math.max(0, Math.round((ut.monthly_rent || 0) - inklBelop));
          }
          ut.fee_from_agreement = true; // merkes diskré i UI (tooltip)
        } else {
          // Under signering o.l. → vis som estimat (~), omregnet til eks. mva.
          if (fast > 0) ut.pending_fee_fixed = Math.round(fast / 1.25);
          else ut.pending_fee_percent = Math.round((pct / 1.25) * 10) / 10;
        }
      }
    }
    if (ut.tenant_name && !ut.lease_id) {
      const kandidater = (leierPerBygg.get(u.buildingId) || [])
        .filter((l) => normNavn(l?.tenant?.name) === normNavn(ut.tenant_name))
        .sort((a, b) => leaseRang(a) - leaseRang(b));
      const l = kandidater[0];
      if (l) {
        ut.lease_id = String(l.contract_id || '');
        if (!ut.move_in_date) ut.move_in_date = String(l.start_date || l.expected_rent_start || '').slice(0, 10);
        if (!ut.move_out_date) ut.move_out_date = String(l.end_date || '').slice(0, 10);
      }
    }
    return ut;
  });
...
async function hentFraPlattform(base, key) {
  ...
  try {
    ...
    if (Array.isArray(raa) && raa.length) {
      let rows = raa.map(normaliserPlattformRad);
      // Beskrivende berikelse (boligtype, datoer, kontrakt-IDer) — best effort.
      // DELVIS berikelse: bruk det som lyktes (units alene gir boligtype/detalj)
      // i stedet for å droppe alt når én av eksportene feiler transient.
      try {
        const [rU2, rC2] = await Promise.allSettled([
          hentJson(`${base}/api/units/export?limit=500`, key, 20000),
          hentJson(`${base}/api/contracts/export?status=all&limit=500`, key, 20000),
        ]);
        if (rU2.status === 'fulfilled') {
          rows = berikMedReferanser(
            rows,
            rU2.value.units || [],
            rC2.status === 'fulfilled' ? (rC2.value.contracts || []) : [],
          );
        }
      } catch (e) { /* berikelse er valgfri */ }
      rows = sorterRader(rows);
      // Totals: plattformens totals kan IKKE brukes rått for honorar/netto —
      // den undertrykker avtalt honorar uten aktiv eier-avtale (f.eks.
      // Knøsesmauet: signert leiekontrakt + avtalt 10 % inkl. mva), mens
      // radene våre beriker med avtalt sats (fee_from_agreement). Derfor:
      // våre beregnede aggregater vinner (konsistent med radene/sum-raden),
      // plattform-egne felt uten lokal motpart (deposit) beholdes.
      const totals = (j.totals && typeof j.totals === 'object' && Number.isFinite(Number(j.totals.actual_rent)))
        ? { ...j.totals, ...beregnTotals(rows) }
        : beregnTotals(rows);
...
export async function hentLeieforhold(target, opts = {}) {
  ...
  try {
    ...
    // KILDEVERN: lease-income er fasit (per-avtale mva, avtalt honorar —
    // f.eks. Knøsesmauet 10 % inkl. mva → 8 % eks. / 2 400 kr). Faller
    // primærkilden transient ut (timeout o.l.) mens units/contracts svarer,
    // ville degraderte tall (global eks. mva-antakelse → 3 000 kr) både
    // vises OG overskrive cachen. Server heller siste vellykkede
    // lease-income-henting så lenge den er under et døgn gammel.
    if (resultat.source !== 'lease-income'
        && cachet?.data?.ok && cachet.data.source === 'lease-income'
        && Date.now() - new Date(cachet.fetchedAt).getTime() < 24 * 60 * 60 * 1000) {
      return {
        ...cachet.data, env, cached: true, stale: true, fetchedAt: cachet.fetchedAt,
        warning: 'Primærkilden svarte ikke i denne hentingen — viser siste vellykkede tall.',
      };
    }
    // Transient berikelsessvikt (units-/contracts-export nede): tett hull i
    // BESKRIVENDE felt fra forrige henting — pengetall/totals røres ALDRI.
    try {
      const gamle = new Map();
      for (const g of (cachet?.data?.rows || [])) if (g.enhet_id) gamle.set(g.enhet_id, g);
      if (gamle.size && Array.isArray(resultat.rows)) {
        const FELT = ['bolig_type', 'enhet_detalj', 'forvaltning_id', 'forvaltning_status', 'forvaltning_start', 'lease_id', 'move_in_date', 'move_out_date'];
        resultat.rows = resultat.rows.map((r) => {
          const g = gamle.get(r.enhet_id);
          if (!g) return r;
          let ut = r;
          for (const felt of FELT) {
            if (!ut[felt] && g[felt]) { if (ut === r) ut = { ...r }; ut[felt] = g[felt]; }
          }
          return ut;
        });
      }
    } catch (e) { /* tilbakefyll er valgfritt */ }
