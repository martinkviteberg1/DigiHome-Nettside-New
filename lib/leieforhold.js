...
function berikMedReferanser(rows, units, contracts) {
  ...
  return rows.map((r) => {
    ...
    // Avtalt sats fra forvaltningsavtalen — også når den venter signering.
    // lease-income sender først honorar når avtalen fakturerer, men satsen ER
    // avtalt og gir et langt bedre estimat enn porteføljesnittet. Kontrakten
    // lagrer prosent som brøk (0.1 = 10 %); vær defensiv mot begge formater.
    if (forv && !(ut.fee_percent > 0) && !(ut.fee_amount > 0)) {
      const pRaa = Number(forv.fee_percent);
      if (String(forv.fee_model || 'percent') === 'fixed' && Number(forv.fee_fixed) > 0) {
        ut.pending_fee_fixed = Math.round(Number(forv.fee_fixed));
      } else if (pRaa > 0) {
        ut.pending_fee_percent = Math.round((pRaa <= 1 ? pRaa * 100 : pRaa) * 10) / 10;
      }
    }
