...
function byggFraEnheterOgKontrakter({ units, contracts, properties }, idag) {
  ...
  for (const [bid, bUnits] of enheterPerBygg) {
    ...
    for (const u of bUnits) {
      ...
      const lease = kravPerEnhet.get(u.unitId) || null;
      ...
      if (lease) {
        ...
        rows.push(lagRad({
          ...
          move_in_date: (gruppe === 'future' || gruppe === 'signing') ? String(lease.start_date || lease.expected_rent_start || '').slice(0, 10) : '',
    ...
    if (romEnheter.length) {
      ...
      romLeases.forEach((l, i) => {
        ...
        rows.push(lagRad({
          ...
          move_in_date: (gruppe === 'future' || gruppe === 'signing') ? String(l.start_date || '').slice(0, 10) : '',
        }));
      });
