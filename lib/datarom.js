...
export async function beregnOversikt(db) {
  ...
  // Kostnader = ev. rest-kostnader på enhetene + felleskostnadene fra
  // Enhetsøkonomi (Leieforhold → Økonomi-modus: lønn, markedsføring m.m.).
  let fellesMnd = 0;
  try {
    const fellesDok = await db.collection('enhetsokonomi').find({ type: 'felles' }, { projection: { _id: 0, belop: 1 } }).toArray();
    fellesMnd = fellesDok.reduce((a, d) => a + n(d.belop), 0);
  } catch (e) { /* tom er OK */ }
