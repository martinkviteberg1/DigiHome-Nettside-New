...
export async function syncFromPlatform(db, { target, secret, since, until, channelHint, maxPages = 20, pageLimit = 200 } = {}) {
  ...
  const ownerOnly = ownerRows.filter((r) => mapLeadType(r.lead_type || r.type || r.kundetype) === 'huseier');
  const contactRows = ownerRows.filter((r) => mapLeadType(r.lead_type || r.type || r.kundetype) === 'kontakt');
  const allRows = [...ownerOnly, ...contactRows, ...tenantRows];
  ...
  const platformOwners = ownerOnly.map((r, i) => normalizeRecord(r, i, { importedAt: new Date().toISOString(), source: 'api' })).filter(Boolean);
