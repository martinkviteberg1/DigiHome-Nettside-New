...
export async function syncFromPlatform(db, { target, secret, since, until, channelHint, maxPages = 20, pageLimit = 200 } = {}) {
  ...
  for (const p of paths) {
    try {
      ...
      const res = await fetch(u.toString(), {
        headers: {
          ...(secret ? { 'X-API-Key': secret, 'X-Webhook-Secret': secret, Authorization: `Bearer ${secret}` } : {}),
        },
        signal: ctrl.signal,
  ...
  while (cursor && page < maxPages) {
    ...
    try {
      ...
      const res = await fetch(u.toString(), {
        headers: { ...(secret ? { 'X-API-Key': secret, 'X-Webhook-Secret': secret, Authorization: `Bearer ${secret}` } : {}) },
      });
      if (!res.ok) break;
      const j = await res.json().catch(() => ({}));
      const rows = Array.isArray(j) ? j : (j.leads || j.data || j.items || []);
