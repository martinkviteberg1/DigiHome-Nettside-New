...
// Ringeklikk har sin egen konverteringshandling i Google Ads
// («DigiHome – Ringeklikk (nettsted)», sekundær — måles, styrer ikke bud).
...
// Ringeklikk. Telefon er historisk vår sterkeste kanal (3 av 5 vunne kunder
// siste 90 dager), men var HELT usporet før august 2026 — betalte kanaler ble
// derfor systematisk underkreditert.
//
// Førsteparts-hendelsen (track('call_click')) sendes fra CallTracking-komponenten
// og krever IKKE samtykke. Denne funksjonen håndterer bare de eksterne taggene,
// som alle self-gater på om taggen faktisk er lastet.
export function trackCall({ placement = 'ukjent', href = '' } = {}) {
  const gtag = safeGtag();
  try {
    if (gtag && GA4_ID) gtag('event', 'call_click', { placement, link_url: href });
  } catch (e) {}
  try {
    if (gtag && ADS_ID && ADS_CALL_LABEL) {
      gtag('event', 'conversion', { send_to: `${ADS_ID}/${ADS_CALL_LABEL}` });
    }
  } catch (e) {}
  // Meta: Contact er standardhendelsen for «tok kontakt utenom skjema».
  try {
    if (typeof window !== 'undefined' && typeof window.fbq === 'function') {
      window.fbq('track', 'Contact', { content_name: placement, content_category: 'call' });
    }
  } catch (e) {}
}
...
// Forventet verdi av ETT huseierlead, i kroner.
//
// Utledning (produksjonstall, aug. 2026): snitt kundeverdi 26 350 kr ×
// historisk lead→kunde-rate 7,7 % ≈ 2 029 kr → vi runder ned til 2 000 kr.
//
// Vi bruker ETT tall for alle nivåer med vilje. Med 7 vunne kunder totalt blir
// gjennomsnitt pr. produktnivå ren støy, og et oppdiktet «full forvaltning er
// verdt 1,4×» ville sett presist ut uten å være det. Når vi har nok vunne
// kunder pr. nivå, splitter vi — ikke før.
export const LEAD_VALUE_NOK = Number(process.env.NEXT_PUBLIC_LEAD_VALUE_NOK || 2000) || 0;

export async function trackLead({ value = 0, currency = 'NOK', formId, leadId, source, email, phone } = {}) {
  const gtag = safeGtag();
  if (!gtag || !GA4_ID) return;

  // Ingen kaller sendte verdi tidligere → alle konverteringer havnet på 0 kr,
  // og Google måtte overstyre med «bruk alltid standardverdi». Nå sender vi den
  // faktiske forventningsverdien. Er den 0, utelates feltet HELT slik at
  // Google faller tilbake på sin egen standardverdi i stedet for å bokføre 0.
  const resolved = Number(value) > 0 ? Number(value) : LEAD_VALUE_NOK;
  const money = resolved > 0 ? { value: resolved, currency } : {};
  ...
  try {
    if (typeof window !== 'undefined' && typeof window.fbq === 'function') {
      ...
      if (resolved > 0) { cd.value = resolved; cd.currency = currency; }
