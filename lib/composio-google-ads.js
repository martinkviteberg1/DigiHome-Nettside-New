...
// Annonse-tabell via Composio (fallback når native REST-API ikke er
// konfigurert i miljøet): samme radform som lib/google-ads-native.js →
// runAdsWithMetrics, slik at /admin/ads/table kan bruke dem om hverandre.
export async function runAdsWithMetricsViaComposio({ since, until, customerId } = {}) {
  const co = await client();
  const status = await getConnectionStatus();
  if (!status.connected) throw new Error('Google Ads er ikke tilkoblet (Composio)');
  const cid = (customerId || defaultCustomerId() || '').replace(/[^0-9]/g, '');
  const from = new Date(since || Date.now() - 30 * 86400000).toISOString().slice(0, 10);
  const to = new Date(until || Date.now()).toISOString().slice(0, 10);
  const run = async (query) => {
    ...
    if (cid) args.customer_id = cid;
    const res = await co.tools.execute(GAQL_TOOL, {
      userId: USER_ID, connectedAccountId: status.connectedAccountId,
      arguments: args, ...toolkitVersionParams(),
    });
    if (res && res.successful === false) {
      const msg = (res.error && (res.error.message || res.error)) || 'Google Ads-spørringen feilet';
      throw new Error(typeof msg === 'string' ? msg : JSON.stringify(msg));
  ...
  const baseQ = "SELECT campaign.id, campaign.name, campaign.status, ad_group.id, ad_group.name, ad_group_ad.ad.id, ad_group_ad.ad.type, ad_group_ad.status, ad_group_ad.policy_summary.approval_status, ad_group_ad.ad.final_urls, ad_group_ad.ad.responsive_search_ad.headlines, ad_group_ad.ad.responsive_search_ad.descriptions FROM ad_group_ad WHERE ad_group_ad.status != 'REMOVED'";
  const metricsQ = `SELECT ad_group_ad.ad.id, metrics.cost_micros, metrics.impressions, metrics.clicks, metrics.conversions, metrics.conversions_value FROM ad_group_ad WHERE segments.date BETWEEN '${from}' AND '${to}' AND ad_group_ad.status != 'REMOVED'`;
  const [baseRows, metricRows] = await Promise.all([run(baseQ), run(metricsQ).catch(() => [])]);
  ...
  const ads = baseRows.map((r) => {
    ...
    return {
      ...
      cost: r2(mm.cost), impressions: mm.impressions, clicks: mm.clicks,
      ctr: mm.impressions > 0 ? r2((mm.clicks / mm.impressions) * 100) : 0,
      cpc: mm.clicks > 0 ? r2(mm.cost / mm.clicks) : 0,
      conversions: r2(mm.conversions), convValue: r2(mm.convValue),
      cpa: mm.conversions > 0 ? r2(mm.cost / mm.conversions) : null,
      roas: mm.cost > 0 ? r2(mm.convValue / mm.cost) : null,
    };
  }).filter((a) => a.id);
...
// Hent faktiske annonser (responsive search ads) med kreativ-innhold (read-only).
export async function runAdsCreatives({ customerId } = {}) {
  const co = await client();
  const status = await getConnectionStatus();
  if (!status.connected) throw new Error('Google Ads er ikke tilkoblet ennå.');
  const cid = (customerId || defaultCustomerId() || '').replace(/[^0-9]/g, '');
  ...
  if (cid) args.customer_id = cid;
  const res = await co.tools.execute(GAQL_TOOL, {
    userId: USER_ID, connectedAccountId: status.connectedAccountId, arguments: args, ...toolkitVersionParams(),
  });
  if (res && res.successful === false) {
    const msg = (res.error && (res.error.message || res.error)) || 'Google Ads-spørringen feilet';
    throw new Error(typeof msg === 'string' ? msg : JSON.stringify(msg));
  ...
  const ads = rows.map((r) => {
    ...
    const ps = aga.policySummary || aga.policy_summary || {};
    return {
      id: String(ad.id || ''),
      ...
      adGroup: ag.name || '',
      status: aga.status || '',
      approval: ps.approvalStatus || ps.approval_status || '',
      finalUrl: (ad.finalUrls && ad.finalUrls[0]) || (ad.final_urls && ad.final_urls[0]) || '',
...
// Cachet kreativ-henting (TTL + timeout + stale-serve), aldri henge.
export async function getCachedCreatives(db, { force = false } = {}) {
  ...
  task.catch(() => {});
  if (existing && !force) {
    return { ads: existing.ads, fetchedAt: existing.fetchedAt, cached: true, stale: true };
  }
  try {
