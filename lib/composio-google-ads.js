...
// Nær-sanntid: hent rapport for en periode, med kort server-side cache (TTL).
// Returnerer { report, fetchedAt, cached, stale, error? }.
export async function getCachedReport(db, period = 'last_30d', { force = false } = {}) {
  ...
  try {
    ...
    await coll.updateOne({ key }, { $set: { key, period: p, report, fetchedAt } }, { upsert: true });
    return { report, fetchedAt, cached: false, stale: false };
  } catch (e) {
    // ved feil: returner sist kjente (stale) hvis det finnes, ellers kast videre
    if (existing) return { report: existing.report, fetchedAt: existing.fetchedAt, cached: true, stale: true, error: e.message };
    throw e;
  }
}
