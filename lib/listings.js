...
// soverom og prisintervall. Aldri eier, aldri leietaker, aldri eksakt
// kontraktsleie — et beløp skal ikke kunne knyttes til en navngitt leieavtale.
// ---------------------------------------------------------------------------
...
export const GATE = {
  ...
  mangler_pris: { label: 'Mangler pris', fix: 'Legg inn leiepris i utleiemodulen — eller sett en prisantydning under «Rediger boligdata».', where: 'begge' },
...
export function listingGate(p) {
  ...
  // Pris hentet direkte fra FINN er fortsatt sperret. En redaksjonell
  // prisantydning ('redaksjonell') er derimot DigiHomes egen opplysning og
  // slipper gjennom — den merkes som prisantydning hele veien ut.
...
export function toListingCard(p) {
  ...
  return {
    ...
    // estimatbeløp, fordi plattformen selv har flagget leien som estimat
    // (ledige boliger har prisantydning — utleide har faktisk kontraktsleie),
    // eller fordi en DigiHome-forvalter har satt antydningen selv.
    rentIndicative: p?.rentBandSource === 'plattform-estimat' || p?.rentIsEstimate === true || p?.rentBandSource === 'redaksjonell',
...
// Beløp skal ikke brytes over to linjer. Det løses i CSS (whitespace-nowrap) og
// IKKE med hardt mellomrom i dataene: prisintervallet sammenlignes som ren
// tekst i probes, tester og nyhetsbrev, og et usynlig \u00a0 der ville skapt
// falske avvik.
...
export function toListingDetail(p) {
  return {
    ...
    description: txt(p?.description) || null,
