...
export function toListingCard(p) {
  ...
  // rom leies ut viser vi ingen pris før forvalteren har satt en redaksjonell
  // prisantydning (som per definisjon gjelder det som annonseres).
  const editorialPrice = p?.rentBandSource === 'redaksjonell';
  const band = txt(p?.monthlyRentBand) || null;
  return {
    id: String(p?.id || ''),
    slug: listingSlug(p),
    ...
    rentBand: scope === 'rom' && !editorialPrice ? null : band,
    ...
    rentIndicative: p?.rentBandSource === 'plattform-estimat' || p?.rentIsEstimate === true || editorialPrice,
