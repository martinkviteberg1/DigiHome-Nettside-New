...
// Manuell, kuratert stylingflyt: hver jobb = ett originalbilde + parametere
// (modus/intensitet/instruks) → kandidatbilde → review (godkjenn/forkast).
export const JOBBER_COLL = 'salgsradar_stylingjobber';
export const RADAR_STATUSER = ['ny', 'analysert', 'kontaktet', 'dialog', 'tilbud', 'vunnet', 'tapt', 'ikke_relevant'];
...
// FINN CDN har varianter (240w…1600w + «default» som bare er ~430px!).
// Normaliser ALLTID til 1600w — full kvalitet for visning og AI-styling.
export function normaliserFinnBildeUrl(u) {
  return String(u || '').replace(/\/dynamic\/(?:default|\d+w)\/item\//, '/dynamic/1600w/item/');
}
...
// ── Annonsør: proff-profil (megler/utleiefirma) eller privat ────────────────
// FINN legger firmaprofil + kontaktpersoner i en egen JSON-blob
// (<script data-company-profile-data>). Private annonser mangler blobben helt
// — FINN skjuler da all kontaktinfo bak innlogging (og den skal vi ikke omgå).
export function parseAnnonsor(html) {
  let profil = null;
  const m = html.match(/<script[^>]*data-company-profile-data[^>]*>\s*([\s\S]*?)\s*<\/script>/);
  if (m) { try { profil = JSON.parse(m[1]); } catch (e) { profil = null; } }
  const cp = profil?.companyProfile || null;
  const orgNavn = String(cp?.orgName || '').trim().slice(0, 80);
  const raaKontakter = Array.isArray(cp?.contacts) ? cp.contacts : Array.isArray(profil?.contacts) ? profil.contacts : [];
  const kontakter = raaKontakter.slice(0, 4).map((k) => ({
    navn: String(k?.name || '').trim().slice(0, 60),
    tittel: String(k?.title || '').trim().slice(0, 60),
    telefon: String(k?.phone?.[0]?.phoneFormatted || k?.phonesWithLabels?.[0]?.phone || '').replace(/[^\d+]/g, '').slice(0, 15),
    epost: String(k?.email || '').trim().slice(0, 120),
  })).filter((k) => k.navn || k.telefon || k.epost);
  // PRIVATE annonser: FINN viser aldri navnet anonymt, men telefonen ligger i
  // sidens serialiserte data («"mobile","92072453"») når utleier har valgt å
  // oppgi den. Fang den — det er gull for selgerne (ring i stedet for FINN-melding).
  if (!orgNavn && !kontakter.length) {
    const tlfPriv = (html.match(/\\?"mobile\\?",\s*\\?"([\d ]{8,12})/) || [])[1];
    const renTlf = String(tlfPriv || '').replace(/\D/g, '').slice(0, 15);
    if (renTlf.length >= 8) kontakter.push({ navn: '', tittel: '', telefon: renTlf, epost: '' });
  }
  const s = orgNavn.toLowerCase();
  const type = !orgNavn ? 'privat'
    : s.includes('utleiemegleren') ? 'utleiemegleren'
    : s.includes('husleie') ? 'husleie'
    : 'megler';
  return {
    v: 2, // parserversjon: 2 = inkluderer privat-telefon fra serialiserte data
    type, // 'privat' | 'utleiemegleren' | 'husleie' | 'megler' ('ukjent' settes ved feilet berikelse)
    orgNavn,
    orgId: cp?.orgId || null,
    hjemmeside: String(cp?.homepageUrl || '').slice(0, 200),
    logo: String(cp?.logo || '').slice(0, 300),
    kontakter,
...
export function parseFinnAnnonse(html, finnkode) {
  ...
  )).map(normaliserFinnBildeUrl).slice(0, 20);
  ...
  // Annonsør (proff/privat) + kontaktperson: dagens FINN-format legger dette i
  // company-profile-blobben — de gamle contactName/mobile-nøklene er utdaterte
  // men beholdes som fallback for eldre sidevarianter.
  const annonsor = parseAnnonsor(html);
  const forsteKontakt = annonsor.kontakter[0] || null;
  return {
    finnkode,
    ...
    kontaktTlf: (kontaktTlf || forsteKontakt?.telefon || '').replace(/\s/g, ''),
    kontaktNavn: (kontaktNavn || forsteKontakt?.navn || '').replace(/\s+/g, ' ').trim().slice(0, 60),
    kontaktEpost: forsteKontakt?.epost || '',
    kontaktTittel: forsteKontakt?.tittel || '',
    annonsor,
    bilder,
    beskrivelse,
  };
...
export function validerIngestAnnonse(body = {}) {
  ...
  for (const b of bilderInn.slice(0, 20)) {
    ...
    if (u.protocol === 'https:' && u.hostname === 'images.finncdn.no') bilder.push(normaliserFinnBildeUrl(u.toString()).slice(0, 300));
  }
...
// ── GPT som tekstforfatter ──────────────────────────────────────────────────
// Arbeidsdeling: Gemini (syn) scorer bildene og graver frem fakta — GPT
// (nyeste, via egen OpenAI-nøkkel med Emergent-fallback i chatLLM) SKRIVER de
// kundevendte tekstene: FINN-melding, tilbudstekster, annonseutkast og
// salgsvinkel. Feiler hele GPT-kjeden beholdes Geminis tekster — analysen
// ryker aldri av dette. Modell kan overstyres med RADAR_TEKST_MODELL i env
// eller per funksjon fra admin-UI (feature: salgsradar_tekst).
const TEKST_MODELL = () => process.env.RADAR_TEKST_MODELL || 'gpt-5.6';

export async function skrivSalgstekster(fakta, grunnlag = {}) {
  const rensT = (s, maks) => String(s || '').replace(/\s+/g, ' ').trim().slice(0, maks);
  const rensAvsnittT = (s, maks) => String(s || '').replace(/[ \t]+/g, ' ').replace(/\n{3,}/g, '\n\n').trim().slice(0, maks);
  const bildeLinjer = (grunnlag.bildeVurdering || []).slice(0, 5)
    .map((bv, i) => `  ${i + 1}. ${bv.rom} (${bv.score}/10): ${bv.funn || '—'}`).join('\n');
  const prompt = `Du er senior tekstforfatter hos DigiHome, en profesjonell utleieforvalter i Bergen. En kollega (bildeanalytiker) har vurdert en FINN-leieannonse vi vil vinne som forvaltningsoppdrag. Skriv de kundevendte tekstene — bedre, varmere og mer presist enn utkast, men nøkternt og uten å dikte.

ANNONSEDATA:
${fakta}

BILDEANALYTIKERENS GRUNNLAG (internt — ikke siter direkte):
${(grunnlag.funn || []).length ? `Funn: ${(grunnlag.funn || []).join(' · ')}` : 'Funn: ingen'}
${grunnlag.salgsvinkel ? `Vinkel-utkast: ${grunnlag.salgsvinkel}` : ''}
${bildeLinjer ? `Rom på bildene:\n${bildeLinjer}` : ''}
Stylingpotensial: ${grunnlag.stylingPotensial || 'ukjent'} · Bildene i hovedsak umøblerte: ${grunnlag.bilderUmoblert ? 'ja' : 'nei'}

Returner KUN gyldig JSON uten markdown-gjerder, nøyaktig denne strukturen:
{"finnMelding":"…","heroIntro":"…","potensialTekst":"…","salgsvinkel":"…","annonseUtkast":{"tittel":"…","beskrivelse":"…","hoydepunkter":["…"],"fasiliteter":["…"]}}

Regler:
- finnMelding: kort, høflig melding (maks 500 tegn) til annonsøren via FINN-meldinger. Presenter DigiHome i én setning, nevn noe konkret og positivt fra akkurat denne annonsen, og vis til lenken {LENKE} der de ser vårt konkrete tilbud. ALDRI tall, priser, prosenter eller garantier. Dette er første kontakt med en huseier som leier ut selv — ikke en melding til leietakere.
- heroIntro: 1-2 nøkterne setninger til toppen av tilbudssiden om akkurat denne boligen/beliggenheten. ALDRI tall, priser, arealer eller antall rom — de vises allerede på siden.
- potensialTekst: 2-3 setninger, positivt innrammet: hvilket potensial vi ser i presentasjonen og utleien. ALDRI kritikk av eier/bilder, ALDRI tall.
- salgsvinkel: 1-2 setninger til selgeren vår (internt) — hvilken vinkel vinner denne leaden.
- annonseUtkast — annonsen slik en toppmegler ville publisert den. KUN fakta med dekning i grunnlaget/bildene — aldri dikt opp fasiliteter, mål eller egenskaper:
  - tittel: selgende, maks 60 tegn, led med sterkeste salgspunkt (beliggenhet, lys, utsikt, standard) — ikke bare adresse/boligtype.
  - beskrivelse: 2-4 korte avsnitt (skill med \\n\\n). Første avsnitt selger følelsen og beliggenheten, så innholdet rom for rom, avslutt praktisk. Konkret, varm, profesjonell — ikke svulstig.
  - hoydepunkter: 3-5 punkter à maks 8 ord.
  - fasiliteter: 4-10 stikkord med dekning i grunnlaget (f.eks. «Balkong», «Møblert», «Oppvaskmaskin»).
- Norsk bokmål. Nøkternt og troverdig — ingen superlativer, utropstegn eller floskler.`;

  const svar = await chatLLM({
    messages: [{ role: 'user', content: prompt }],
    model: TEKST_MODELL(),
    maxTokens: 3000,
    feature: 'salgsradar_tekst',
  });
  // Robust JSON-uttrekk (modeller pakker av og til JSON i prosa/gjerder)
  const t = String(svar || '').replace(/```(?:json)?/gi, '').trim();
  const forste = t.indexOf('{'); const siste = t.lastIndexOf('}');
  let parsed = null;
  for (const k of [t, forste >= 0 && siste > forste ? t.slice(forste, siste + 1) : '']) {
    if (!k) continue;
    try { const o = JSON.parse(k); if (o && typeof o === 'object') { parsed = o; break; } } catch (e) { /* prøv neste */ }
  }
  if (!parsed) return null;
  const au = parsed.annonseUtkast || {};
  return {
    modell: TEKST_MODELL(),
    tekster: {
      finnMelding: rensT(parsed.finnMelding, 700),
      heroIntro: rensT(parsed.heroIntro, 300),
      potensialTekst: rensT(parsed.potensialTekst, 500),
      salgsvinkel: rensT(parsed.salgsvinkel, 400),
      annonseUtkast: {
        tittel: rensT(au.tittel, 80),
        beskrivelse: rensAvsnittT(au.beskrivelse, 2400),
        hoydepunkter: (Array.isArray(au.hoydepunkter) ? au.hoydepunkter : []).map((h) => rensT(h, 60)).filter(Boolean).slice(0, 5),
        fasiliteter: (Array.isArray(au.fasiliteter) ? au.fasiliteter : []).map((f) => rensT(f, 30)).filter(Boolean).slice(0, 12),
      },
    },
  };
}
...
- Norsk bokmål, nøktern tone, ingen superlativer eller utropstegn.`;
  ...
  // 4) GPT (nyeste) skriver de kundevendte tekstene — Gemini-tekstene over er
  //    fallback hvis hele GPT-kjeden feiler. Felt-for-felt: tomme GPT-felt
  //    beholder Gemini-versjonen.
  try {
    const gpt = await skrivSalgstekster(fakta, {
      funn: aiDoc.funn,
      salgsvinkel: aiDoc.salgsvinkel,
      bildeVurdering: aiDoc.bildeVurdering,
      stylingPotensial: aiDoc.stylingPotensial,
      bilderUmoblert: aiDoc.bilderUmoblert,
    });
    if (gpt && gpt.tekster) {
      const tk = gpt.tekster;
      if (tk.finnMelding) aiDoc.finnMelding = tk.finnMelding;
      if (tk.heroIntro) aiDoc.tilbudTekst.heroIntro = tk.heroIntro;
      if (tk.potensialTekst) aiDoc.tilbudTekst.potensialTekst = tk.potensialTekst;
      if (tk.salgsvinkel) aiDoc.salgsvinkel = tk.salgsvinkel;
      const au2 = tk.annonseUtkast || {};
      if (au2.tittel) aiDoc.annonseUtkast.tittel = au2.tittel;
      if (au2.beskrivelse) aiDoc.annonseUtkast.beskrivelse = au2.beskrivelse;
      if (au2.hoydepunkter?.length) aiDoc.annonseUtkast.hoydepunkter = au2.hoydepunkter;
      if (au2.fasiliteter?.length) aiDoc.annonseUtkast.fasiliteter = au2.fasiliteter;
      aiDoc.tekstModell = gpt.modell;
    }
  } catch (e) {
    console.error('[salgsradar] GPT-tekstforfatter feilet — beholder Gemini-tekstene:', e.message);
  }
...
// Kill-switch: RADAR_AUTO=0 i miljøet. Dagstak er AV som standard — kan
// aktiveres ved behov via RADAR_AUTO_ANALYSE_TAK / RADAR_AUTO_BILDE_TAK i env.
const AUTO_ANALYSE_TAK = () => Number(process.env.RADAR_AUTO_ANALYSE_TAK) || Infinity; // leads/dag
const AUTO_BILDE_TAK = () => Number(process.env.RADAR_AUTO_BILDE_TAK) || Infinity;     // bilder/dag
...
export async function kjorAutoPipeline(db, leadId) {
  ...
  try {
    ...
    await db.collection(LEADS_COLL).updateOne({ id }, { $set: { auto: { status: 'analyserer', startet: new Date().toISOString(), oppdatert: new Date().toISOString() } } });

    // KUN AI-analyse. Bildestyling er en MANUELL, kuratert beslutning i UI-et
    // (styling-jobber med review) — vi styler ALDRI bilder automatisk lenger.
    // Styling er et salgsverktøy for leads DigiHome faktisk vil forfølge.
    ...
    await settAuto({ status: 'ferdig', ferdigAt: new Date().toISOString(), oppdatert: new Date().toISOString() });
    return { ok: true, analysert: true };
  } catch (e) {
...
export async function opprettLead(db, annonse, analyse, kildeUrl, { kilde = 'manuell' } = {}) {
  ...
  if (eksisterende) {
    ...
    if (!annonse.kontaktEpost && eksisterende.kontaktEpost) delete oppdatering.$set.kontaktEpost;
    if (!annonse.kontaktTittel && eksisterende.kontaktTittel) delete oppdatering.$set.kontaktTittel;
    // Annonsør: en re-parse som ikke finner proff-profilen skal aldri
    // degradere en kjent megler-lead til «privat» (FINN kan endre markup)
    if (oppdatering.$set.annonsor && eksisterende.annonsor
      && oppdatering.$set.annonsor.type === 'privat' && !(oppdatering.$set.annonsor.kontakter || []).length
      && !['privat', 'ukjent'].includes(eksisterende.annonsor.type)) {
      delete oppdatering.$set.annonsor;
    }
...
export async function listLeads(db) {
  ...
  return docs.map((d) => ({
    ...
    // Foreslått stylingmodus (kun et UI-forslag — brukeren velger alltid selv):
    // umøblert utleie + tomme bilder → lysløft, ellers varsom optimalisering.
    foreslattStil: velgAutoStil(d) === 'nordisk' ? 'optimal' : velgAutoStil(d),
  }));
}
...
export async function oppdaterLead(db, body = {}) {
  ...
  // Manuell kontaktinfo: FINN viser private utleieres navn kun for innloggede
  // brukere — selgeren kan lime inn navnet selv fra annonsen. (Megler-
  // kontakter fylles automatisk fra firmaprofilen.)
  if (body.kontaktNavn !== undefined) set.kontaktNavn = String(body.kontaktNavn).replace(/\s+/g, ' ').trim().slice(0, 60);
  if (body.kontaktTlf !== undefined) set.kontaktTlf = String(body.kontaktTlf).replace(/[^\d+]/g, '').slice(0, 15);
  // Aktivitetslogg — manuelle innslag (kontakt/notat/oppgave) fra Aktivitet-fanen.
  // Kun ekte, brukerskapte hendelser lagres — ingenting fabrikkeres.
  let push = null;
  if (body.loggInnslag && typeof body.loggInnslag === 'object') {
    const loggType = String(body.loggInnslag.type || '');
    if (!['kontakt', 'notat', 'oppgave'].includes(loggType)) return { ok: false, error: 'Ugyldig loggtype', status: 400 };
    const loggTekst = String(body.loggInnslag.tekst || '').trim().slice(0, 1000);
    if (!loggTekst) return { ok: false, error: 'Loggteksten kan ikke være tom', status: 400 };
    push = { logg: { id: crypto.randomUUID(), type: loggType, tekst: loggTekst, at: new Date().toISOString(), ...(loggType === 'oppgave' ? { gjort: false } : {}) } };
  }
  if (body.loggOppdater && typeof body.loggOppdater === 'object' && body.loggOppdater.id) {
    await db.collection(LEADS_COLL).updateOne(
      { id, 'logg.id': String(body.loggOppdater.id) },
      { $set: { 'logg.$.gjort': Boolean(body.loggOppdater.gjort) } },
    );
  }
  if (body.loggSlett) {
    await db.collection(LEADS_COLL).updateOne({ id }, { $pull: { logg: { id: String(body.loggSlett) } } });
  }
  ...
  // Redigering av annonseutkastet (tittel/tekst/høydepunkter/fasiliteter)
  if (body.annonseUtkast && typeof body.annonseUtkast === 'object') {
    if (!cur.ai) return { ok: false, error: 'Kjør AI-analyse først', status: 400 };
    const au = body.annonseUtkast;
    if (au.tittel !== undefined) set['ai.annonseUtkast.tittel'] = String(au.tittel).replace(/\s+/g, ' ').trim().slice(0, 80);
    if (au.beskrivelse !== undefined) set['ai.annonseUtkast.beskrivelse'] = String(au.beskrivelse).replace(/[ \t]+/g, ' ').replace(/\n{3,}/g, '\n\n').trim().slice(0, 2400);
    if (Array.isArray(au.hoydepunkter)) set['ai.annonseUtkast.hoydepunkter'] = au.hoydepunkter.map((h) => String(h).replace(/\s+/g, ' ').trim().slice(0, 60)).filter(Boolean).slice(0, 5);
    if (Array.isArray(au.fasiliteter)) set['ai.annonseUtkast.fasiliteter'] = au.fasiliteter.map((f) => String(f).replace(/\s+/g, ' ').trim().slice(0, 30)).filter(Boolean).slice(0, 12);
    set['ai.annonseUtkast.redigert'] = true;
  }
  ...
  await db.collection(LEADS_COLL).updateOne({ id }, { $set: set, ...(push ? { $push: push } : {}) });
...
export async function slettLeads(db, ids = []) {
  ...
  await db.collection(JOBBER_COLL).deleteMany({ leadId: { $in: rene } });
...
export async function slettLead(db, id) {
  ...
  await db.collection(JOBBER_COLL).deleteMany({ leadId: String(id) });
  return { ok: true };
}
...
NØDBREMS FOR TOMME ROM: Hvis rommet i bildet er tomt eller umøblert (ingen møbler synlige), skal du IKKE legge til møbler, tekstiler, planter eller gjenstander av noe slag — gjør da KUN fotoløftet (naturlig dagslys, jevn eksponering, korrekt hvitbalanse, fjerning av rusk). Rommet skal forbli tomt.`,
...
// Merking (ærlighetsprinsippet): optimal/lysloft er «AI-forbedret foto»,
// staging-modusene er «AI-møblert · illustrasjon». Brukes i admin + tilbud.
export const STIL_MERKING = {
  optimal: 'AI-forbedret foto',
  lysloft: 'AI-forbedret foto',
  nordisk: 'AI-møblert · illustrasjon',
  moderne: 'AI-møblert · illustrasjon',
  varm: 'AI-møblert · illustrasjon',
};

// Komponerer full prompt fra modus + valgfri intensitet/fritekstinstruks.
// Instruksen er underordnet de absolutte reglene og kan aldri overstyre
// nødbremsen mot møblering av tomme rom.
export function byggStylingPrompt(stil = 'optimal', { intensitet = 'full', instruks = '' } = {}) {
  let p = STILER[stil] || STILER.optimal;
  if (intensitet === 'varsom') {
    p += '\nVARSOM MODUS: Gjør kun de mest nødvendige og minst synlige forbedringene. Behold bildets karakter og alle eksisterende gjenstander — prioriter lys, eksponering, hvitbalanse og opprydding av smårot fremfor å bytte eller tilføre noe. Resultatet skal ligge svært tett på originalen.';
  }
  const ren = String(instruks || '').replace(/\s+/g, ' ').trim().slice(0, 500);
  if (ren) {
    p += `\nSPESIFIKK INSTRUKS FRA STYLISTEN (underordnet de absolutte reglene over — reglene gjelder alltid): ${ren}`;
  }
  return p;
}
...
async function lastNedFinnBilde(kildeUrl) {
  let u;
  try { u = new URL(normaliserFinnBildeUrl(kildeUrl)); } catch (e) { throw new Error('Ugyldig bilde-URL'); }
...
export async function stilBilde(kildeUrl, stil = 'optimal', tilpasning = {}) {
  const key = process.env.EMERGENT_LLM_KEY;
  if (!key || !key.startsWith('sk-emergent-')) throw new Error('EMERGENT_LLM_KEY mangler i miljøet');
  const stilTekst = byggStylingPrompt(stil, tilpasning);
    ...
    try {
      const r = await fetch('https://integrations.emergentagent.com/llm/v1/chat/completions', {
        ...
        body: JSON.stringify({
          ...
          // 2K-output fra Nano Banana Pro (probet 15.02.2026: 1792x2388 —
          // høyere enn FINN-originalen). Fallback-modellen støtter ikke
          // image_config, så den sendes kun til gemini-3-modeller.
          ...(modell.includes('gemini-3') ? { image_config: { image_size: '2K' } } : {}),
        }),
      });
      const body = await r.json().catch(() => ({}));
      if (r.ok) {
...
export async function lagreStyletBilde(db, leadId, kildeUrl, stil, dataUrl) {
  ...
  // ERSTATT-SEMANTIKK: maks ÉN aktiv stylet versjon per originalbilde.
  // Duplikater (samme kildeUrl to ganger) ødela original↔AI-parringen i
  // før/etter-visningene — fjern gammel versjon før ny lagres.
  const lead = await db.collection(LEADS_COLL).findOne({ id: String(leadId) }, { projection: { stylet: 1 } });
  const gamle = (lead?.stylet || []).filter((s) => s.kildeUrl === kildeUrl).map((s) => s.id);
  if (gamle.length) {
    await db.collection(BILDER_COLL).deleteMany({ id: { $in: gamle } });
    await db.collection(LEADS_COLL).updateOne({ id: String(leadId) }, { $pull: { stylet: { kildeUrl } } });
  }
  await db.collection(BILDER_COLL).insertOne({
    id, leadId: String(leadId), kildeUrl, stil, dataUrl: medMerke, vannmerket: medMerke !== dataUrl, godkjent: true, createdAt: new Date().toISOString(),
  });
  await db.collection(LEADS_COLL).updateOne(
    { id: String(leadId) },
...
// ── Manuell stylingflyt: jobber + review ─────────────────────────────────────
// En stylingjobb = ett originalbilde + parametere → kandidatbilde → review.
// Kandidater havner ALDRI i lead.stylet (og dermed aldri i tilbud/annonse)
// før de er eksplisitt godkjent. Parringen original↔resultat er garantert
// via normalisert kildeUrl lagret på jobben OG på bildedokumentet.
const GYLDIGE_INTENSITETER = ['varsom', 'full'];

export async function opprettStylingJobber(db, leadId, bilder = [], { av = '' } = {}) {
  const id = String(leadId || '');
  const lead = await db.collection(LEADS_COLL).findOne({ id });
  if (!lead) return { ok: false, error: 'Lead ikke funnet', status: 404 };
  const leadBilder = new Set((lead.bilder || []).map((b) => normaliserFinnBildeUrl(b)));
  const inn = (Array.isArray(bilder) ? bilder : []).slice(0, 10);
  if (!inn.length) return { ok: false, error: 'Ingen bilder valgt', status: 400 };
  // Ikke start dobbelt: aktive jobber (venter/kjører) per kildeUrl blokkerer ny
  const aktive = await db.collection(JOBBER_COLL)
    .find({ leadId: id, status: { $in: ['venter', 'kjorer'] } }, { projection: { kildeUrl: 1 } })
    .toArray();
  const aktiveKilder = new Set(aktive.map((j) => j.kildeUrl));
  const now = new Date().toISOString();
  const jobber = [];
  const hoppet = [];
  const sett = new Set();
  for (const b of inn) {
    const kilde = normaliserFinnBildeUrl(String(b?.kildeUrl || ''));
    if (!kilde || !leadBilder.has(kilde)) { hoppet.push({ kildeUrl: kilde, grunn: 'Bildet tilhører ikke denne annonsen' }); continue; }
    if (sett.has(kilde)) continue; // duplikat i samme forespørsel
    if (aktiveKilder.has(kilde)) { hoppet.push({ kildeUrl: kilde, grunn: 'En jobb kjører allerede for dette bildet' }); continue; }
    sett.add(kilde);
    jobber.push({
      id: uuidv4(),
      leadId: id,
      kildeUrl: kilde,
      stil: STILER[b?.stil] ? b.stil : 'optimal',
      intensitet: GYLDIGE_INTENSITETER.includes(b?.intensitet) ? b.intensitet : 'full',
      instruks: String(b?.instruks || '').replace(/\s+/g, ' ').trim().slice(0, 500),
      status: 'venter',
      feil: null,
      resultatBildeId: null,
      review: null, // 'venter' | 'godkjent' | 'forkastet' (settes når ferdig)
      av: String(av || '').slice(0, 120),
      createdAt: now,
      updatedAt: now,
    });
  }
  if (!jobber.length) return { ok: false, error: hoppet[0]?.grunn || 'Ingen gyldige bilder', status: 400 };
  await db.collection(JOBBER_COLL).insertMany(jobber.map((j) => ({ ...j })));
  return { ok: true, jobber, hoppet };
}

// Bakgrunnskjøring (fire-and-forget fra API-ruten). Sekvensiell per kall —
// atomisk claiming (venter→kjorer) gjør det trygt selv om to kjørere starter.
export async function kjorStylingJobber(db, leadId) {
  const id = String(leadId || '');
  for (let i = 0; i < 12; i += 1) { // hard øvre grense per kjøring
    const r = await db.collection(JOBBER_COLL).findOneAndUpdate(
      { leadId: id, status: 'venter' },
      { $set: { status: 'kjorer', startetAt: new Date().toISOString(), updatedAt: new Date().toISOString() } },
      { sort: { createdAt: 1 }, returnDocument: 'after' },
    );
    const jobb = r?.value ?? (r?.id ? r : null);
    if (!jobb || !jobb.id) break;
    try {
      const dataUrl = await stilBilde(jobb.kildeUrl, jobb.stil, { intensitet: jobb.intensitet, instruks: jobb.instruks });
      const medMerke = await leggPaaVannmerke(dataUrl);
      const bildeId = uuidv4();
      await db.collection(BILDER_COLL).insertOne({
        id: bildeId, leadId: id, kildeUrl: jobb.kildeUrl, stil: jobb.stil,
        dataUrl: medMerke, vannmerket: medMerke !== dataUrl,
        godkjent: false, jobbId: jobb.id, createdAt: new Date().toISOString(),
      });
      await db.collection(JOBBER_COLL).updateOne(
        { id: jobb.id },
        { $set: { status: 'ferdig', review: 'venter', resultatBildeId: bildeId, ferdigAt: new Date().toISOString(), updatedAt: new Date().toISOString() } },
      );
    } catch (e) {
      await db.collection(JOBBER_COLL).updateOne(
        { id: jobb.id },
        { $set: { status: 'feilet', feil: String(e?.message || e).slice(0, 200), updatedAt: new Date().toISOString() } },
      );
    }
  }
}

export async function listStylingJobber(db, leadId) {
  return db.collection(JOBBER_COLL)
    .find({ leadId: String(leadId || '') }, { projection: { _id: 0 } })
    .sort({ createdAt: -1 })
    .limit(60)
    .toArray();
}

// Review: godkjenn (→ lead.stylet, erstatter ev. gammel versjon av samme
// original), forkast (kandidat slettes) eller provIgjen (ny jobb, gamle
// kandidaten forkastes — parametere kan justeres).
export async function reviewStylingJobb(db, { jobbId, handling, stil, intensitet, instruks, av = '' } = {}) {
  const jobb = await db.collection(JOBBER_COLL).findOne({ id: String(jobbId || '') });
  if (!jobb) return { ok: false, error: 'Jobb ikke funnet', status: 404 };
  const now = new Date().toISOString();

  if (handling === 'godkjenn') {
    if (jobb.status !== 'ferdig' || !jobb.resultatBildeId) return { ok: false, error: 'Jobben har ikke et ferdig bilde', status: 400 };
    if (jobb.review === 'godkjent') return { ok: true, allerede: true };
    const bilde = await db.collection(BILDER_COLL).findOne({ id: jobb.resultatBildeId }, { projection: { id: 1 } });
    if (!bilde) return { ok: false, error: 'Kandidatbildet finnes ikke lenger', status: 410 };
    // Erstatt ev. eksisterende godkjent versjon av samme original
    const lead = await db.collection(LEADS_COLL).findOne({ id: jobb.leadId }, { projection: { stylet: 1 } });
    const gamle = (lead?.stylet || []).filter((s) => s.kildeUrl === jobb.kildeUrl).map((s) => s.id);
    if (gamle.length) {
      await db.collection(BILDER_COLL).deleteMany({ id: { $in: gamle } });
      await db.collection(LEADS_COLL).updateOne({ id: jobb.leadId }, { $pull: { stylet: { kildeUrl: jobb.kildeUrl } } });
    }
    await db.collection(BILDER_COLL).updateOne({ id: jobb.resultatBildeId }, { $set: { godkjent: true } });
    await db.collection(LEADS_COLL).updateOne(
      { id: jobb.leadId },
      { $push: { stylet: { id: jobb.resultatBildeId, kildeUrl: jobb.kildeUrl, stil: jobb.stil } }, $set: { updatedAt: now } },
    );
    await db.collection(JOBBER_COLL).updateOne({ id: jobb.id }, { $set: { review: 'godkjent', reviewAt: now, updatedAt: now } });
    return { ok: true, bildeId: jobb.resultatBildeId };
  }

  if (handling === 'forkast') {
    if (jobb.review === 'godkjent') return { ok: false, error: 'Jobben er allerede godkjent — fjern bildet fra galleriet i stedet', status: 400 };
    if (jobb.resultatBildeId) await db.collection(BILDER_COLL).deleteOne({ id: jobb.resultatBildeId, godkjent: { $ne: true } });
    await db.collection(JOBBER_COLL).updateOne({ id: jobb.id }, { $set: { review: 'forkastet', resultatBildeId: null, reviewAt: now, updatedAt: now } });
    return { ok: true };
  }

  if (handling === 'provIgjen') {
    if (['venter', 'kjorer'].includes(jobb.status)) return { ok: false, error: 'Jobben kjører fortsatt', status: 409 };
    // Forkast gammel kandidat (om den finnes og ikke er godkjent)
    if (jobb.resultatBildeId && jobb.review !== 'godkjent') {
      await db.collection(BILDER_COLL).deleteOne({ id: jobb.resultatBildeId, godkjent: { $ne: true } });
      await db.collection(JOBBER_COLL).updateOne({ id: jobb.id }, { $set: { review: 'forkastet', resultatBildeId: null, reviewAt: now, updatedAt: now } });
    }
    const ny = await opprettStylingJobber(db, jobb.leadId, [{
      kildeUrl: jobb.kildeUrl,
      stil: STILER[stil] ? stil : jobb.stil,
      intensitet: GYLDIGE_INTENSITETER.includes(intensitet) ? intensitet : jobb.intensitet,
      instruks: instruks !== undefined ? instruks : jobb.instruks,
    }], { av });
    if (!ny.ok) return ny;
    return { ok: true, nyJobb: ny.jobber[0], startKjorer: true };
  }

  return { ok: false, error: 'Ugyldig handling', status: 400 };
}

// Fjern et godkjent stylet bilde fra leadens galleri (kuratering)
export async function fjernStyletBilde(db, leadId, bildeId) {
  const id = String(leadId || '');
  const bid = String(bildeId || '');
  const lead = await db.collection(LEADS_COLL).findOne({ id }, { projection: { stylet: 1 } });
  if (!lead) return { ok: false, error: 'Lead ikke funnet', status: 404 };
  if (!(lead.stylet || []).some((s) => s.id === bid)) return { ok: false, error: 'Bildet finnes ikke på leaden', status: 404 };
  await db.collection(LEADS_COLL).updateOne({ id }, { $pull: { stylet: { id: bid } }, $set: { updatedAt: new Date().toISOString() } });
  await db.collection(BILDER_COLL).deleteOne({ id: bid });
  return { ok: true };
}
...
export async function hentTilbud(db, slug, { sporAapning = false } = {}) {
  ...
  // Personlig avsender — tildelt selger (offentlig: kun navn/tittel/kontakt/foto,
  // aldri provisjon eller interne felter). Pool-leads → null → generisk avsender.
  let selger = null;
  const tildeltTb = lead.salg?.tildeltTil;
  if (tildeltTb?.id) {
    try {
      const u = await db.collection('admin_users').findOne(
        { id: tildeltTb.id },
        { projection: { _id: 0, name: 1, email: 1, avatar: 1, tittel: 1, telefon: 1 } },
      );
      if (u) {
        selger = {
          navn: u.name || tildeltTb.navn || '',
          tittel: u.tittel || 'Utleierådgiver',
          epost: u.email || '',
          telefon: u.telefon || '',
          avatar: u.avatar || '',
        };
      }
    } catch (e) { /* avsender er best effort — siden faller tilbake til generisk */ }
  }
  ...
  return {
    ...
    // Vurderingsdato + aggregert grunnlag (kun tall — aldri interne rådata).
    vurdert: lead.ai?.analysertAt || lead.updatedAt || lead.createdAt || null,
    grunnlag: (() => {
      const g = lead.analyse?.grunnlag || {};
      if (!(g.antallILeide > 0)) return null;
      return {
        antallILeide: g.antallILeide,
        antallISone: g.antallISone || 0,
        sone: g.sone || null,
        // Snitt oppnådd leie i samme postsone — KUN aggregert tall, og kun når
        // det bygger på minst 2 leieforhold (aldri enkeltleier ut offentlig).
        snittSone: (g.antallISone >= 2 && Number(g.snittSone) > 0) ? Math.round(g.snittSone) : null,
      };
    })(),
...
export async function registrerTilbudKontakt(db, slug, body = {}) {
  ...
  // Navn er valgfritt — tilbudet er allerede knyttet til en identifisert bolig
  const navn = String(body.navn || '').trim().slice(0, 80) || (tlf ? 'Huseier' : '');
  if (!navn || (!tlf && !melding)) return { ok: false, error: 'Telefon eller melding kreves', status: 400 };
  await db.collection(LEADS_COLL).updateOne(
    { id: lead.id },
    {
      $push: { kontaktLogg: { navn, telefon: tlf, melding, at: new Date().toISOString() } },
      $set: { status: ['vunnet', 'tapt', 'ikke_relevant'].includes(lead.status) ? lead.status : 'dialog', updatedAt: new Date().toISOString() },
    },
  );
  return { ok: true, leadId: lead.id, adresse: lead.adresse || lead.tittel, tildeltTilId: lead.salg?.tildeltTil?.id || null };
}

/* ═══════════════════════════════════════════════════════════════════════════
   ANNONSØR-BERIKELSE — eldre leads mangler annonsor-feltet (parseren fikk
   det først nå). Henter annonsen på nytt fra FINN og fyller annonsør +
   kontaktperson. Kjøres lazy fra admin-UI og etter agent-ingest; feilede
   oppslag merkes 'ukjent' så vi ikke hamrer FINN i evighet.
   ═══════════════════════════════════════════════════════════════════════ */
export async function berikAnnonsorer(db, { maks = 15 } = {}) {
  const kandidater = await db.collection(LEADS_COLL)
    .find(
      {
        annonseAktiv: { $ne: false },
        finnkode: { $exists: true, $nin: ['', null] },
        // Mangler annonsor helt, ELLER beriket med eldre parser (v1 fanget
        // ikke privat-telefonen fra serialiserte data) — re-berikes én gang
        $or: [{ annonsor: { $exists: false } }, { 'annonsor.v': { $exists: false } }],
      },
      { projection: { id: 1, finnkode: 1, kontaktNavn: 1, kontaktTlf: 1, kontaktEpost: 1, kontaktTittel: 1 } },
    )
    .limit(Math.min(30, Math.max(1, Number(maks) || 15)))
    .toArray();
  let oppdatert = 0; let feilet = 0;
  for (const l of kandidater) {
    try {
      const html = await hentFinnHtml(l.finnkode);
      const annonsor = parseAnnonsor(html);
      const fk = annonsor.kontakter[0] || null;
      const set = { annonsor, updatedAt: new Date().toISOString() };
      if (!l.kontaktNavn && fk?.navn) set.kontaktNavn = fk.navn;
      if (!l.kontaktTlf && fk?.telefon) set.kontaktTlf = fk.telefon;
      if (!l.kontaktEpost && fk?.epost) set.kontaktEpost = fk.epost;
      if (!l.kontaktTittel && fk?.tittel) set.kontaktTittel = fk.tittel;
      await db.collection(LEADS_COLL).updateOne({ id: l.id }, { $set: set });
      oppdatert += 1;
    } catch (e) {
      // Annonsen kan være tatt av FINN eller nettverket feilet — merk som
      // ukjent (UI viser da ingen badge) i stedet for å prøve på nytt evig
      await db.collection(LEADS_COLL).updateOne(
        { id: l.id },
        { $set: { annonsor: { v: 2, type: 'ukjent', orgNavn: '', orgId: null, hjemmeside: '', logo: '', kontakter: [], hentetAt: new Date().toISOString() } } },
      ).catch(() => {});
      feilet += 1;
    }
  }
  return { ok: true, sjekket: kandidater.length, oppdatert, feilet };
}

/* ═══════════════════════════════════════════════════════════════════════════
   SALGSPIPELINE — tildeling, terminale årsaker og provisjon
   Modell: hybrid pool. Nye leads ligger utildelt («poolen»); selgere kan TA
   en ledig lead selv, leder (master-admin) kan tildele og omfordele fritt.
   Provisjonssats settes per selger (admin_users.provisjonssats). Ved VUNNET
   låses provisjonen til selgeren som var tildelt i vinnerøyeblikket.
   Alt salgsrelatert bor under lead.salg — rører ikke radarens øvrige felter.
   ═══════════════════════════════════════════════════════════════════════ */

export const RADAR_TERMINALE = ['vunnet', 'tapt', 'ikke_relevant'];
export const RADAR_ARSAKER = {
  tapt: ['Valgte konkurrent', 'For lav leie', 'Ville ikke bruke forvalter', 'Timing — utsatt', 'Annet'],
  ikke_relevant: ['Feil segment', 'Duplikat', 'Allerede utleid', 'Utenfor område', 'Annet'],
};

const salgsInnslag = (type, tekst, aktor) => ({
  id: uuidv4(),
  type, // 'tildeling' | 'status' | 'vunnet' | 'oppfolging'
  tekst: String(tekst).slice(0, 300),
  av: aktor?.navn || 'System',
  at: new Date().toISOString(),
});

// Selgere = admin-brukere med salgsradar-modulen (pluss provisjonssats)
export async function listSelgere(db) {
  const docs = await db.collection('admin_users')
    .find({ moduler: 'salgsradar' }, { projection: { _id: 0, id: 1, name: 1, email: 1, provisjonssats: 1, avatar: 1 } })
    .sort({ name: 1 })
    .toArray();
  return docs.map((u) => ({ id: u.id, navn: u.name || u.email || 'Ukjent', epost: u.email || '', provisjonssats: Math.max(0, Number(u.provisjonssats) || 0), avatar: u.avatar || '' }));
}

export async function settProvisjonssats(db, { brukerId, sats }) {
  const s = Number(sats);
  if (!brukerId || !Number.isFinite(s) || s < 0 || s > 100) return { ok: false, error: 'Ugyldig sats (0–100)', status: 400 };
  const r = await db.collection('admin_users').updateOne({ id: String(brukerId) }, { $set: { provisjonssats: Math.round(s * 10) / 10 } });
  if (!r.matchedCount) return { ok: false, error: 'Bruker ikke funnet', status: 404 };
  return { ok: true };
}

// Tildeling — leder fritt; selger kan kun ta ledige leads eller frasi seg egne
export async function tildelLead(db, { id, brukerId, aktor }) {
  const cur = await db.collection(LEADS_COLL).findOne({ id: String(id || '') });
  if (!cur) return { ok: false, error: 'Ikke funnet', status: 404 };
  const naavaerende = cur.salg?.tildeltTil?.id || null;
  let mottaker = null;
  if (brukerId) {
    const u = await db.collection('admin_users').findOne({ id: String(brukerId) }, { projection: { id: 1, name: 1, email: 1 } });
    if (!u) return { ok: false, error: 'Selger ikke funnet', status: 404 };
    mottaker = { id: u.id, navn: u.name || u.email || 'Ukjent' };
  }
  if (!aktor.erLeder) {
    const tarLedig = mottaker && mottaker.id === aktor.id && !naavaerende;
    const frasierEgen = !mottaker && naavaerende === aktor.id;
    if (!tarLedig && !frasierEgen) return { ok: false, error: 'Kun leder kan omfordele leads', status: 403 };
  }
  const tekst = mottaker
    ? (mottaker.id === aktor.id ? `${aktor.navn} tok leaden` : `Tildelt ${mottaker.navn} av ${aktor.navn}`)
    : `Lagt tilbake i poolen av ${aktor.navn}`;
  await db.collection(LEADS_COLL).updateOne({ id: cur.id }, {
    $set: { 'salg.tildeltTil': mottaker, 'salg.tildeltAv': aktor.navn, 'salg.tildeltAt': new Date().toISOString(), updatedAt: new Date().toISOString() },
    $push: {
      'salg.logg': { $each: [salgsInnslag('tildeling', tekst, aktor)], $slice: -100 },
      'salg.historikk': { $each: [{ fra: naavaerende, til: mottaker?.id || null, av: aktor.navn, at: new Date().toISOString() }], $slice: -50 },
    },
  });
  const lead = await db.collection(LEADS_COLL).findOne({ id: cur.id }, { projection: { _id: 0 } });
  return { ok: true, lead };
}

// Statusflytt med pipeline-regler: årsak kreves på tapt/ikke relevant,
// vunnet krever verdigrunnlag, kun leder kan gjenåpne terminale leads.
export async function settSalgsstatus(db, { id, status, arsak, vunnet, aktor }) {
  const cur = await db.collection(LEADS_COLL).findOne({ id: String(id || '') });
  if (!cur) return { ok: false, error: 'Ikke funnet', status: 404 };
  if (!RADAR_STATUSER.includes(status)) return { ok: false, error: 'Ugyldig status', status: 400 };
  if (RADAR_TERMINALE.includes(cur.status) && !aktor.erLeder) return { ok: false, error: 'Kun leder kan gjenåpne en avsluttet lead', status: 403 };
  const eierId = cur.salg?.tildeltTil?.id || null;
  if (!aktor.erLeder && eierId && eierId !== aktor.id) return { ok: false, error: 'Leaden er tildelt en annen selger', status: 403 };

  const set = { status, updatedAt: new Date().toISOString() };
  const loggInnslag = [];
  // Selger som jobber en utildelt lead tar den implisitt (hindrer eierløse dialoger)
  if (!eierId && !aktor.erLeder) {
    set['salg.tildeltTil'] = { id: aktor.id, navn: aktor.navn };
    set['salg.tildeltAv'] = aktor.navn;
    set['salg.tildeltAt'] = new Date().toISOString();
    loggInnslag.push(salgsInnslag('tildeling', `${aktor.navn} tok leaden`, aktor));
  }

  if (status === 'tapt' || status === 'ikke_relevant') {
    const valg = String(arsak?.valg || '').slice(0, 60);
    if (!RADAR_ARSAKER[status].includes(valg)) return { ok: false, error: 'Velg en årsak', status: 400 };
    set['salg.arsak'] = { status, valg, tekst: String(arsak?.tekst || '').slice(0, 500), at: new Date().toISOString(), av: aktor.navn };
    set['salg.vunnet'] = null;
    loggInnslag.push(salgsInnslag('status', `Markert ${status === 'tapt' ? 'tapt' : 'ikke relevant'}: ${valg}`, aktor));
  } else if (status === 'vunnet') {
    // Provisjonen låses til tildelt selger (eller aktøren selv om leaden var ledig)
    const selger = (vunnet?.selgerId
      ? await db.collection('admin_users').findOne({ id: String(vunnet.selgerId) }, { projection: { id: 1, name: 1, email: 1, provisjonssats: 1 } })
      : null)
      || (set['salg.tildeltTil'] || cur.salg?.tildeltTil
        ? await db.collection('admin_users').findOne({ id: (set['salg.tildeltTil'] || cur.salg.tildeltTil).id }, { projection: { id: 1, name: 1, email: 1, provisjonssats: 1 } })
        : null);
    if (!selger) return { ok: false, error: 'Tildel en selger før leaden markeres vunnet', status: 400 };
    const mndLeie = Math.max(0, Math.round(Number(vunnet?.mndLeie) || Number(cur.analyse?.anbefaltLeie) || Number(cur.pris) || 0));
    const honorarPct = Math.min(15, Math.max(4, Number(vunnet?.honorarPct) || Number(cur.analyse?.honorarPct) || 8));
    const grunnlag = Math.max(0, Math.round(Number(vunnet?.grunnlag) || (mndLeie * 12 * honorarPct) / 100));
    const sats = Math.max(0, Math.min(100, vunnet?.provisjonssats != null && aktor.erLeder ? Number(vunnet.provisjonssats) : Number(selger.provisjonssats) || 0));
    if (!grunnlag) return { ok: false, error: 'Angi avtaleverdi (grunnlag)', status: 400 };
    set['salg.vunnet'] = {
      mndLeie, honorarPct, grunnlag,
      provisjonssats: Math.round(sats * 10) / 10,
      provisjon: Math.round((grunnlag * sats) / 100),
      selger: { id: selger.id, navn: selger.name || selger.email || 'Ukjent' },
      at: new Date().toISOString(),
      av: aktor.navn,
    };
    set['salg.arsak'] = null;
    if (!cur.salg?.tildeltTil && !set['salg.tildeltTil']) {
      set['salg.tildeltTil'] = { id: selger.id, navn: selger.name || selger.email || 'Ukjent' };
    }
    loggInnslag.push(salgsInnslag('vunnet', `Vunnet — grunnlag ${grunnlag.toLocaleString('nb-NO')} kr, provisjon ${Math.round((grunnlag * sats) / 100).toLocaleString('nb-NO')} kr til ${selger.name || selger.email}`, aktor));
  } else {
    // Gjenåpning fra terminal rydder utfallet — historikken beholdes i loggen
    if (RADAR_TERMINALE.includes(cur.status)) {
      set['salg.vunnet'] = null;
      set['salg.arsak'] = null;
      loggInnslag.push(salgsInnslag('status', `Gjenåpnet fra ${cur.status} → ${status}`, aktor));
    } else {
      loggInnslag.push(salgsInnslag('status', `Status: ${cur.status || 'ny'} → ${status}`, aktor));
    }
  }
  await db.collection(LEADS_COLL).updateOne({ id: cur.id }, {
    $set: set,
    $push: { 'salg.logg': { $each: loggInnslag, $slice: -100 } },
  });
  const lead = await db.collection(LEADS_COLL).findOne({ id: cur.id }, { projection: { _id: 0 } });
  return { ok: true, lead };
}

// Neste oppfølging — kobles på varselflyten senere; lagres alltid på leaden
export async function settOppfolging(db, { id, dato, aktor }) {
  const cur = await db.collection(LEADS_COLL).findOne({ id: String(id || '') });
  if (!cur) return { ok: false, error: 'Ikke funnet', status: 404 };
  const iso = dato ? new Date(dato).toISOString() : null;
  if (dato && Number.isNaN(new Date(dato).getTime())) return { ok: false, error: 'Ugyldig dato', status: 400 };
  await db.collection(LEADS_COLL).updateOne({ id: cur.id }, {
    $set: { 'salg.oppfolging': iso, updatedAt: new Date().toISOString() },
    $push: { 'salg.logg': { $each: [salgsInnslag('oppfolging', iso ? `Oppfølging satt til ${new Date(iso).toLocaleDateString('nb-NO')}` : 'Oppfølging fjernet', aktor)], $slice: -100 } },
  });
  const lead = await db.collection(LEADS_COLL).findOne({ id: cur.id }, { projection: { _id: 0 } });
  return { ok: true, lead };
}

// Selgerrapport — vunnet/provisjon per selger (valgfritt periodefilter på vunnet-dato)
export async function selgerRapport(db, { fra, til } = {}) {
  const [selgere, leads] = await Promise.all([
    listSelgere(db),
    db.collection(LEADS_COLL).find({}, { projection: { _id: 0, id: 1, status: 1, salg: 1 } }).toArray(),
  ]);
  const fraT = fra ? new Date(fra).getTime() : null;
  const tilT = til ? new Date(til).getTime() : null;
  const iPeriode = (iso) => {
    const t = new Date(iso || 0).getTime();
    return (!fraT || t >= fraT) && (!tilT || t <= tilT);
  };
  const rader = new Map(selgere.map((s) => [s.id, { ...s, aktive: 0, vunnet: 0, grunnlag: 0, provisjon: 0, tapt: 0, ikkeRelevant: 0 }]));
  let pool = 0;
  for (const l of leads) {
    const eier = l.salg?.tildeltTil?.id || null;
    if (!eier && !RADAR_TERMINALE.includes(l.status)) pool += 1;
    if (!eier) continue;
    if (!rader.has(eier)) rader.set(eier, { id: eier, navn: l.salg.tildeltTil.navn, epost: '', provisjonssats: 0, aktive: 0, vunnet: 0, grunnlag: 0, provisjon: 0, tapt: 0, ikkeRelevant: 0 });
    const rad = rader.get(eier);
    if (!RADAR_TERMINALE.includes(l.status)) rad.aktive += 1;
    else if (l.status === 'tapt') rad.tapt += 1;
    else if (l.status === 'ikke_relevant') rad.ikkeRelevant += 1;
    else if (l.status === 'vunnet' && l.salg?.vunnet && iPeriode(l.salg.vunnet.at)) {
      rad.vunnet += 1;
      rad.grunnlag += Number(l.salg.vunnet.grunnlag) || 0;
      rad.provisjon += Number(l.salg.vunnet.provisjon) || 0;
    }
  }
  const ut = Array.from(rader.values()).map((r) => ({
    ...r,
    winRate: r.vunnet + r.tapt > 0 ? Math.round((r.vunnet / (r.vunnet + r.tapt)) * 100) : null,
  })).sort((a, b) => b.provisjon - a.provisjon || b.vunnet - a.vunnet || a.navn.localeCompare(b.navn, 'nb'));
  return { ok: true, selgere: ut, pool };
}
