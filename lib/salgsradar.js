...
export async function listSelgere(db) {
  const docs = await db.collection('admin_users')
    .find({ moduler: 'salgsradar' }, { projection: { _id: 0, id: 1, name: 1, email: 1, provisjonssats: 1 } })
    .sort({ name: 1 })
    .toArray();
  return docs.map((u) => ({ id: u.id, navn: u.name || u.email || 'Ukjent', epost: u.email || '', provisjonssats: Math.max(0, Number(u.provisjonssats) || 0) }));
}
