...
{"bilder":{"lys":0-10,"skarphet":0-10,"ryddighet":0-10,"styling":0-10},"bilderUmoblert":true|false,"tekst":0-10 eller null,"funn":["…"],"stylingPotensial":"lav"|"middels"|"høy","salgsvinkel":"…","finnMelding":"…","heroIntro":"…","potensialTekst":"…"}
...
export function velgAutoStil(lead) {
  const mob = String(lead.mobler || '').toLowerCase();
  const umoblertUtleie = mob.includes('umøblert') || mob.includes('umoblert');
  const tommeBilder = lead.ai?.bilderUmoblert === true;
  if (tommeBilder && umoblertUtleie) return 'lysloft';
  if (tommeBilder && mob && !umoblertUtleie) return 'nordisk';
  return 'optimal';
}
...
export function retryKandidater(lead) {
  ...
  const eksplisitt = (Array.isArray(lead?.auto?.feiletBilder) ? lead.auto.feiletBilder : []).filter((b) => b && !styletKilder.has(b));
  ...
  for (let forsok = 1; forsok <= 3; forsok += 1) {
    ...
    try {
      ...
      if (r.ok) {
        ...
        // 4xx utenom 429 er permanente (auth/validering) — ikke vits å prøve igjen
        if (r.status < 500 && r.status !== 429) permanent = sisteFeil;
      }
    } catch (e) {
