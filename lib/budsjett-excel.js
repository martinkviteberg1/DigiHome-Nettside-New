...
export async function lagBudsjettExcelRullerende({ budsjettA, budsjettB, fraAar, fraMnd }) {
  const wb = nyArbeidsbok();
  ...
  tittelRader(ws, `DigiHome — Budsjett neste 12 måneder (${MND[fraMnd]} ${String(fraAar).slice(2)} → ${MND[vindu[11].m]} ${String(vindu[11].y).slice(2)})`,
    `Generert ${new Date().toLocaleDateString('nb-NO', { day: 'numeric', month: 'long', year: 'numeric' })} · alle tall i kr eks. mva · rullerende vindu over årsgrensen — summer og resultat er levende formler`);
