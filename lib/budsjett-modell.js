...
export function beregnInvestorModell({ antallMnd, fakta = {}, drivere = {}, startYm = null }) {
  ...
  const kSystem = [], kBemanning = [], kMfFast = [], kProvisjon = [], kPartner = [], kAdmin = [], kAndre = [];
  ...
  for (let t = 0; t < N; t++) {
    ...
    kProvisjon.push(cp); kPartner.push(cpa); kAdmin.push(cad); kAndre.push(can);
    const ks = cs + cb + cmf + cp + cpa + cad + can;
    kostSum.push(ks);
  ...
  return {
    ...
    aar,
    kost: { system: kSystem, bemanning: kBemanning, mfFast: kMfFast, provisjon: kProvisjon, partner: kPartner, admin: kAdmin, andre: kAndre },
...
export function beregnTech({ antallMnd, drivere = {}, fakta = {} }) {
  ...
  const kUtv = [], kHost = [], kVar = [], kSup = [], kAnn = [], kSalg = [], kPart = [], kMf = [], kAndre = [], kostSum = [], resultat = [], akkumulert = [];
  const pa = d.partner;
  const partnerAktiv = (t) => pa.paa && t + 1 >= pa.fraMnd;
  ...
  for (let t = 0; t < N; t++) {
    ...
    const cmf = r0(k.markedsforingFast * kostF);
    ...
    kUtv.push(cu); kHost.push(ch); kVar.push(cv); kSup.push(cs); kAnn.push(annT); kSalg.push(csalg); kPart.push(cpa); kMf.push(cmf); kAndre.push(ca);
    const ks = cu + ch + cv + cs + annT + csalg + cpa + cmf + ca;
    kostSum.push(ks);
  ...
  for (let t = 0; t < N; t++) {
    ...
    sm.push(kAnn[t] + kSalg[t] + kPart[t] + kMf[t]); rd.push(kUtv[t]); ga.push(kAndre[t]);
  ...
  const saas = {
    ...
    sammendrag: {
      ...
      sm: { annonser: r0(sum(kAnn)), partner: r0(sum(kPart)), markedsforing: r0(sum(kMf)), salg: r0(sum(kSalg)), andelPct: sumInn > 0 ? Math.round((sumSm / sumInn) * 100) : null },
  ...
  return {
    ...
    kost: { utvikling: kUtv, hosting: kHost, variabel: kVar, support: kSup, annonser: kAnn, salg: kSalg, partner: kPart, markedsforing: kMf, andre: kAndre },
