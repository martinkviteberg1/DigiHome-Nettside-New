...
// Kontaktskjema-/henvendelsesrader ligger i samme Mongo-kolleksjon som huseiere,
// men skal aldri forurense huseier-KPI, annonse-ROAS eller salgshastighet.
const OWNER_LEAD_QUERY = { lead_type: { $nin: ['kontakt', 'contact', 'henvendelse', 'inquiry', 'leietaker'] } };
...
// Kun kjente typer lagres som seg selv — ukjente kollapser til 'pageview'.
// VIKTIG: mangler en type her, blir hendelsen STILLE omdøpt til 'pageview'.
// Det blåser opp sidevisninger og sletter signalet. Listen ble utvidet i aug. 2026
// etter at 14 typer som faktisk fyres fra klienten manglet.
// Legger du til en ny track('…') i frontend, MÅ typen inn her.
const ALLOWED_TYPES = new Set([
  // Basis
  'pageview', 'web_vital', 'outbound', 'cta_click', 'address_search',
  // Skjemaflyt (huseier/leietaker/LP)
  'form_start', 'form_step', 'form_error', 'form_input_mode',
  'lead_step', 'lead_submit', 'contact_submit',
  // Valg i onboarding
  'owner_kind_choice', 'tier_entry_choice', 'tier_select', 'tier_switch_outside_area',
  'entry_address_selected', 'account_handoff_redirect',
  // Priskalkulator
  'wizard_step', 'wizard_complete',
  // Kampanje/landingssider
  'magic_link_view', 'page_view_campaign',
  // Konseptsider
  'ny_concept_view', 'ny_concept_address_selected', 'ny2_concept_view',
  // Ringesporing + samtykke (aug. 2026)
  'call_click', 'consent_choice', 'consent_view',
]);
...
export async function computeAnalytics(db, days = 30) {
  ...
  for (const e of events) {
    ...
    if (e.type === 'form_step') {
      // KANONISK FLYT (14/7): kun events fra dagens flyt-versjon (utleier-v3)
      // telles — historiske labels fra utgåtte flyter (Velkommen, Bekreft,
      // gamle posisjoner) blandet trakten og skar rekkefølgen.
      if (!(e.meta && e.meta.flow === 'utleier-v3')) continue;
  ...
  // Skjema-steg (drop-off) — KANONISK rekkefølge (14/7): trakten speiler dagens
  // faktiske flyt 1:1 (adresse-inngang → tjenestevalg → wizardens tre steg).
  // Alle steg vises alltid (også med 0) så formen er stabil.
  const CANONICAL_FORM_STEPS = ['Adresse', 'Tjenestevalg', 'Eiendommen', 'Dine mål', 'Om deg'];
  const formSteps = CANONICAL_FORM_STEPS
    .map((label, i) => ({ label, count: stepMap.has(label) ? stepMap.get(label)._s.size : 0, order: i + 1 }))
    .map((s, i, arr) => ({
      ...s,
...
export function leadQuality(lead) {
  ...
  if (lead.forwarded && (lead.platform_id || lead.self_service)) s += 10;
...
export async function computeLeadIntel(db, days = 30) {
  ...
  const leads = await db.collection('leads')
    .find({ ...OWNER_LEAD_QUERY, createdAt: { $gte: fromIso }, deleted: { $ne: true } }).project({ _id: 0 }).sort({ createdAt: 1 }).limit(20000).toArray();
  ...
  const PIPE_LABEL = { new: 'Ny', contacted: 'Kontaktet', qualified: 'Kvalifisert', viewing: 'Befaring', offer: 'Tilbud sendt', won: 'Vunnet', lost: 'Tapt', disqualified: 'Diskvalifisert' };

  for (const l of leads) {
    const crmVerified = l.forwarded === true && (!!l.platform_id || l.self_service === true);
    const status = crmVerified ? 'Videresendt' : (l.status === 'new' ? 'Ny' : (l.status || 'Ny'));
  ...
  const PIPE_ORDER = ['Ny', 'Kontaktet', 'Kvalifisert', 'Befaring', 'Tilbud sendt', 'Vunnet', 'Tapt', 'Diskvalifisert'];
...
export async function computeLandingPages(db, days = 30, pages = []) {
  ...
  if (huseierSources.length) {
    const rows = await db.collection('leads')
      .find({ ...OWNER_LEAD_QUERY, createdAt: { $gte: fromIso }, source: { $in: huseierSources }, deleted: { $ne: true } })
...
export async function computeAdsEconomics(db, imp) {
  ...
  const paidLeads = await db.collection('leads')
    .find({ ...OWNER_LEAD_QUERY, createdAt: { $gte: fromIso, $lte: toIso }, 'attribution.channel': 'Betalt', deleted: { $ne: true } })
...
export async function computeMetaEconomics(db, snap) {
  ...
  const allLeads = await db.collection('leads')
    .find({ ...OWNER_LEAD_QUERY, createdAt: { $gte: fromIso, $lte: toIso }, deleted: { $ne: true } })
    .project({ _id: 0, status: 1, wonValue: 1, attribution: 1, createdAt: 1 })
    .limit(50000).toArray();
...
export async function computeAdsLeadsSeries(db, fromIso, toIso) {
  ...
  const rows = await db.collection('leads')
    .find({ ...OWNER_LEAD_QUERY, createdAt: { $gte: from, $lte: to }, deleted: { $ne: true } })
...
export async function buildPaidFunnel(db, { days = 30, adStats = {} } = {}) {
  ...
  // calls = økter med minst ett klikk på et telefonnummer. Telefon er historisk
  // vår beste kanal (3 av 5 vunne siste 90 d), men var helt usporet før aug. 2026.
  // consentGranted/Denied = grunnlaget for samtykkeraten. Meta-pixelen laster kun
  // ved samtykke, så samtykkeraten forklarer gapet mellom klikk og «sidevisning».
  const zero = () => ({ sessions: 0, formPage: 0, start: 0, step2: 0, step3: 0, submit: 0, calls: 0, leads: 0, qualified: 0, won: 0, wonValue: 0, consentGranted: 0, consentDenied: 0 });
  ...
  for (const e of evs) {
    ...
    if (!sid) continue;
    let s = sess.get(sid);
    if (!s) { s = { chan: 'other', stages: new Set(), consent: null }; sess.set(sid, s); }
    ...
    if (e.type === 'call_click') s.stages.add('calls');
    if (e.type === 'consent_choice' && meta.choice) s.consent = String(meta.choice);
  }
  for (const s of sess.values()) {
    const a = agg[s.chan] || agg.other;
    for (const st of s.stages) if (a[st] != null) a[st] += 1;
    if (s.consent === 'all') a.consentGranted += 1;
    else if (s.consent) a.consentDenied += 1;
  }
  ...
  const leads = await db.collection('leads')
    .find({ ...OWNER_LEAD_QUERY, createdAt: { $gte: fromIso }, deleted: { $ne: true } })
  ...
  const mk = (key, label, ads) => {
    ...
    const consentTotal = a.consentGranted + a.consentDenied;
    return {
      key, label,
      ...
      // Ringeklikk er også en henvendelse. Kost pr. henvendelse (lead + ringeklikk)
      // er et sannere effektivitetstall enn CPL alene.
      contacts: a.leads + a.calls,
      costPerContact: spend != null && (a.leads + a.calls) > 0 ? Math.round((spend / (a.leads + a.calls)) * 100) / 100 : null,
      costPerCall: spend != null && a.calls > 0 ? Math.round((spend / a.calls) * 100) / 100 : null,
      ...
      // Samtykkerate: forklarer hvorfor Meta rapporterer færre sidevisninger enn
      // klikk. Pixelen laster ikke uten samtykke — trafikken er der likevel.
      consentRate: consentTotal > 0 ? Math.round((a.consentGranted / consentTotal) * 1000) / 10 : null,
      consentAnswers: consentTotal,
    };
  };
  const channels = [
...
export async function computeVelocity(db, days = 90) {
  const fromIso = new Date(Date.now() - days * 86400000).toISOString();
  // self_service (selvforvaltning, auto-won ved opprettelse) holdes utenfor —
  // de var aldri i salgstrakten og ville skjevt tid-til-salg mot 0.
  const leads = await db.collection('leads')
    .find({ ...OWNER_LEAD_QUERY, createdAt: { $gte: fromIso }, deleted: { $ne: true }, self_service: { $ne: true } })
...
export async function computeMarketingTrends(db, { days = 84, adSeries = {} } = {}) {
  ...
  const created = await db.collection('leads').find({ ...OWNER_LEAD_QUERY, createdAt: { $gte: fromIso }, deleted: { $ne: true } }).project(proj).limit(50000).toArray();
  const wonExtra = await db.collection('leads').find({ ...OWNER_LEAD_QUERY, wonAt: { $gte: fromIso }, createdAt: { $lt: fromIso }, deleted: { $ne: true } }).project(proj).limit(50000).toArray();
