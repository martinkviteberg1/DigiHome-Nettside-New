...
export async function computeLandingPages(db, days = 30, pages = []) {
  ...
  const events = await db.collection('events')
    .find({ ts: { $gte: fromIso }, type: 'pageview', path: { $regex: '^/lp/' } })
  ...
  // Skjematrakt for 2-stegs skjemaene: lead_step-events (start → steg 2 → innsendt).
  // Unike sesjoner per steg → viser hvor brukerne faller fra.
  const funnelById = new Map(); // slug -> { start:Set, step2:Set, submit:Set }
  const stepEvents = await db.collection('events')
    .find({ ts: { $gte: fromIso }, type: 'lead_step' })
    .project({ _id: 0, path: 1, sessionId: 1, id: 1, meta: 1 })
    .limit(200000)
    .toArray();
