...
export async function sendMetaCapiEvent({
  ...
  fbp, fbc, fbclid,
  externalId, zip, city, state, country,
  clientIp, userAgent,
  value, currency = 'NOK',
  customData = {},
  testEventCode,
  ...
  try {
    ...
    // Geo (Metas krav: lowercase, strip mellomrom, hash). Hoeyere Event Match Quality.
    const norm = (v) => String(v || '').trim().toLowerCase().replace(/\s+/g, '');
    const hz = sha256(norm(zip)); if (hz) user_data.zp = [hz];
    const hc = sha256(norm(city)); if (hc) user_data.ct = [hc];
    const hs = sha256(norm(state)); if (hs) user_data.st = [hs];
    const hco = sha256(norm(country)); if (hco) user_data.country = [hco];
    // external_id (stabil first-party-ID, f.eks. visitorId) — hashet, bedre matching.
    if (externalId) { const hx = sha256(String(externalId)); if (hx) user_data.external_id = [hx]; }
    ...
    // Test-hendelser: parameter (per-kall) har forrang over env-variabelen.
    // Med test_event_code vises hendelsen KUN i Meta Events Manager → «Test events»
    // og påvirker IKKE annonsealgoritmen (trygg ende-til-ende-verifisering).
    const effectiveTestCode = testEventCode || TEST_CODE || '';
    ...
    return {
      ...
      events_received: j.events_received, fbtrace_id: j.fbtrace_id,
      messages: j.messages || undefined,
      testEvent: effectiveTestCode ? effectiveTestCode : undefined,
    };
  } catch (e) {
