...
// Engangs-migreringer for organisasjonskartet — idempotente via app_migreringer.
// 2026-02: DIGIHOME TECH AS står i Brønnøysund med BÅDE gammelt og nytt Blue
// Sky-byrå som aktiv regnskapsfører (registerdata). Bergen-enheten (936595960)
// er den operative — den gamle (921171986) skjules fra kartet i selskaper der
// begge er synlige. Reverserbart: admin kan vise rollen igjen fra person-
// skuffen, og synk overstyrer aldri skjult-flagget.
async function kjorOrgMigreringer(db) {
  const MIG = 'org-2026-02-skjul-utdatert-regnskapsforer-921171986';
  const gjort = await db.collection('app_migreringer').findOne({ id: MIG }, { projection: { _id: 1 } });
  if (gjort) return;
  const gammel = await db.collection(PERSON_COLL).findOne({ erEnhet: true, orgnr: '921171986' }, { projection: { id: 1 } });
  const ny = await db.collection(PERSON_COLL).findOne({ erEnhet: true, orgnr: '936595960' }, { projection: { id: 1 } });
  if (gammel && ny) {
    const nyRoller = await db.collection(ROLLE_COLL)
      .find({ personId: ny.id, rolleKode: 'REGN', skjult: { $ne: true } }, { projection: { selskapId: 1 } }).toArray();
    for (const nr of nyRoller) {
      await db.collection(ROLLE_COLL).updateMany(
        { personId: gammel.id, rolleKode: 'REGN', selskapId: nr.selskapId, skjult: { $ne: true } },
        { $set: { skjult: true } },
      );
    }
  }
  await db.collection('app_migreringer').insertOne({ id: MIG, kjortAt: new Date().toISOString() });
}

export async function hentOrganisasjon(db) {
  await kjorOrgMigreringer(db);
