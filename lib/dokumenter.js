...
export async function konverterDocxTilPdf(docxBuffer) {
  ...
  if (!_docxWasm) {
    ...
    // Prod (standalone) og dev har ulik filplassering — prøv kandidater i
    // rekkefølge. Wasm-filen traces inn i standalone-bygget via
    // outputFileTracingIncludes i next.config.js.
    const kandidater = [
      path.join(process.cwd(), 'node_modules', 'docx-to-pdf-wasm', 'build', 'docx-to-pdf.wasm'),
      path.join(process.cwd(), '..', 'node_modules', 'docx-to-pdf-wasm', 'build', 'docx-to-pdf.wasm'),
      '/app/node_modules/docx-to-pdf-wasm/build/docx-to-pdf.wasm',
    ];
    let wasmBuf = null; const feil = [];
    for (const sti of kandidater) {
      try { wasmBuf = await fs.readFile(sti); break; } catch (e) { feil.push(`${sti}: ${e.code || e.message}`); }
    }
    if (!wasmBuf) throw new Error(`Fant ikke docx-to-pdf.wasm — ${feil.join(' | ')}`);
    _docxWasm = await WebAssembly.compile(wasmBuf);
  }
