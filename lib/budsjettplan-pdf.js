...
export async function lagVekstbudsjettPdf({ plan }) {
  ...
  /* ════ SIDE 2: NØKKELTALL ════ */
  {
    ...
    const kort = erModell && s ? [
      ...
      ['Enheter under forvaltning', des(s.enheterVedSlutt), `${des(s.sumNyeBrutto)} nye brutto · ARR v/slutt ${krKompakt(s.arrExit || 0)}`],
    ] : [
      ['Sum inntekter', kr(sumInn), ''],
      ['Sum kostnader', kr(sumKost), ''],
  ...
  /* ════ SIDE 3: UTVIKLING (grafer + årsoversikt) ════ */
  {
    ...
    const kol = [MARG, MARG + 98, MARG + 188, MARG + 276, MARG + 330, MARG + 392, W - MARG];
    const hodeT = ['År', 'Inntekter', 'Kostnader', 'Resultat', 'Res.grad', 'Enh. v/slutt', 'ARR v/slutt'];
    ...
    for (const [aarNavn, idx] of Object.entries(grupper)) {
      ...
      hoyre(page, erModell && m ? des(m.enheter[sisteI]) : '—', kol[5], y3, { font: F.body, size: 9, color: BLEKK });
      hoyre(page, krKompakt((innSerie[sisteI] || 0) * 12), kol[6], y3, { font: F.body, size: 9, color: GRAA });
    ...
    hoyre(page, krKompakt((innSerie[N - 1] || 0) * 12), kol[6], y3, { font: F.medium, size: 9, color: GRAA });
  ...
  if (erModell && m) {
    ...
    // Årlig justering — gjelder fra år 2 i planen
    yv -= 30;
    tekst(page, 'ÅRLIG JUSTERING (FRA ÅR 2)', MARG, yv, { font: F.medium, size: 7, color: LILLA, tracking: 1.2 });
    yv -= 6;
    for (const [l, v] of [
      ['Indeksregulering leie (KPI)', `${kommatall(d.indeksPct || 0)} %/år`],
      ['Lønnsvekst', `${kommatall(d.lonnsvekstPct || 0)} %/år`],
      ['Kostnadsinflasjon', `${kommatall(d.kostInflasjonPct || 0)} %/år`],
    ]) {
      yv -= 17;
      tekst(page, l, MARG, yv, { font: F.body, size: 8.5, color: rgb(0.34, 0.33, 0.31) });
      hoyre(page, v, MARG + 225, yv, { font: F.medium, size: 8.5, color: BLEKK });
