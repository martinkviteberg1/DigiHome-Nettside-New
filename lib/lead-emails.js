...
export function buildLeadReceipt(lead = {}) {
  ...
  const bodyHtml = `
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td style="padding:36px 40px 40px;">
      ...
      <p style="margin:0;font-size:11px;font-weight:500;letter-spacing:2.4px;text-transform:uppercase;color:#9B5BD6;font-family:${BODY_FF};">Vi er i gang</p>
      <h1 style="margin:12px 0 0;font-size:31px;line-height:1.1;letter-spacing:-1px;color:#0A0A0A;font-family:${HEAD_FF};font-weight:700;">Takk${fn ? ', ' + esc(fn) : ''} — vi ser på boligen din nå.</h1>
...
export function buildLeadAdminNotification(lead = {}, { kind = 'huseier' } = {}) {
  ...
  const rowHtml = ([k, v]) => `
      <tr>
        ...
        <td style="padding:9px 0;font-size:14px;font-weight:600;color:#0A0A0A;border-bottom:1px solid #F3EEE8;font-family:${BODY_FF};">${esc(v)}</td>
      </tr>`;
  ...
  const bodyHtml = `
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td style="padding:36px 40px 38px;">
      ...
      <table role="presentation" cellpadding="0" cellspacing="0" style="margin:16px 0 0;"><tr>
        ...
        <td style="padding-right:8px;">${pill(lead.lead_type || kind, '#E9F7EF', '#1F7A4D')}</td>
        ${lead.source ? `<td>${pill(lead.source, '#F0EAE2', '#57504A')}</td>` : ''}
      </tr></table>
      ...
      ${section('Boligen', [
        ['Boligtype', lead.property_type], ['Soverom', lead.bedrooms],
        ['Areal', lead.sqm ? `${lead.sqm} m²` : ''],
        ['Ønsket modell', RENTAL_LABEL[rentalRaw] || rentalRaw],
        ['Matrikkel', matrikkel], ['Bygningstype', lead.bygningstype],
        ['Hjemmelshaver', hjemmelshaver], ['Ønsket område', lead.preferred_area],
      ])}
      ${section('Sporing', [
        ['Kampanje', att.campaign], ['Annonse', att.content], ['Søkeord', att.term],
        ['Henvist fra', refHost], ['Klikk-ID', clickIds], ['Landingsside', att.landing_page],
      ])}
