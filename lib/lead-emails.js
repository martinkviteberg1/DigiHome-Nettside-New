...
// Logo som PNG (SVG blokkeres av Gmail/Outlook) + aksentlinje + lenke-footer.
...
<body style="margin:0;padding:0;background:#F5F1EC;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#F5F1EC;padding:36px 16px;">
    <tr><td align="center">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:560px;">
        <tr><td style="padding:0 10px 20px;">
          <a href="${esc(BASE_URL)}" style="text-decoration:none;">
            <img src="${esc(logoUrl)}" alt="DigiHome" width="118" height="28" style="display:block;width:118px;height:28px;border:0;" />
          </a>
        </td></tr>
        <tr><td style="height:6px;border-radius:22px 22px 0 0;background:#9B5BD6;background:linear-gradient(90deg,#9B5BD6 0%,#D298FF 60%,#F0C86B 100%);font-size:0;line-height:0;">&nbsp;</td></tr>
        <tr><td style="background:#FFFFFF;border-radius:0 0 22px 22px;padding:38px 36px 34px;box-shadow:0 10px 34px rgba(10,10,10,0.07);">
          ${bodyHtml}
        </td></tr>
        <tr><td style="padding:24px 10px 0;text-align:center;color:#8A8178;font-size:12px;line-height:1.8;">
...
export function buildLeadReceipt(lead = {}) {
  ...
  const bodyHtml = `
    ...
    <p style="margin:16px 0 0;font-size:15px;line-height:1.65;color:#57504A;">
      ...
      og tar kontakt for en uforpliktende prat — <b>som regel i løpet av dagen</b>, og senest neste virkedag.
    </p>
    ...
    <table role="presentation" cellpadding="0" cellspacing="0" style="margin:26px 0 0;"><tr>
      <td style="padding-right:10px;"><a href="${esc(BASE_URL)}/bli-utleier" style="display:inline-block;background:#0A0A0A;color:#fff;font-size:14px;font-weight:700;padding:13px 24px;border-radius:999px;text-decoration:none;">Se hvordan det fungerer</a></td>
      <td><a href="tel:${esc(site.phoneHref)}" style="display:inline-block;background:#F0EAE2;color:#0A0A0A;font-size:14px;font-weight:700;padding:13px 24px;border-radius:999px;text-decoration:none;">Ring oss</a></td>
    </tr></table>
    <p style="margin:22px 0 0;font-size:14px;line-height:1.6;color:#57504A;">
      Haster det? Ring oss direkte på <a href="tel:${esc(site.phoneHref)}" style="color:#9B5BD6;font-weight:700;text-decoration:none;">${esc(site.phone)}</a> — så tar vi det med en gang.
    </p>
  ...
  const preheader = 'Takk! En lokal forvalter går gjennom detaljene og tar kontakt — som regel i løpet av dagen.';
  return {
    subject: 'Vi har mottatt henvendelsen din — en lokal forvalter tar kontakt',
...
export async function sendLeadAutoReceipt(lead) {
  ...
  return sendHtmlEmail({ to: lead.email, subject, html, fromName: 'DigiHome' });
}
...
export async function sendLeadAdminNotification(lead, { kind = 'huseier' } = {}) {
  ...
  return sendHtmlEmail({ to, subject, html, fromName: 'DigiHome Leads', replyTo });
}
