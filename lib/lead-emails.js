...
import { resolveOwnerKind } from '@/lib/self-service';
...
// HVILKEN miljøvariabel styrer mottakerne, og hvem står der? Preview og
// produksjon har ulike env-verdier, og «bare én person får varselet» skyldes
// nesten alltid at overstyringen mangler i det ene miljøet. Presedensen her MÅ
// speile leadNotifyRecipients() og interestNotifyRecipients().
export function notifyRecipientSources(env = process.env) {
  const pick = (names) => {
    for (const n of names) {
      const raw = String(env[n] || '').trim();
      if (!raw) continue;
      const recipients = raw.split(',').map((s) => s.trim()).filter(Boolean);
      if (recipients.length) return { source: n, recipients };
    }
    return { source: null, recipients: [] };
  };
  return {
    boliginteresse: pick(['NL_INTEREST_NOTIFY', 'LEAD_NOTIFY_RECIPIENTS', 'ADS_REPORT_RECIPIENTS', 'ADMIN_SEED_EMAIL']),
    leads: pick(['LEAD_NOTIFY_RECIPIENTS', 'ADS_REPORT_RECIPIENTS', 'ADMIN_SEED_EMAIL']),
  };
}
...
export function buildLeadReceipt(lead = {}) {
  ...
  const introHtml = isSelf
    ? `Du har valgt <b style="color:#0A0A0A;">selvforvaltning — 5&nbsp;% av husleien</b>, uten faste kostnader eller bindingstid.
        ${hasAccount
  ...
  // Kvitteringen er kundens kopi av avtalen (avtalen sier «du får en kopi på
  // e-post»): lenke til teksten + versjon. Privatpersoner får i tillegg
  // angrerettsinformasjon og standard angreskjema — angrerettloven krever at
  // begge gis på et varig medium, ellers forlenges fristen til 12 måneder.
  const erPrivat = resolveOwnerKind(lead) !== 'business';
  const avtaleUrl = `${BASE_URL}/avtale/selvforvaltning`;
  const godtattTid = lead.terms_accepted && lead.terms_accepted.at
    ? new Date(lead.terms_accepted.at).toLocaleString('nb-NO', { timeZone: 'Europe/Oslo', dateStyle: 'long', timeStyle: 'short' }) : '';
  const termsHtml = isSelf && lead.terms_accepted && lead.terms_accepted.at
    ? `
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:14px 0 0;background:#EAF7EF;border-radius:14px;">
        <tr><td style="padding:13px 18px;font-size:13px;line-height:1.55;color:#1F7A4D;font-family:${BODY_FF};">
          &#10003;&nbsp; Avtale om selvforvaltning godtatt digitalt — ${esc(godtattTid)}${lead.terms_accepted.version ? ` <span style="color:#5E9C79;">(${esc(lead.terms_accepted.version)})</span>` : ''}<br />
          <a href="${avtaleUrl}" style="color:#1F7A4D;font-weight:600;">Les avtalen du godtok</a> — 5&nbsp;% av innbetalt husleie, ingen bindingstid.
        </td></tr>
      </table>${erPrivat ? `
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:10px 0 0;background:#F7F2EB;border-radius:14px;">
        <tr><td style="padding:14px 18px;font-size:12.5px;line-height:1.6;color:#57504A;font-family:${BODY_FF};">
          <b style="color:#0A0A0A;">Angrerett.</b> Som privatperson har du 14 dagers angrerett fra i dag (angrerettloven). Du angrer ved å sende en tydelig melding til
          <a href="mailto:support@digihome.no" style="color:#9B5BD6;font-weight:600;text-decoration:none;">support@digihome.no</a>.
          Tjenesten starter straks på din anmodning; angrer du etter at den er tatt i bruk, betaler du honorar for leie innbetalt fram til meldingen.<br />
          <span style="color:#8A8178;">Standard angreskjema — kopier inn i e-posten: «Til ${esc(site.legalName)}, ${esc(site.address.street)}, ${esc(site.address.postal)} ${esc(site.address.city)}. Jeg underretter herved om at jeg ønsker å gå fra min avtale om selvforvaltning, inngått ${esc(godtattTid)}. Navn, adresse, dato.»</span>
        </td></tr>
      </table>` : ''}` : '';
  const bodyHtml = `
    ...
    <table role="presentation" width="100%" cellpadding="0" cellspacing="0"><tr><td class="dh-pad" style="padding:36px 40px 38px;">
      ...
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="margin:18px 0 0;background:#F6F0FE;border-radius:14px;">
        <tr><td style="padding:14px 18px;font-size:13px;line-height:1.6;color:#5B3E85;font-family:${BODY_FF};">
          <b style="color:#4A2E75;">Selvbetjent løp:</b> Avtalen (5&nbsp;% av husleien) er akseptert digitalt i skjemaet — dette er en kunde, ikke et salgslead.
      ...
      ${section('Kontakt', [
        ...
        // Ubekreftet org.nr flagges — da må identiteten sjekkes manuelt før
        // kontrakt og faktura.
        ['Registrerer som', lead.owner_kind === 'business' ? 'Bedrift' : ''],
        ['Selskap', lead.owner_kind === 'business'
          ? [lead.company_name || 'Navn ikke oppgitt', lead.org_no ? `org.nr ${lead.org_no}` : '', lead.company_form || ''].filter(Boolean).join(' · ')
          : ''],
        ['Org.nr sjekket', lead.owner_kind === 'business'
          ? (lead.company_verified
            ? `Bekreftet i Enhetsregisteret${lead.company_status && lead.company_status !== 'aktiv' ? ` — <b style="color:#B42318;">${esc(lead.company_status)}</b>` : ''}`
            : `<b style="color:#B42318;">Ikke bekreftet</b>${lead.company_verify_note ? ` — ${esc(lead.company_verify_note)}` : ''}`)
          : '', true],
...
export async function sendLeadAdminNotification(lead, { kind = 'huseier' } = {}) {
  if (!emailConfigured()) return { ok: false, skipped: 'sendgrid-ikke-konfigurert' };
  // Testleads (@example.com og liknende) skal ikke vekke teamet. Samme regel som
  // for boliginteresse-varselet — et varsel teamet lærer å overse er verdiløst.
  if (isUndeliverableTestAddress(lead?.email)) return { ok: false, skipped: 'testlead' };
