...
export async function computeKpiDashboard(db, { days = 30, from, to } = {}) {
  ...
  const proj = { projection: { _id: 0, status: 1, createdAt: 1, wonAt: 1, statusUpdatedAt: 1, firstResponseAt: 1, forwarded_at: 1, wonValue: 1, wonValueActual: 1, wonValueEstimate: 1, wonCurrency: 1, name: 1, 'attribution.channel': 1 } };
  ...
  // Drill-down for KPI-modalen: hvilke kunder som faktisk teller i CAC/omsetning.
  const wonRows = [];
  ...
  for (const l of leads) {
    ...
    if (inWin(w, win.curFrom, win.curTo)) {
      ...
      const c = ts(l.createdAt);
      let ttwDays = null;
      if (isFinite(c) && w >= c) { const days = (w - c) / 86400000; if (days <= 730) { ttwSum += days; ttwCount++; ttwDays = round1(days); } }
      wonRows.push({
        name: l.name || 'Uten navn',
        wonAt: d,
        value: round2(val),
        channel: (l.attribution && l.attribution.channel) || 'Direkte',
        daysToWin: ttwDays,
      });
    } else if (inWin(w, win.prevFrom, win.prevTo)) { newCustPrev++; revenuePrev += val; }
  }
  wonRows.sort((a, b) => (b.value || 0) - (a.value || 0) || String(b.wonAt).localeCompare(String(a.wonAt)));
  ...
  // Kost pr. lead som FAKTISK kom fra annonser. cpl over deler forbruket på alle
  // huseierleads (også organiske) og er derfor for gunstig.
  const cplPaid = eff.cplPaid != null ? eff.cplPaid : null;
  const paidLeadsCur = mm?.leads?.paid != null ? mm.leads.paid : null;
  ...
  // --- LTV-modell ---
  let ltv, ltvBasis, paybackMonths = null, paybackBasis = null;
  ...
  if (settings.ltvMode === 'actual' && actualLtv > 0) {
    ...
    if (cac != null && monthlyGp > 0) paybackMonths = round1(cac / monthlyGp + waitMonths);
    paybackBasis = {
      mode: 'actual', cac,
      monthlyFee: revenueModel.ltv.monthlyFee || 0,
      marginPct: settings.grossMarginPct != null ? settings.grossMarginPct : 100,
      monthlyGrossProfit: round2(monthlyGp),
      waitMonths: round1(waitMonths),
      daysToFirstLease: revenueModel.timing?.daysToFirstLease ?? null,
    };
    ...
    paybackBasis = {
      mode: 'recurring', cac, monthlyFee: settings.monthlyFee,
      marginPct: settings.grossMarginPct != null ? settings.grossMarginPct : 100,
      monthlyGrossProfit: round2(settings.monthlyFee * marginFrac), waitMonths: 0, daysToFirstLease: null,
    };
  } else {
    ltv = avgCustomerValue;
    ltvBasis = 'contract';
    paybackMonths = eff.cacPaybackMonths != null ? eff.cacPaybackMonths : null;
    paybackBasis = { mode: 'contract', cac, monthlyFee: null, marginPct: 100, monthlyGrossProfit: null, waitMonths: 0, daysToFirstLease: null };
  }
  const ltvCac = (cac != null && cac > 0 && ltv > 0) ? round1(ltv / cac) : null;
  // Framoverskuende LTV:CAC — inkluderer KONTRAHERTE leiekontrakter (inngått,
  // ikke startet). Vises alltid side om side med realisert, aldri i stedet for.
  const ltvContractedBasis = revenueModel?.ltv?.contractedBasis || null;
  const ltvContracted = ltvContractedBasis?.value ?? null;
  const ltvCacContracted = (cac != null && cac > 0 && ltvContracted > 0) ? round1(ltvContracted / cac) : null;
  ...
  return {
    ...
    hero: {
      ...
      // Framoverskuende variant — signert, men ikke startet leie er tatt med.
      ltvCacContracted: {
        value: ltvCacContracted, ltv: ltvContracted, cac, unit: ':1',
        monthlyFee: ltvContractedBasis?.monthlyFee ?? null,
        customersWithLease: ltvContractedBasis?.customersWithLease ?? null,
      },
      cpl: {
        value: cpl, delta: null, unit: 'kr', inverse: true,
        // Skiller mellom «annonseforbruk / alle huseierleads» (cpl, flatterende)
        // og «annonseforbruk / leads som faktisk kom fra annonser» (paid).
        paid: cplPaid, paidLeads: paidLeadsCur,
        allLeads: mm?.leads?.utleier ?? null,
        organicLeads: mm?.leads?.organic ?? null,
        basis: 'cpl = forbruk / alle huseierleads · paid = forbruk / annonse-attribuerte huseierleads',
      },
      ...
      timeToWin: { value: timeToWin, unit: 'dager', inverse: true, sampleSize: ttwCount },
      conversionRate: {
        value: conversionRateCur, delta: pctChange(conversionRateCur, conversionRatePrev), unit: '%',
        basis: { customers: newCustCur, leads: newLeadsCur, prevCustomers: newCustPrev, prevLeads: newLeadsPrev, prevPct: conversionRatePrev },
      },
    },
    metrics: {
      ...
      spend: { total: round2(spend.total || 0), google: round2(spend.google || 0), meta: round2(spend.meta || 0), unit: 'kr', sources: mm?.spendSources || null, campaigns: mm?.topCampaigns || [] },
      roasTrue: { value: roasTrue, unit: 'x' },
      roasAds: { value: roasAds, unit: 'x' },
      responseHours: { value: respCount ? round1(respSum / respCount) : null, sla24hPct: respCount ? Math.round((slaHit / respCount) * 100) : null, unit: 't', count: respCount, slaHit },
      paybackMonths: { value: paybackMonths, unit: 'mnd', basis: paybackBasis },
      pipeline,
    },
    funnel,
    runRate,
    ...
    // Drill-down for KPI-detaljmodalen. Admin-only (bak API-nøkkel) — brukes til
    // å vise HVILKE poster som faktisk utgjør et tall, ikke bare summen.
    drill: {
      wonInPeriod: wonRows.slice(0, 40),
      wonInPeriodTotal: wonRows.length,
      campaigns: mm?.topCampaigns || [],
      spendSources: mm?.spendSources || null,
    },
