...
export async function lagVekstbudsjettExcel({ plan }) {
  ...
  if (erModell && m) {
    ...
    const R = { nye: 6, enh: 7, prisF: 8, kostF: 9, innFra: 11, eksist: 11, reut: 12, vekst: 13, oppstart: 14, innSum: 15, kostFra: 17, kostSum: 24, res: 26, akk: 27, grad: 28 };
    ...
    rad(md, rr, 'Markedsføring (fast)', fSerie((k) => `${F.mf}*${k}${R.kostF}`), { beIdx }); rr += 1;
    rad(md, rr, 'Salgsprovisjon / CAC', fSerie((k) => `${k}${R.nye}*${F.prov}*${k}${R.kostF}`), { stripe: true, beIdx }); rr += 1;
    rad(md, rr, 'Performance-partner (fast honorar + % av honorar fra nye enheter)', m.kost.partner || Array(N).fill(0), { beIdx }); rr += 1;
    rad(md, rr, 'Administrasjon', fSerie((k) => `${F.admin}*${k}${R.kostF}`), { stripe: true, beIdx }); rr += 1;
    rad(md, rr, 'Andre faste kostnader', fSerie((k) => `${F.andre}*${k}${R.kostF}`), { beIdx }); rr += 1;
    const kostSumRad = rr;
