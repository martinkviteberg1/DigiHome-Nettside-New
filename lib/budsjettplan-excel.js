...
export async function lagVekstbudsjettExcel({ plan }) {
  ...
  if (erModell && s) {
    ...
    kv(ov, r, 'ARR run-rate ved periodens slutt', s.arrExit || 0, { fmt: VALUTA, note: 'siste måneds inntekt × 12 — exit run-rate' }); r += 1;
    ...
    // Flerårsplaner: ett blikk per planår — resultat, inntekter, enheter og ARR
    if ((m.aar || []).length > 1) {
      kvSeksjon(ov, r, 'Per planår'); r += 1;
      for (const a of m.aar) {
        kv(ov, r, `År ${a.nr} (${mndNavn(plan.startYm, a.fraIdx)} – ${mndNavn(plan.startYm, a.tilIdx)})`, a.resultat, {
          fmt: RESULTATFMT,
          note: `inntekter ${a.inntekt.toLocaleString('nb-NO')} kr · ${Math.round(a.enheterSlutt)} enheter v/slutt · ARR ${a.arrExit.toLocaleString('nb-NO')} kr`,
        }); r += 1;
      }
      r += 1;
    }
  ...
  if (erModell && m) {
    /* Fast radkart — gjør at Sammendrag/Årsoversikt kan referere levende celler.
       ...
       prisF/kostF: årlige justeringsfaktorer (indeksregulering/inflasjon) som
       LEVENDE formler — trappes per planår (INT((COLUMN()-2)/12)). */
    // Performance-partner-raden (kohortbasert honorar) er en verdiserie fra motoren — skyver
    // kostSum/res/akk/grad én rad ned i forhold til tidligere versjoner.
    // + Grunnleggere-raden (lønn i trinn, fordelt mellom selskapene) skyver kostSum/res/akk/grad én rad til.
    const R = { nye: 6, enh: 7, prisF: 8, kostF: 9, innFra: 11, eksist: 11, reut: 12, vekst: 13, oppstart: 14, innSum: 15, kostFra: 17, kostSum: 25, res: 27, akk: 28, grad: 29 };
    // Linjer med trinn (nominelle beløp fra gitt måned) eller organisk andel kan ikke uttrykkes som én
    // formel mot Forutsetninger — da eksporteres motorens verdiserie i stedet (merket i etiketten).
    const harTrinn = (felt) => (m.drivere?.kostTrinn?.[felt] || []).length > 0;
    const organisk = Number(m.drivere?.organiskAndelPct) || 0;
    const F = { oppstart: "Forutsetninger!$B$8", system: "Forutsetninger!$B$12", mf: "Forutsetninger!$B$15", prov: "Forutsetninger!$B$16", admin: "Forutsetninger!$B$17", andre: "Forutsetninger!$B$18", indeks: "Forutsetninger!$B$21", inflasjon: "Forutsetninger!$B$23" };
    ...
    rad(md, rr, 'Prisfaktor (indeksregulering leie, per planår)', fSerie(() => `POWER(1+${F.indeks}/100,INT((COLUMN()-2)/12))`), { fmt: '0.000', sum: 'siste', beIdx }); rr += 1;
    rad(md, rr, 'Kostnadsfaktor (inflasjon, per planår)', fSerie(() => `POWER(1+${F.inflasjon}/100,INT((COLUMN()-2)/12))`), { fmt: '0.000', sum: 'siste', stripe: true, beIdx }); rr += 1;

    seksjon(md, rr, 'Inntekter', N); rr += 1;
    rad(md, rr, 'Kontraktsfestet honorar (dagens portefølje)', m.eksisterende, { beIdx }); rr += 1;
    rad(md, rr, 'Forventet re-utleie ved kontraktslutt', m.reutleie || Array(N).fill(0), { stripe: true, beIdx }); rr += 1;
    ...
    rad(md, rr, 'Systemkostnad', fSerie((k) => `${k}${R.enh}*${F.system}*${k}${R.kostF}`), { beIdx }); rr += 1;
    rad(md, rr, 'Bemanning (budsjettert trapp, inkl. lønnsvekst)', m.kost.bemanning, { stripe: true, beIdx }); rr += 1;
    rad(md, rr, harTrinn('mfFast') ? 'Markedsføring (fast, i trinn)' : 'Markedsføring (fast)', harTrinn('mfFast') ? m.kost.mfFast : fSerie((k) => `${F.mf}*${k}${R.kostF}`), { beIdx }); rr += 1;
    rad(md, rr, organisk > 0 ? `Salgsprovisjon / CAC (${100 - organisk} % av nye er betalte)` : 'Salgsprovisjon / CAC', organisk > 0 ? m.kost.provisjon : fSerie((k) => `${k}${R.nye}*${F.prov}*${k}${R.kostF}`), { stripe: true, beIdx }); rr += 1;
    rad(md, rr, 'Performance-partner (fast honorar + % av honorar fra nye enheter)', m.kost.partner || Array(N).fill(0), { beIdx }); rr += 1;
    rad(md, rr, harTrinn('adminFast') ? 'Administrasjon (i trinn)' : 'Administrasjon', harTrinn('adminFast') ? m.kost.admin : fSerie((k) => `${F.admin}*${k}${R.kostF}`), { stripe: true, beIdx }); rr += 1;
    rad(md, rr, harTrinn('andreFaste') ? 'Andre faste kostnader (i trinn)' : 'Andre faste kostnader', harTrinn('andreFaste') ? m.kost.andre : fSerie((k) => `${F.andre}*${k}${R.kostF}`), { beIdx }); rr += 1;
    rad(md, rr, 'Grunnleggere (lønn i trinn inkl. påslag, selskapets andel)', m.kost.grunnleggere || Array(N).fill(0), { stripe: true, beIdx }); rr += 1;
    const kostSumRad = rr;
    ...
    if (aarY0 && aarM0) {
      ...
      aar.columns = [{ width: 14 }, { width: 17 }, { width: 17 }, { width: 17 }, { width: 13 }, { width: 15 }, { width: 17 }, { width: 18 }];
      tittel(aar, 'Årsoversikt', `${plan.navn} · levende summer fra Månedsbudsjett-arket`);
      const hodeA = ['År', 'Inntekter', 'Kostnader', 'Resultat', 'Resultatgrad', 'Nye enheter', 'Enheter ved årsslutt', 'ARR v/årsslutt'];
      hodeA.forEach((h, i) => {
      ...
      for (const [y, idx] of Object.entries(grupper)) {
        ...
        aar.getCell(`H${ra}`).value = { formula: `'${MB}'!${c1}${R.innSum}*12` };
        ...
        aar.getCell(`H${ra}`).numFmt = VALUTA;
        for (let cc = 1; cc <= 8; cc++) { aar.getRow(ra).getCell(cc).border = { bottom: KANT }; if (ra % 2) aar.getRow(ra).getCell(cc).fill = STRIPE_FYLL; }
        ra += 1;
      }
      // Totalrad
      ...
      aar.getCell(`H${ra}`).value = { formula: `H${ra - 1}` };
      aar.getCell(`H${ra}`).numFmt = VALUTA;
      for (let cc = 1; cc <= 8; cc++) { const cell = aar.getRow(ra).getCell(cc); cell.fill = SUM_FYLL; cell.font = { bold: true, size: 10 }; }
    }
  } else {
  ...
  if (erModell && m) {
    ...
    // FASTE RADER B21–B23 — Månedsbudsjettets faktor-formler peker hit.
    kvSeksjon(fo, fr, 'Årlig justering (trappes per planår — år 1 påvirkes aldri)'); fr += 1;
    kv(fo, fr, 'Indeksregulering leie (KPI)', d.indeksPct, { fmt: '0.0" %"', note: 'husleieloven § 4-2 — alle inntektslag unntatt oppstartshonorar' }); fr += 1;
    kv(fo, fr, 'Lønnsvekst per år', d.lonnsvekstPct, { fmt: '0.0" %"', note: 'bemanningskostnaden (bakt inn i bemanningsraden)' }); fr += 1;
    kv(fo, fr, 'Kostnadsinflasjon per år', d.kostInflasjonPct, { fmt: '0.0" %"', note: 'system, markedsføring, CAC, administrasjon og andre faste' }); fr += 2;
    ...
    if (d.partner && d.partner.paa) {
      fr += 1;
      kvSeksjon(fo, fr, 'Performance-partner (markedsføring på resultat)'); fr += 1;
      kv(fo, fr, 'Fast honorar/mnd', d.partner.fastPerMnd, { fmt: VALUTA, note: 'indekseres med kostnadsinflasjon' }); fr += 1;
      kv(fo, fr, 'Honorar av honoraret fra nye enheter', `${String(d.partner.honorarPct).replace('.', ',')} %`, { note: `i enhetens første ${d.partner.varighetMnd > 0 ? `${d.partner.varighetMnd} måneder` : 'levetid'} — kohortbasert, følger churn` }); fr += 1;
      kv(fo, fr, 'Andel av nye enheter tilskrevet partner', `${String(d.partner.andelNyePct).replace('.', ',')} %`); fr += 1;
      kv(fo, fr, 'Starter i måned', d.partner.fraMnd, { fmt: '#,##0' }); fr += 1;
    }
