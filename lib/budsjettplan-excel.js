...
export async function lagVekstbudsjettExcel({ plan }) {
  ...
  if (erModell && m) {
    ...
    const R = { nye: 6, enh: 7, prisF: 8, kostF: 9, innFra: 11, eksist: 11, reut: 12, vekst: 13, oppstart: 14, innSum: 15, kostFra: 17, kostSum: 23, res: 25, akk: 26, grad: 27 };
    ...
    rad(md, rr, 'Administrasjon', fSerie((k) => `${F.admin}*${k}${R.kostF}`), { beIdx }); rr += 1;
    rad(md, rr, 'Andre faste kostnader', fSerie((k) => `${F.andre}*${k}${R.kostF}`), { stripe: true, beIdx }); rr += 1;
    const kostSumRad = rr;
