...
export async function weeklyReport({ economics, combined, recommendations = [], topAds = [], searchTerms = [], period = 'siste 30 dager' } = {}) {
  ...
  const user = `Data (JSON):\n${JSON.stringify(data)}\n\nSkriv en ukerapport med disse seksjonene: 1) kort oppsummering (2-3 setninger), 2) "Hva gikk bra", 3) "Hva sløser penger", 4) "Anbefalte handlinger denne uka" (prioritert punktliste). Maks ~250 ord.`;
