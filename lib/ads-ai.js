...
export async function generateRsaCopy({ theme = 'Utleie i Bergen', examples = [] } = {}) {
  ...
  const content = await chatLLM({ messages: [{ role: 'system', content: sys }, { role: 'user', content: user }], temperature: 0.7, maxTokens: 900, feature: 'ads_rsa_tekst' });
...
export async function generateMetaCopy({ theme = 'Utleie i Bergen' } = {}) {
  ...
  const content = await chatLLM({ messages: [{ role: 'system', content: sys }, { role: 'user', content: user }], temperature: 0.75, maxTokens: 800, feature: 'ads_meta_tekst' });
  const j = parseJsonLoose(content) || {};
  return {
...
export async function weeklyReport({ economics, combined, recommendations = [], topAds = [], searchTerms = [], campaigns = [], period = 'siste 30 dager' } = {}) {
  ...
  return chatLLM({ messages: [{ role: 'system', content: sys }, { role: 'user', content: user }], temperature: 0.4, maxTokens: 900, feature: 'ads_ukerapport' });
}
