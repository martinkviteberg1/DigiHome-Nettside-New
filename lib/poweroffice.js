...
// Delte integrasjonsnøkler til stede? (klientnøkkel kommer per selskap)
export function poFellesKonfigurert() {
  return Boolean(APP_KEY && SUB_KEY);
}
...
async function hentToken(ctx) {
  ...
  if (!poFellesKonfigurert()) throw new Error('PowerOffice er ikke konfigurert (mangler app-/abonnementsnøkkel i .env)');
  ...
  const basic = Buffer.from(`${APP_KEY}:${clientKey}`).toString('base64');
