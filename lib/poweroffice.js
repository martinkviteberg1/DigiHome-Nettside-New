...
// Pågående token-henting per nøkkel: parallelle kall (f.eks. resultat + balanse
// samtidig fra decket) deler ÉN OAuth-forespørsel i stedet for å kappes om den.
// Uten dette kunne ett av to samtidige kall få avslag hos PowerOffice på kald cache
// (typisk første treff i prod) → «0 kr» i decket mens admin virket.
const _tokenPaagaar = new Map(); // `${env}::${clientKey}` -> Promise<string>
...
async function hentToken(ctx) {
  ...
  const paagaar = _tokenPaagaar.get(key);
  ...
  const p = (async () => {
    try {
      ...
      _tokenPaagaar.delete(key);
    }
  })();
  _tokenPaagaar.set(key, p);
  return p;
}

const sov = (ms) => new Promise((res) => setTimeout(res, ms));

async function poFetch(path, params, ctx, forsok = 0) {
  const { env, clientKey } = normCtx(ctx);
  const { sub } = creds(env);
  ...
  if (!r.ok) {
    // 401: token kan være trukket tilbake tidligere enn utløp → tøm cache, hent nytt, prøv én gang.
    if (r.status === 401 && forsok === 0) { _tokenCache.delete(`${env}::${clientKey}`); return poFetch(path, params, ctx, 1); }
    // 429/5xx: forbigående hos PowerOffice → kort pause og inntil to nye forsøk.
    if ((r.status === 429 || r.status >= 500) && forsok < 2) {
      const ra = Number(r.headers.get('retry-after')) * 1000;
      await sov(Math.min(ra || (r.status === 429 ? 900 : 450 * (forsok + 1)), 3000));
      return poFetch(path, params, ctx, forsok + 1);
    }
    const t = await r.text().catch(() => '');
  ...
  if (!txt) return [];
  let data;
  try { data = JSON.parse(txt); } catch (e) { return []; } // tomt/ikke-JSON svar (periode uten posteringer) → tom liste, ikke krasj
