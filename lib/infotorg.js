...
export async function addressToMatrikkel(address) {
  ...
  // Google-labels har komma («Lien 7, 5057 Bergen»). Geonorge tokeniserer på
  // ord — komma på tokens («7,») kan ødelegge matchingen. Normaliser bort.
  clean = clean.replace(/,/g, ' ').replace(/\s+/g, ' ').trim();
  if (!clean) return null;
  const url = `${KARTVERKET_ADDR_API}/sok?sok=${encodeURIComponent(clean)}&treffPerSide=10&utkoordsys=4326`;
  // Geonorge svarer normalt <500ms. 2 forsøk à 5s: rask ærlig feil når tjenesten
  // er nede (~11s) i stedet for ~19-30s spinner for brukeren.
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (!hits.length) return null; // GENUINT ikke funnet (tjenesten svarte)
      // Beste treff — «Lien 7» finnes flere steder i Norge: (1) matcher søkets
      // postnummer, (2) Bergen kommune (4601), (3) første nasjonale treff.
      const pm = clean.match(/\b(\d{4})\b/);
      const best =
        (pm && hits.find((h) => String(h.postnummer || '') === pm[1])) ||
        hits.find((h) => String(h.kommunenummer || '') === '4601') ||
        hits[0];
      return {
  ...
  // Alle forsøk feilet på nett/timeout — tjenesten er NEDE, adressen er ikke
  // «ikke funnet». Kalleren skal skille disse (502 vs 404).
