...
// Bygger en kompakt, stabil JSON-kontrakt for ett tidsvindu (default 7 dager).
export async function buildMarketingMetrics(db, { days = 7, customerId } = {}) {
  const d = Math.max(1, Math.min(90, Number(days) || 7));
  const cid = customerId || defaultCustomerId();
  const now = Date.now();
  const curFrom = now - d * 86400000;
  const prevFrom = now - 2 * d * 86400000;
  ...
  const [gCur, gPrev, mCur, mPrev, campaigns] = await Promise.all([
    gOn ? runAdsWithMetrics({ since: new Date(curFrom).toISOString(), until: new Date(now).toISOString(), customerId: cid }).then((r) => r.ads).catch(() => []) : [],
    ...
    mOn ? fetchMetaAdsWithInsights({ since: ymd(curFrom), until: ymd(now) }).catch(() => []) : [],
  ...
  try {
    const [uDocs, tDocs] = await Promise.all([
      db.collection('leads').find({ created_at: { $gte: sinceIso } }, { projection: { _id: 0, status: 1 } }).toArray(),
      db.collection('tenant_leads').find({ created_at: { $gte: sinceIso } }, { projection: { _id: 0 } }).toArray(),
    ]);
    ...
    const wonWin = wonDocs.filter((x) => new Date(x.won_at || x.updated_at || x.created_at || 0).getTime() >= curFrom);
  ...
  return {
    period: { label: `siste ${d} dager`, days: d, from: new Date(curFrom).toISOString(), to: new Date(now).toISOString() },
    currency: 'NOK',
    ...
    efficiency: {
      cpl: newLeads > 0 ? round2(cur.cost / newLeads) : null,
