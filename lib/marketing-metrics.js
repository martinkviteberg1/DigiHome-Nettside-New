...
export async function buildMarketingMetrics(db, { days = 7, from, to, customerId } = {}) {
  ...
  try {
    const [uDocs, tDocs] = await Promise.all([
      db.collection('leads').find({ created_at: { $gte: sinceIso, $lte: untilIso } }, { projection: { _id: 0, status: 1 } }).toArray(),
      db.collection('tenant_leads').find({ created_at: { $gte: sinceIso, $lte: untilIso } }, { projection: { _id: 0 } }).toArray(),
    ]);
    ...
    const wonWin = wonDocs.filter((x) => { const t = new Date(x.won_at || x.updated_at || x.created_at || 0).getTime(); return t >= curFrom && t <= curTo; });
  ...
  return {
    ...
    efficiency: {
      ...
      // Lead→kunde-konvertering (markedsattribuert).
      leadToWonPct: newLeads > 0 ? round2((wonCount / newLeads) * 100) : null,
    },
    channelSplit: {
