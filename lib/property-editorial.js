...
//     Den setter bare den publiserte månedsleien, merket 'redaksjonell'. MRR,
//     LTV og realisert honorar er urørt — bare en faktisk startet leiekontrakt
//     gir realisert inntekt.
...
//  3. ADRESSE: gatefeltet deles — `area` (gruppering/filter/slug) blir gatenavnet
//     alene, mens annonsen viser full adresse med husnummer, slik eier har
//     bestemt. Eier, leietaker og eksakt kontraktsleie er fortsatt aldri med.
//
// REN LOGIKK — ingen DB, ingen import fra properties-sync (det ville gitt en
// sirkulær import). Kallende kode sender inn rentTextFromAmount.
// ---------------------------------------------------------------------------
import { BERGEN_DISTRICTS } from './geo-bergen';
import { toIsoDate, AVAILABLE_TEXTS, RENTAL_SCOPES } from './listings';
...
export const EDITORIAL_FIELDS = [
  ...
  { key: 'rentAmount', label: 'Månedsleie', kind: 'int', min: 1000, max: 200000, unit: 'kr/mnd', target: 'monthlyRentBand', hint: 'Publiseres som eksakt månedsleie på boligsiden og i nyhetsbrevet. Teller aldri i nøkkeltall.' },
  ...
  // UTLEIEENHET: kan boligen leies som hele enheten, som rom i bofellesskap,
  // eller begge? Plattformen har ikke feltet. Når «begge» er valgt må
  // interessenten krysse av for hva hun ser etter, slik at forvalteren vet hva
  // henvendelsen gjelder. Feltet påvirker aldri nøkkeltall.
  { key: 'rentalScope', label: 'Utleieenhet', kind: 'enum', options: RENTAL_SCOPES, target: 'rentalScope', hint: 'Ved «begge» må interessenten velge hele enheten eller rom når hun melder interesse. Ved kun rom vises ikke plattformens helbolig-leie — sett månedsleien per rom i Månedsleie.' },
  { key: 'roomsVacant', label: 'Ledige rom', kind: 'int', min: 1, max: 30, target: 'roomsVacant', hint: 'Vises som «2 av 4 rom ledige». Bare relevant ved rom i bofellesskap.' },
  { key: 'roomsTotal', label: 'Rom totalt i bofellesskapet', kind: 'int', min: 1, max: 30, target: 'roomsTotal' },
  ...
  { key: 'area', label: 'Gate', kind: 'street', max: 80, target: 'area', hint: 'Skriv full adresse med husnummer — nummeret vises i annonsen, mens filtre og gruppering bruker gatenavnet.' },
...
// Brukes til `area`, som er gruppe-/filter-/slug-nøkkelen — der hører et
// husnummer ikke hjemme. Selve annonsen viser full adresse (se houseNumberOf).
...
// Husnummeret alene: «Baglergaten 8B» → «8B». Tomt hvis redaktøren bare skrev
// gatenavnet.
export function houseNumberOf(s) {
  const m = /\s(\d+\s*[A-Za-zÆØÅæøå]?)\s*$/.exec(String(s || '').replace(/\s+/g, ' ').replace(/[,;].*$/, ''));
...
function coerce(f, raw) {
  switch (f.kind) {
    ...
    case 'street': {
      // Vi lagrer det redaktøren faktisk skrev — inkludert husnummer. Nummeret
      // trengs i annonsen; det er `area` (gruppering/slug) som skal være uten.
      const v = String(raw).replace(/\s+/g, ' ').replace(/[,;].*$/, '').trim().slice(0, f.max);
      return v || null;
    }
    default: {
...
export function normalizeEditorial(patch = {}, existing = {}) {
  ...
  // UTLEIEENHET — tverrfelt-regler. To feilkilder vi lukker her, fordi de
  // ellers ender som en selvmotsigende annonse:
  //  · «3 av 2 rom ledige» er ikke en skrivefeil leseren tilgir.
  //  · Leies HELE enheten ut, er romtellingen irrelevant og misvisende. Da
  //    fjerner vi den i stedet for å nekte redaktøren å lagre — hun har nettopp
  //    fortalt oss at boligen ikke leies ut rom for rom.
  if (out.rentalScope === 'hele') {
    for (const k of ['roomsVacant', 'roomsTotal']) {
      if (out[k] !== undefined) { delete out[k]; if (!cleared.includes(k)) cleared.push(k); }
    }
  }
  if (out.roomsVacant != null && out.roomsTotal != null && Number(out.roomsVacant) > Number(out.roomsTotal)) {
    return { error: 'Ledige rom kan ikke være flere enn rom totalt i bofellesskapet', field: 'roomsVacant' };
  }
...
export function applyEditorial(out, ed, { rentTextFromAmount } = {}) {
  const applied = [];
  const values = {};
  ...
  out.platformValues = {
    ...
    // Plattformen har ikke utleieenhet ennå — feltet ligger her slik at admin
    // viser «—» fra plattformen, og slik at plattformen vinner automatisk hvis
    // den en dag begynner å sende det.
    rentalScope: out.rentalScope ?? null,
    roomsVacant: out.roomsVacant ?? null,
    roomsTotal: out.roomsTotal ?? null,
  };
  ...
  for (const f of EDITORIAL_FIELDS) {
    ...
    if (f.key === 'rentAmount') {
      const text = typeof rentTextFromAmount === 'function' ? rentTextFromAmount(v) : null;
      if (text) {
        out.monthlyRentBand = text;
        out.rentBandSource = RENT_SOURCE_EDITORIAL;
        // Sporbarhet: tallet er DigiHomes egen opplysning, ikke utleiemodulens.
        // Flagget følger boligen inn i admin — men publiseres som helt vanlig
        // månedsleie, fordi det er nettopp det den er for en ledig bolig.
    ...
    // GATE: `area` er gruppe-/filter-/slug-nøkkelen og skal være gatenavnet
    // alene, mens annonsen viser full adresse. Skriver redaktøren «Baglergaten
    // 8B», splitter vi: area = «Baglergaten», street = «Baglergaten 8B».
    if (f.key === 'area') {
      const hn = houseNumberOf(v);
      const name = stripHouseNumber(v) || v;
      out.area = name;
      out.areaSource = RENT_SOURCE_EDITORIAL;
      if (hn) {
        out.street = `${name} ${hn}`;
        out.houseNumber = hn;
      }
