  ...
  if (!force) {
    try {
      ...
      if (cached) {
        ...
        // Stale-while-revalidate: server den gamle rapporten UMIDDELBART og
        // forny i bakgrunnen. SSB-tall endres årlig — 27s synkron ventetid for
        // første besøkende etter TTL-utløp er uakseptabelt (estimatet vises nå
        // på kvitteringssiden i skjemaet).
        try {
          buildReport(database, citySlug, { withAi: false }).catch(() => {});
        } catch (e) { /* bakgrunnsfornyelse er best-effort */ }
        return clean(cached);
      }
