...
async function sendSignaturEpost(jobb, signatar) {
  const { sendHtmlEmail, emailConfigured } = await import('./email');
  if (!emailConfigured() || !signatar.epost) return false;
  const base = (process.env.NEXT_PUBLIC_BASE_URL || '').replace(/\/$/, '');
  const lenke = `${base}/api/signer/${jobb.id}/${signatar.sid}`;
  const frist = new Date(Date.now() + (jobb.dagerFrist || 10) * 86400000)
    .toLocaleDateString('nb-NO', { day: 'numeric', month: 'long', year: 'numeric' });
  const hilsenNavn = signatar.navn ? signatar.navn.split(' ')[0] : '';
  const html = `
  <div style="margin:0;padding:32px 16px;background:#f6f5f2;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;">
    <div style="max-width:520px;margin:0 auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid rgba(0,0,0,0.06);">
      <div style="padding:28px 32px 0;">
        <p style="margin:0;font-size:15px;font-weight:700;letter-spacing:-0.01em;color:#0a0a0a;">DigiHome</p>
      </div>
      <div style="padding:24px 32px 8px;">
        <p style="margin:0 0 6px;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:0.08em;color:#8b5cf6;">Til signering</p>
        <h1 style="margin:0 0 14px;font-size:21px;line-height:1.3;letter-spacing:-0.01em;color:#0a0a0a;">${xesc(jobb.tittel)}</h1>
        <p style="margin:0 0 4px;font-size:14px;line-height:1.6;color:#555;">Hei${hilsenNavn ? ` ${xesc(hilsenNavn)}` : ''},</p>
        <p style="margin:0 0 18px;font-size:14px;line-height:1.6;color:#555;">${jobb.melding ? xesc(jobb.melding) : 'Du er bedt om å signere dokumentet over elektronisk.'}</p>
        <a href="${lenke}" style="display:inline-block;background:#0a0a0a;color:#ffffff;text-decoration:none;font-size:14.5px;font-weight:600;padding:13px 26px;border-radius:999px;">Åpne og signer med BankID&nbsp;&nbsp;→</a>
        <p style="margin:16px 0 0;font-size:12.5px;color:#999;">Frist: <strong style="color:#555;">${frist}</strong></p>
      </div>
      <div style="padding:20px 32px 26px;">
        <div style="border-top:1px solid rgba(0,0,0,0.06);padding-top:16px;">
          <p style="margin:0 0 6px;font-size:11.5px;line-height:1.6;color:#aaa;">Signeringen utføres trygt hos <strong style="color:#888;">Posten signering</strong> med BankID. Fungerer ikke knappen? Kopier denne lenken inn i nettleseren:</p>
          <p style="margin:0;font-size:11px;word-break:break-all;color:#8b5cf6;">${lenke}</p>
        </div>
      </div>
    </div>
    <p style="max-width:520px;margin:14px auto 0;font-size:11px;color:#b5b5b5;text-align:center;">Sendt av DigiHome (Digihome Tech AS) · Denne lenken er personlig — ikke videresend e-posten</p>
  </div>`;
  try {
...
export async function opprettSigneringsjobb(db, { filId, tittel, melding, signatarer, dagerFrist, av }) {
