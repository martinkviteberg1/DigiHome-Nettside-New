...
export async function pollSignering(db, { maks = 15 } = {}) {
  ...
  for (const flow of flows) {
    for (let i = 0; i < maks; i += 1) {
      ...
      // KRITISK: respekter Postens poll-vindu OGSÅ midt i loopen. Å polle
      // igjen umiddelbart etter en hendelse gir 429 med ESKALERENDE straffe-
      // vinduer — da blir neste signaturs hendelse liggende ufanget i timevis.
      if (nesteHeader && new Date(nesteHeader).getTime() > Date.now() + 1500) break;
    }
  }
...
async function behandleDirectStatus(db, mat, xml) {
  ...
  if (jobb) {
    ...
    // jobber, rå sid for eldre. Posisjonsfallback KUN når attributtet mangler
    ...
    statuser.forEach(([, attrs, verdi], idx) => {
      ...
      let mål = sid ? oppdaterte.find((s) => s.postenId === sid || s.sid === sid) : null;
