...
// KRITISK: Preview og produksjon MÅ bruke ulike polling-køer hos Posten.
// Køen er DESTRUKTIV — hvis to miljøer deler kø, stjeler de hverandres
// statushendelser (miljøet som poller først bekrefter og sletter hendelsen).
// Produksjon (digihome.no) beholder basisnavnet; alle andre miljøer får
// «-preview»-suffiks. Kan overstyres eksplisitt med POSTEN_ENV=prod|preview.
const KO = () => {
  const base = (process.env.POSTEN_POLLING_QUEUE || 'digihome-saker').trim();
  const envOverstyr = (process.env.POSTEN_ENV || '').toLowerCase().trim();
  if (envOverstyr === 'prod') return base;
  if (envOverstyr === 'preview') return `${base}-preview`;
  const erProd = /\/\/(www\.)?digihome\.no/i.test(process.env.NEXT_PUBLIC_BASE_URL || '');
  return erProd ? base : `${base}-preview`;
};
const BASE = () => `https://api.signering.posten.no/api/${ORGNR()}`;
// Eksponert for diagnose-endepunktet (viser hvilken kø DETTE miljøet poller)
export const koNavn = () => KO();
...
async function hentMateriale(db) {
  ...
  // Kønavn bestemmes ALLTID av KO() (miljøskilt) — aldri av en lagret verdi,
  // slik at preview/prod aldri kan kollidere selv om config-dokumentet er delt.
  ko = KO();
...
// Motsatsen: attributtverdier fra Postens XML kommer XML-escapet — de må
// unescapes før sammenligning med våre lagrede identifikatorer.
const xunesc = (s) => String(s ?? '')
...
// Finn signataren et signer-attributt peker på. Robust i tre lag:
// (1) eksakt match på postenId/sid, (2) match etter XML-unescaping,
// (3) suffiks-fallback: postenId slutter alltid på «· <6 tegn av sid>» —
// matcher vi suffikset entydig mot én signatar, er koblingen trygg selv om
// Posten skulle normalisere navnedelen av identifikatoren.
export function finnSignatarForAttr(signatarer, attr) {
  const raa = String(attr || '');
  if (!raa) return null;
  let treff = signatarer.find((s) => s.postenId === raa || s.sid === raa);
  if (treff) return treff;
  const ren = xunesc(raa).trim();
  treff = signatarer.find((s) => s.postenId === ren || s.sid === ren);
  if (treff) return treff;
  const sufM = ren.match(/·\s*([0-9a-fA-F]{6})\s*$/);
  if (sufM) {
    const kandidater = signatarer.filter((s) => String(s.sid || '').toLowerCase().startsWith(sufM[1].toLowerCase()));
...
// Manifest for direkteflyt: signatarer identifiseres med NAVN + kort suffiks
// (signer-identifier) — lesbart i Postens avsenderportal og i status-
// responser. Ingen fødselsnummer sendes til Posten på forhånd.
export function byggDirectManifest({ signatarer, tittel, melding }) {
  const signerXml = signatarer.map((s) => `<signer><signer-identifier>${xesc(s.postenId || s.sid)}</signer-identifier></signer>`).join('');
  return '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
...
// ── Batch: ÉN samle-e-post per signatar med alle dokumentene ─────────────────
// Samme rolige BankID-design som enkelt-e-posten, men med en dokumentliste der
// hver rad har egen personlig signeringslenke, og hoved-CTA til det første
// dokumentet. Signeringssiden viser «Neste dokument →» etter hvert dokument,
// så signataren tar hele bunken i ett strekk.
async function sendBatchSignaturEpost(signatar, jobber) {
  const { sendHtmlEmail, emailConfigured } = await import('./email');
  if (!emailConfigured() || !signatar.epost || !jobber.length) return false;
  const base = (process.env.NEXT_PUBLIC_BASE_URL || '').replace(/\/$/, '');
  const foerste = jobber[0];
  const frist = new Date(new Date(foerste.jobb.opprettet || Date.now()).getTime() + (foerste.jobb.dagerFrist || 10) * 86400000)
    .toLocaleDateString('nb-NO', { day: 'numeric', month: 'long', year: 'numeric' });
  const hilsenNavn = signatar.navn ? signatar.navn.split(' ')[0] : '';
  const antall = jobber.length;
  const F = "-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif";
  const hovedLenke = `${base}/signering/dokument/${foerste.jobb.id}/${foerste.sid}`;
  const rader = jobber.map((j, i) => {
    const lenke = `${base}/signering/dokument/${j.jobb.id}/${j.sid}`;
    return `<tr>
      <td width="34" valign="middle" style="padding:11px 0 11px 16px;${i ? 'border-top:1px solid #f0eee9;' : ''}">
        <table role="presentation" cellpadding="0" cellspacing="0"><tr>
          <td width="24" height="24" align="center" style="background:#f3eefe;border-radius:7px;font-family:${F};font-size:11px;font-weight:800;color:#7c3aed;">${i + 1}</td>
        </tr></table>
      </td>
      <td valign="middle" style="padding:11px 10px 11px 10px;${i ? 'border-top:1px solid #f0eee9;' : ''}font-family:${F};font-size:13.5px;font-weight:600;color:#1c1917;line-height:1.4;">${xesc(j.jobb.tittel || j.jobb.filNavn || 'Dokument')}</td>
      <td valign="middle" align="right" style="padding:11px 16px 11px 0;${i ? 'border-top:1px solid #f0eee9;' : ''}white-space:nowrap;">
        <a href="${lenke}" style="display:inline-block;font-family:${F};font-size:12px;font-weight:700;color:#7c3aed;text-decoration:none;background:#f3eefe;padding:7px 13px;border-radius:999px;">Signer&nbsp;&rarr;</a>
      </td>
    </tr>`;
  }).join('');
  const html = `<!DOCTYPE html>
<html lang="nb"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light"><meta name="supported-color-schemes" content="light"><title>Til signering</title></head>
<body style="margin:0;padding:0;background:#f4f3f0;-webkit-text-size-adjust:100%;">
  <div style="display:none;max-height:0;overflow:hidden;mso-hide:all;">${antall} dokumenter til BankID-signering — frist ${frist}.&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;&nbsp;&zwnj;</div>
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#f4f3f0;">
    <tr><td align="center" style="padding:40px 16px 16px;">
      <table role="presentation" width="560" cellpadding="0" cellspacing="0" style="width:100%;max-width:560px;">
        <tr><td style="padding:0 6px 14px;">
        ...
        <tr><td style="background:#ffffff;border:1px solid #e9e7e2;border-radius:20px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
            <tr><td style="padding:34px 36px 0;">
              <p style="margin:0;font-family:${F};font-size:11px;font-weight:700;letter-spacing:0.12em;color:#7c3aed;">TIL SIGNERING · ${antall} DOKUMENTER</p>
              <h1 style="margin:10px 0 0;font-family:${F};font-size:25px;line-height:1.25;letter-spacing:-0.02em;font-weight:700;color:#0a0a0a;">${antall} dokumenter venter på din signatur</h1>
              <p style="margin:16px 0 0;font-family:${F};font-size:14.5px;line-height:1.65;color:#57534e;">Hei${hilsenNavn ? ` ${xesc(hilsenNavn)}` : ''} — ${foerste.jobb.melding ? xesc(foerste.jobb.melding) : 'du er bedt om å signere dokumentene under elektronisk med BankID.'} Etter hvert dokument sendes du automatisk videre til neste.</p>
            </td></tr>
            <tr><td style="padding:22px 36px 0;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:#faf9f7;border:1px solid #efede8;border-radius:14px;">${rader}</table>
            </td></tr>
            <tr><td align="left" style="padding:24px 36px 0;">
              <table role="presentation" cellpadding="0" cellspacing="0"><tr>
                <td align="center" bgcolor="#0a0a0a" style="border-radius:999px;">
                  <a href="${hovedLenke}" style="display:inline-block;font-family:${F};font-size:15px;font-weight:600;color:#ffffff;text-decoration:none;padding:15px 30px;border-radius:999px;">Start signeringen (1 av ${antall})&nbsp;&nbsp;&rarr;</a>
                </td>
              </tr></table>
              <p style="margin:12px 0 0;font-family:${F};font-size:12px;line-height:1.6;color:#a8a29a;">Knappen åpner en sikker DigiHome-side der du ser hvert dokument i sin helhet før BankID-signeringen starter.</p>
            </td></tr>
            <tr><td style="padding:26px 36px 0;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-top:1px solid #f0eee9;">
                <tr>
            ...
            <tr><td style="padding:24px 36px 28px;">
              <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="border-top:1px solid #f0eee9;"><tr>
                <td style="padding:16px 0 0;font-family:${F};font-size:11.5px;line-height:1.65;color:#b5b1aa;">
                  Signeringen er juridisk bindende og utføres trygt hos <span style="color:#78716c;font-weight:600;">Posten signering</span> med <span style="color:#78716c;font-weight:600;">BankID</span>. Hvert dokument signeres for seg og får sin egen signerte PDF.
                </td>
              </tr></table>
            </td></tr>
          </table>
        ...
        <tr><td align="center" style="padding:18px 12px 8px;">
          <p style="margin:0;font-family:${F};font-size:11px;line-height:1.7;color:#b5b1aa;">Lenkene er personlige for deg — ikke videresend e-posten.<br>Digihome Tech AS · Org.nr 835 674 622</p>
        </td></tr>
      </table>
    </td></tr>
  </table>
  ...
  try {
    const r = await sendHtmlEmail({
      to: signatar.epost,
      subject: `Til signering: ${antall} dokumenter fra DigiHome`,
      html,
      fromName: 'DigiHome',
      categories: ['signering-batch'],
    });
    return !!(r && r.ok !== false);
  } catch (e) { return false; }
}

// Sender samle-e-poster for en batch: for hver unike signatar-e-post samles
// alle jobber i bunten der personen har tur og ennå ikke er varslet, og ÉN
// e-post sendes med alle lenkene. epostSendtAt markeres per jobb/signatar.
export async function sendBatchSignaturEposter(db, batchId) {
  const jobber = await db.collection(SIGN_JOBB_COLL)
    .find({ batchId: String(batchId || ''), status: 'I_GANG' }, { projection: { _id: 0 } })
    .sort({ opprettet: 1 })
    .toArray();
  if (!jobber.length) return { ok: true, sendt: 0 };
  const perEpost = new Map(); // epost → { signatar, jobber: [{jobb, sid}] }
  for (const jobb of jobber) {
    for (const s of signatarerPaaTur(jobb)) {
      if (!s.epost || s.epostSendtAt) continue;
      const nokkel = s.epost.toLowerCase();
      if (!perEpost.has(nokkel)) perEpost.set(nokkel, { signatar: s, jobber: [] });
      perEpost.get(nokkel).jobber.push({ jobb, sid: s.sid });
    }
  }
  let sendt = 0;
  for (const [, gruppe] of perEpost) {
    const ok = gruppe.jobber.length === 1
      ? await sendSignaturEpost(gruppe.jobber[0].jobb, { ...gruppe.signatar, sid: gruppe.jobber[0].sid })
      : await sendBatchSignaturEpost(gruppe.signatar, gruppe.jobber);
    if (ok) {
      sendt += 1;
      const naa = new Date().toISOString();
      for (const j of gruppe.jobber) {
        await db.collection(SIGN_JOBB_COLL).updateOne(
          { id: j.jobb.id, 'signatarer.sid': j.sid },
          { $set: { 'signatarer.$.epostSendtAt': naa } },
        );
      }
    }
  }
...
export async function opprettSigneringsjobb(db, { filId, tittel, melding, signatarer, dagerFrist, av, avId = null, autoArkiv = null, batchId = null, hoppOverEpost = false }) {
  ...
  const jobbId = uuidv4();
  // postenId = det Posten viser som «undertegner» i sin portal og bruker som
  // signer-attributt i statusresponser: NAVNET + kort suffiks (unik i jobben).
  const medSid = rene.map((s) => {
    const sid = uuidv4();
    const postenId = `${(s.navn || 'Signatar').slice(0, 80)} · ${sid.slice(0, 6)}`.trim();
    return { ...s, sid, postenId };
  });
  ...
  // Exit-URL-ene får jobb-hint slik at exit-siden kan hente full jobbstatus
  // med status_query_token-et Posten appender (offisiell mekanisme — gjør oss
  // uavhengige av polling-køens tidsvinduer for umiddelbar statusvisning).
  const metadataXml = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>'
    ...
    + '<exit-urls>'
    + `<completion-url>${xesc(`${baseUrl}/signering/ferdig?jobb=${jobbId}`)}</completion-url>`
    + `<rejection-url>${xesc(`${baseUrl}/signering/avvist?jobb=${jobbId}`)}</rejection-url>`
    + `<error-url>${xesc(`${baseUrl}/signering/feil?jobb=${jobbId}`)}</error-url>`
    + '</exit-urls>'
  ...
  for (const [, href, innhold] of signerBlokker) {
    ...
    const idVerdi = sidM ? sidM[1].trim() : '';
    const treff = idVerdi && medSid.find((s) => s.postenId === idVerdi || s.sid === idVerdi);
  ...
  // kun første ved sekvensiell rekkefølge). Ved batch-utsendelse hoppes dette
  // over — én samlet e-post per signatar sendes etterpå (sendBatchSignaturEposter).
  if (!hoppOverEpost) {
    for (const s of signatarerPaaTur(jobb)) {
...
export async function hentSignerVisning(db, jobbId, sid) {
  ...
  // Batch: finn neste dokument i samme bunt som VENTER på denne signataren
  // (match på e-post) — driver «Neste dokument →»-flyten på signeringssiden.
  let batch = null;
  if (jobb.batchId && signatar.epost) {
    try {
      const buntJobber = await db.collection(SIGN_JOBB_COLL)
        .find({ batchId: jobb.batchId }, { projection: { _id: 0, id: 1, tittel: 1, status: 1, signatarer: 1, opprettet: 1 } })
        .sort({ opprettet: 1 })
        .toArray();
      const mine = buntJobber
        .map((j) => ({ jobb: j, s: (j.signatarer || []).find((x) => x.epost && x.epost.toLowerCase() === signatar.epost.toLowerCase()) }))
        .filter((x) => x.s);
      const gjenstaar = mine.filter((x) => x.jobb.status === 'I_GANG' && x.s.status === 'VENTER' && x.jobb.id !== jobb.id
        && signatarerPaaTur(x.jobb).some((p) => p.sid === x.s.sid));
      batch = {
        antall: mine.length,
        ferdig: mine.filter((x) => x.s.status === 'SIGNERT').length,
        neste: gjenstaar.length
          ? { jobbId: gjenstaar[0].jobb.id, sid: gjenstaar[0].s.sid, tittel: gjenstaar[0].jobb.tittel || '' }
          : null,
      };
    } catch (e) { batch = null; }
  }
...
export async function pollSignering(db, { maks = 15 } = {}) {
  ...
  const mat = await hentMateriale(db);
  let hendelser = 0;
  // Poll begge køene til de er tomme — DIREKTE først (alle nye jobber er
  // direkteflyt; portal er kun eldre jobber).
  const flows = [];
  ...
  for (const flow of flows) {
    for (let i = 0; i < maks; i += 1) {
      ...
      // KRITISK: respekter Postens poll-vindu OGSÅ midt i loopen — for tidlig
      // polling gir 429 med ESKALERENDE straffevinduer. Etter en hendelse er
      // neste vindu normalt umiddelbart; er det et KORT vindu (≤8 s) venter vi
      // det ut og fortsetter å tømme køen i SAMME kjøring, slik at alle
      // hendelser fra en signeringsøkt (f.eks. en batch) tas i ett jafs.
      if (nesteHeader) {
        const ventMs = new Date(nesteHeader).getTime() - Date.now();
        if (ventMs > 1500) {
          if (ventMs <= 8000 && totalVentetMs + ventMs <= 20000) {
            totalVentetMs += ventMs;
            await new Promise((r) => { setTimeout(r, ventMs + 250); });
          } else break;
        }
      }
    }
...
// ── Token-basert statushenting (Postens offisielle exit-mekanisme) ───────────
// Posten appender status_query_token på exit-URL-en når en signatar er ferdig.
// Én GET mot jobbens status-url med tokenet gir status for ALLE signatarer —
// uavhengig av polling-køens tidsvinduer. Selvhelbredende: reparerer også
// jobber der en tidligere kø-hendelse gikk tapt. jobb-hintet fra exit-URL-en
// prøves først; ellers prøves de nyeste aktive direktejobbene (tokenet er kun
// gyldig for sin egen jobb, så feil jobb gir bare et avvist kall).
export async function hentStatusMedToken(db, { jobbId = null, token } = {}) {
  const t = String(token || '').trim();
  if (!t || t.length > 300 || !/^[\w.:~-]+$/.test(t)) return { ok: false, error: 'Ugyldig token' };
  const filter = { status: 'I_GANG', flyt: 'direkte' };
  const kandidater = [];
  if (jobbId) {
    const j = await db.collection(SIGN_JOBB_COLL).findOne({ ...filter, id: String(jobbId) });
    if (j) kandidater.push(j);
  }
  if (!kandidater.length) {
    kandidater.push(...await db.collection(SIGN_JOBB_COLL).find(filter).sort({ opprettet: -1 }).limit(5).toArray());
  }
  if (!kandidater.length) return { ok: true, oppdatert: false };
  const mat = await hentMateriale(db);
  for (const jobb of kandidater) {
    if (!jobb.statusUrl) continue;
    const skille = jobb.statusUrl.includes('?') ? '&' : '?';
    let res;
    try {
      res = await postenKall({
        metode: 'GET',
        url: `${jobb.statusUrl}${skille}status_query_token=${encodeURIComponent(t)}`,
        headers: { Accept: 'application/xml' },
        tls: mat.tls,
      });
    } catch (e) { continue; }
    if (res.status < 200 || res.status >= 300) continue; // tokenet gjelder ikke denne jobben
    // Lagre tokenet FØR behandling: det blir en varig «reservenøkkel» som
    // cron-avstemmingen kan bruke til å hente full jobbstatus direkte —
    // selv om kø-hendelser går tapt eller behandlingen under feiler nå.
    await db.collection(SIGN_JOBB_COLL).updateOne(
      { id: jobb.id },
      { $set: { sisteToken: t, sisteTokenAt: new Date().toISOString() } },
    ).catch(() => {});
    const xml = res.body.toString('utf8');
    try {
      await behandleDirectStatus(db, mat, xml);
      // Batch: frisk samtidig opp søsterjobber i samme bunt med DERES lagrede
      // tokens — selvhelbredende hvis en tidligere hendelse gikk tapt.
      if (jobb.batchId) {
        reconcileSigneringsjobber(db, { batchId: jobb.batchId, eldreEnnMs: 20000 }).catch(() => {});
      }
      return { ok: true, oppdatert: true, jobbId: jobb.id };
    } catch (e) {
      // eslint-disable-next-line no-console
      console.error('[signering] token-statushenting kunne ikke behandles:', e && e.message);
      return { ok: false, error: e && e.message };
    }
  }
  return { ok: true, oppdatert: false };
}

// ── Selvhelbredende statusavstemming via lagrede status_query_tokens ─────────
// Postens kø-hendelser kan forsinkes eller gå tapt (rate-vinduer, prosess-
// restart, ubekreftede hendelser). Hver gang en signatar returnerer via
// exit-URL lagres tokenet på jobben — her brukes det til å hente FULL
// jobbstatus direkte for aktive jobber som ikke er oppdatert nylig.
// Ugyldige/oppbrukte tokens fjernes stille (et nytt exit-besøk re-armerer).
export async function reconcileSigneringsjobber(db, { maks = 5, eldreEnnMs = 4 * 60000, batchId = null } = {}) {
  const filter = {
    status: 'I_GANG',
    flyt: 'direkte',
    sisteToken: { $type: 'string' },
    oppdatert: { $lt: new Date(Date.now() - Math.max(5000, eldreEnnMs)).toISOString() },
  };
  if (batchId) filter.batchId = String(batchId);
  const jobber = await db.collection(SIGN_JOBB_COLL).find(filter).sort({ oppdatert: 1 }).limit(maks).toArray();
  if (!jobber.length) return { ok: true, sjekket: 0, oppdatert: 0 };
  let mat;
  try { mat = await hentMateriale(db); } catch (e) { return { ok: false, error: e.message, sjekket: 0, oppdatert: 0 }; }
  let oppdatert = 0;
  for (const jobb of jobber) {
    if (!jobb.statusUrl || !jobb.sisteToken) continue;
    const skille = jobb.statusUrl.includes('?') ? '&' : '?';
    let res;
    try {
      res = await postenKall({
        metode: 'GET',
        url: `${jobb.statusUrl}${skille}status_query_token=${encodeURIComponent(jobb.sisteToken)}`,
        headers: { Accept: 'application/xml' },
        tls: mat.tls,
      });
    } catch (e) { continue; }
    if (res.status < 200 || res.status >= 300) {
      // Tokenet er oppbrukt/ugyldig — fjern det så vi ikke hamrer forgjeves.
      await db.collection(SIGN_JOBB_COLL).updateOne({ id: jobb.id }, { $unset: { sisteToken: '' } }).catch(() => {});
      continue;
    }
    try {
      ...
      // eslint-disable-next-line no-console
      console.error('[signering] token-avstemming kunne ikke behandles:', e && e.message);
    }
  }
  return { ok: true, sjekket: jobber.length, oppdatert };
}

// ── Felles etterbehandling ved fullført/stoppet runde ────────────────────────
// (1) In-app-varsler til avsender + sakseier/følgere. (2) Auto-arkivering av
// det signerte dokumentet — signerte dokumenter skal ALLTID være gjenfinnbare
// i Dokumenter-modulen. (3) E-post med signert PDF som vedlegg til alle
// parter. Feiler alltid stille — statusbehandlingen skal aldri veltes av
// etterarbeid.
async function etterStatusOppdatering(db, jobb, jobbStatus, now) {
  ...
  if (jobbStatus === 'FULLFORT') {
    // Auto-arkiv: alltid inn i dokumentarkivet slik at det signerte dokumentet
    // er gjenfinnbart i Dokumenter-modulen. Respekterer et eksisterende
    // arkivvalg (nedgraderer aldri synlighet noen har satt manuelt) — ellers
    // brukes rundens valg eller trygg standard (styret · Avtaler).
    try {
      const { settArkiv } = await import('./dokumenter');
      const filNaa = await db.collection('task_files').findOne({ id: jobb.filId }, { projection: { arkiv: 1 } });
      const alleredeArkivert = !!(filNaa && filNaa.arkiv && filNaa.arkiv.aktiv);
      if (!alleredeArkivert) {
        await settArkiv(db, jobb.filId, {
          aktiv: true,
          synlighet: (jobb.autoArkiv && jobb.autoArkiv.synlighet) || 'styret',
          kategori: (jobb.autoArkiv && jobb.autoArkiv.kategori) || 'Avtaler',
        }, 'Posten signering');
      }
    } catch (e) { /* stille */ }
    // E-post med det signerte dokumentet vedlagt til alle parter + avsender.
    try {
      await sendSignertDokumentEpost(db, jobb, now);
    } catch (e) {
      // eslint-disable-next-line no-console
      console.error('[signering] utsendelse av signert dokument feilet:', e && e.message);
    }
  }
}

// E-post til alle signatarer + avsender når alle har signert: moderne
// varsel (byggSakEpost) med den ferdig signerte PAdES-PDF-en som vedlegg.
// Idempotent via atomisk flagg på jobben — Postens kø kan levere samme
// hendelse flere ganger, men e-posten skal aldri gå dobbelt.
// (Eksportert for testbarhet.)
export async function sendSignertDokumentEpost(db, jobbFoer, now) {
  const { sendHtmlEmail, emailConfigured, byggSakEpost, sakChip } = await import('./email');
  if (!emailConfigured()) return;
  // Atomisk claim: kun første behandling sender e-posten.
  const claim = await db.collection(SIGN_JOBB_COLL).updateOne(
    { id: jobbFoer.id, signertEpostSendtAt: { $exists: false } },
    { $set: { signertEpostSendtAt: now } },
  );
  if (!claim.modifiedCount) return;
  // Fersk jobb (signertAt-tidspunkter) + fersk fil (den signerte versjonen).
  const jobb = (await db.collection(SIGN_JOBB_COLL).findOne({ id: jobbFoer.id })) || jobbFoer;
  const fil = await db.collection('task_files').findOne({ id: jobb.filId });
  if (!fil || !fil.data || !fil.laast) return; // signert versjon ikke lagret ennå
  const mottakere = new Map();
  for (const s of (jobb.signatarer || [])) if (s.epost) mottakere.set(String(s.epost).toLowerCase(), s.navn || '');
  let avsenderEpost = null;
  if (jobb.avId) {
    const av = await db.collection('admin_users').findOne({ id: jobb.avId }, { projection: { email: 1, name: 1 } });
    if (av && av.email) { avsenderEpost = av.email; mottakere.set(String(av.email).toLowerCase(), av.name || ''); }
  }
  if (!mottakere.size) return;
  const eskaper = (s) => String(s || '').replace(/[&<>"]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c]));
  const fmtTid = (iso) => { try { return new Date(iso).toLocaleString('nb-NO', { timeZone: 'Europe/Oslo', day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' }); } catch (e) { return ''; } };
  const rader = (jobb.signatarer || []).map((s, i) => `<table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" ${i ? 'style="border-top:1px solid #f0ecfa;"' : ''}><tr>
      <td width="86" valign="middle" style="padding:10px 0;">${sakChip('Signert', 'groenn')}</td>
      <td valign="middle" style="padding:10px 0;"><span style="color:#1c1917;font-size:13.5px;font-weight:700;">${eskaper(s.navn)}</span></td>
      <td valign="middle" align="right" style="padding:10px 0 10px 12px;white-space:nowrap;"><span style="color:#a6a19a;font-size:12px;">${s.signertAt ? eskaper(fmtTid(s.signertAt)) : ''}</span></td>
    </tr></table>`).join('');
  // Vedleggsgrense: SendGrid tåler ~30 MB totalt. Over ~18 MB binært dropper
  // vi vedlegget og henviser til avsenderen i stedet (skal normalt aldri skje).
  const vedleggOk = String(fil.data).length < 24_000_000;
  const { html, text } = byggSakEpost({
    aktorNavn: null,
    hendelse: 'signering fullført',
    tittel: jobb.tittel || fil.name || 'Dokument',
    intro: vedleggOk
      ? 'Alle parter har signert dokumentet med BankID. Det ferdig signerte dokumentet ligger vedlagt denne e-posten — lagre det gjerne for eget arkiv.'
      : 'Alle parter har signert dokumentet med BankID. Dokumentet er for stort til å vedlegges e-post — kontakt avsenderen for å få det tilsendt.',
    chips: [sakChip('Fullført — alle har signert', 'groenn'), sakChip(`${(jobb.signatarer || []).length} signatarer`, 'graa')],
    listeHtml: rader,
    ctaUrl: '',
    bunntekst: 'Signaturene er utført med BankID hos Posten signering og er innebygd i den vedlagte PDF-en (PAdES-format). Dokumentet kan valideres av bank, revisor og andre mottakere.',
    preheader: `Signert dokument${vedleggOk ? ' vedlagt' : ''}: ${jobb.tittel || fil.name}`,
    merkelapp: 'Signering',
  });
  await sendHtmlEmail({
    to: [...mottakere.keys()],
    subject: `Signert: ${jobb.tittel || fil.name}`,
    html,
    text,
    fromName: 'DigiHome Signering',
    replyTo: avsenderEpost || undefined,
    individual: true,
    categories: ['signering-fullfort'],
    attachments: vedleggOk ? [{ content: fil.data, filename: fil.name || 'signert.pdf', type: 'application/pdf' }] : [],
  });
  try {
    const { filLogg } = await import('./dokumenter');
    await filLogg(db, jobb.filId, `Signert dokument sendt på e-post til ${mottakere.size} ${mottakere.size === 1 ? 'mottaker' : 'mottakere'} (vedlagt PDF)`, 'Posten signering');
  } catch (e) { /* stille */ }
}
...
// (Eksportert for testbarhet.)
export async function behandleDirectStatus(db, mat, xml) {
  ...
  // Hendelse for en jobb dette miljøet IKKE kjenner (typisk: et annet miljø
  // deler kø ved feilkonfigurasjon). ALDRI bekreft — bekreftelse SLETTER
  // hendelsen permanent for miljøet som faktisk eier jobben. Uten bekreftelse
  // legger Posten den tilbake i køen etter ~10 min.
  if (!jobb) {
    // eslint-disable-next-line no-console
    console.warn(`[signering] hendelse for ukjent direkte-jobb (postenJobId=${postenJobId}, status=${jobbStatus}) — bekreftes IKKE, forblir i kø`);
    return;
  }

  // Monotont vern på jobbnivå: et forsinket/utdatert I_GANG-svar (kø-hendelser
  // og token-svar kan komme ut av rekkefølge) skal aldri rulle tilbake en jobb
  // som allerede er FULLFORT/FEILET lokalt. Hendelsen bekreftes likevel slik
  // at køen ryddes — vi har allerede nyere tilstand lagret.
  if ((jobb.status === 'FULLFORT' || jobb.status === 'FEILET') && jobbStatus === 'I_GANG') {
    if (confirmationUrl) {
      await postenKall({ metode: 'POST', url: confirmationUrl, headers: { 'Content-Length': '0' }, tls: mat.tls }).catch(() => {});
    }
    return;
  }
  ...
  if (jobb) {
    ...
    // <status signer="id" since="...">SIGNED</status> — signer-attributtet er
    // signer-identifieren fra manifestet: postenId (navn · suffiks) for nye
    // jobber, rå sid for eldre. Matching skjer robust (eksakt → XML-unescapet
    // → entydig sid-suffiks). Posisjonsfallback KUN når attributtet mangler
    // helt (én signatar) — aldri gjett på tvers av personer.
    ...
    statuser.forEach(([, attrs, verdi], idx) => {
      ...
      let mål = sid ? finnSignatarForAttr(oppdaterte, sid) : null;
      if (!mål && !sid && oppdaterte.length === 1) mål = oppdaterte[0];
      if (!mål && !sid && statuser.length === oppdaterte.length && oppdaterte[idx]) mål = oppdaterte[idx];
      if (mål) {
        const st = verdi.trim();
        const ny = SIGNATAR_STATUS[st] || st || mål.status;
        // Monotont vern per signatar: en SIGNERT signatar nedgraderes aldri
        // av et eldre statussvar som fortsatt sier VENTER.
        if (!(mål.status === 'SIGNERT' && ny === 'VENTER')) {
          mål.status = ny;
          if (st === 'SIGNED') mål.signertAt = siden || mål.signertAt || now;
        }
        mappet += 1;
      }
    });
    // VERN: fantes det statuser vi ikke klarte å knytte til noen signatar,
    // må hendelsen ALDRI bekreftes (bekreftelse sletter den permanent hos
    // Posten uten at noe ble lagret her). Kast → forblir i køen + full
    // diagnose i loggen slik at rotårsaken er synlig.
    if (statuser.length && !mappet) {
      // eslint-disable-next-line no-console
      console.error(`[signering] statusendring for jobb ${postenJobId} kunne ikke mappes til signatarer — signer-attr: ${statuser.map(([, a]) => (a.match(/signer="([^"]*)"/) || [])[1] || '(mangler)').join(', ')} | våre: ${oppdaterte.map((s) => `${s.postenId || ''}/${(s.sid || '').slice(0, 8)}`).join(', ')}`);
      throw new Error('Statusendring kunne ikke mappes til signatarer — bekreftes ikke');
    }
...
async function behandleStatusEndring(db, mat, xml) {
  ...
  // Ukjent jobb i dette miljøet → ALDRI bekreft (se behandleDirectStatus).
  if (!jobb) {
    // eslint-disable-next-line no-console
    console.warn(`[signering] hendelse for ukjent portal-jobb (ref=${referanse || postenJobId}) — bekreftes IKKE, forblir i kø`);
    return;
  }
...
export async function listSigneringsjobber(db, filId) {
  return db.collection(SIGN_JOBB_COLL)
    .find(filId ? { filId: String(filId) } : {}, { projection: { _id: 0, sisteToken: 0 } })
    .sort({ opprettet: -1 })
    .limit(30)
    .toArray();
}
