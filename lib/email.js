...
// ────────────────────────────────────────────────────────────────────────────
// Saker-e-post: samme moderne, e-postsikre design som chat-varslene.
// Varm off-white bakgrunn, hvitt kort med lilla aksentstrek, aktør-avatar,
// badge-chips for status/prioritet/frist, sitatboble med @-tagger som badges
// og tydelig CTA. Brukes av alle saks-, påminnelses- og digest-e-poster.
// ────────────────────────────────────────────────────────────────────────────
const SAK_CHIP_TONER = {
  lilla: ['#f2edfc', '#5b21b6'],
  graa: ['#f1efec', '#57534e'],
  groenn: ['#e5f5ec', '#0a7d55'],
  gul: ['#fdf3e0', '#b45309'],
  roed: ['#fdeaee', '#be123c'],
  bla: ['#e7f1fb', '#1d4ed8'],
};
export function sakChip(label, tone = 'graa') {
  const [bg, fg] = SAK_CHIP_TONER[tone] || SAK_CHIP_TONER.graa;
  return `<span style="display:inline-block;background-color:${bg};color:${fg};font-size:11.5px;font-weight:700;border-radius:99px;padding:5px 11px;margin:0 6px 6px 0;white-space:nowrap;">${eskaperHtml(label)}</span>`;
}

// @-tagger → lilla badges (lengste navn først slik at «Martin Kviteberg» ikke
// delvis matches av «Martin»). Forventer tekst/HTML der navnene står som ren
// tekst (escaped) — badger alle forekomster av «@Navn».
export function badgeMentionsHtml(htmlTekst, navnListe = []) {
  let ut = String(htmlTekst || '');
  const navn = [...new Set((navnListe || []).filter(Boolean))].sort((a, b) => b.length - a.length);
  for (const n of navn) {
    const moenster = eskaperHtml(n).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    ut = ut.replace(
      new RegExp(`@${moenster}`, 'gi'),
      `<span style="display:inline-block;background-color:#ede7fb;color:#5b21b6;font-weight:700;border-radius:6px;padding:0 6px;">@${eskaperHtml(n)}</span>`,
    );
  }
  return ut;
}

export function byggSakEpost({
  aktorNavn = null,        // personen bak hendelsen (null → DigiHome-avatar)
  hendelse = '',           // «tildelte deg saken», «kommenterte saken», …
  tittel = '',             // sakstittel / overskrift
  intro = null,            // valgfri grå setning under tittelen
  melding = null,          // sitat/kommentar (ren tekst) → boble (og tekstversjon)
  meldingHtml = null,      // ferdig-rendret markdown-HTML for boblen (overstyrer melding)
  mentions = [],           // navn som skal badges i melding/beskrivelse
  beskrivelseHtml = null,  // ferdig-rendret markdown-HTML (vises i dus boks)
  chips = [],              // ferdige chip-html-strenger fra sakChip()
  punkter = [],            // sjekklistepunkter [{tekst, fristTekst, forfalt}]
  listeHtml = null,        // rå html-blokk (digest-/påminnelseslister)
  ctaLabel = 'Åpne saken',
  ctaUrl = '',
  bunntekst = '',
  preheader = '',
  ...
  const harAktor = !!aktorNavn;
  const farge = harAktor ? epostAvatarFarge(aktorNavn) : '#141414';
  const initialer = harAktor ? epostInitialer(aktorNavn) : 'DH';
  let klokkeslett = '';
  try {
    klokkeslett = new Date(tidspunkt).toLocaleString('nb-NO', { timeZone: 'Europe/Oslo', weekday: 'long', hour: '2-digit', minute: '2-digit' });
    klokkeslett = klokkeslett.charAt(0).toUpperCase() + klokkeslett.slice(1);
  ...
  const chipsHtml = (chips || []).filter(Boolean).length ? `
          <tr><td colspan="3" style="padding-top:12px;">${chips.filter(Boolean).join('')}</td></tr>` : '';

  const introHtml = intro ? `
          <tr><td colspan="3" style="padding-top:10px;">
            <p style="margin:0;font-size:13px;line-height:1.6;color:#78716c;">${eskaperHtml(intro)}</p>
          </td></tr>` : '';

  let meldingBlokk = '';
  const bobleInnhold = meldingHtml
    ? badgeMentionsHtml(meldingHtml, mentions)
    : (melding ? badgeMentionsHtml(eskaperHtml(melding).replace(/\n/g, '<br/>'), mentions) : '');
  if (bobleInnhold) {
    meldingBlokk = `
          <tr><td colspan="3" style="padding-top:14px;">
            <div style="background-color:#f5f1fc;border-radius:4px 18px 18px 18px;padding:15px 18px;font-size:14.5px;line-height:1.6;color:#292524;">${bobleInnhold}</div>
          </td></tr>`;
  }

  const beskrivelseBlokk = beskrivelseHtml ? `
          <tr><td colspan="3" style="padding-top:14px;">
            <div style="background-color:#faf9f7;border:1px solid #efece7;border-radius:16px;padding:14px 18px 6px;">${badgeMentionsHtml(beskrivelseHtml, mentions)}</div>
          </td></tr>` : '';

  const punkterHtml = (punkter || []).length ? `
          <tr><td colspan="3" style="padding-top:14px;">
            ${punkter.slice(0, 12).map((p, i) => `
            <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" ${i ? 'style="border-top:1px solid #f0ecfa;"' : ''}><tr>
              <td width="26" valign="top" style="padding:10px 0;"><span style="display:inline-block;width:14px;height:14px;border:2px solid #d9d0f2;border-radius:99px;font-size:0;line-height:0;">&nbsp;</span></td>
              <td style="padding:9px 0;">
                <p style="margin:0;font-size:13.5px;font-weight:700;line-height:1.45;color:#1c1917;">${eskaperHtml(p.tekst)}</p>
                <p style="margin:2px 0 0;font-size:12px;font-weight:${p.fristTekst ? 700 : 400};color:${p.forfalt ? '#be123c' : (p.fristTekst ? '#b45309' : '#a6a19a')};">${p.fristTekst ? `Frist · ${eskaperHtml(p.fristTekst)}${p.forfalt ? ' · forfalt' : ''}` : 'Ingen frist'}</p>
              </td>
            </tr></table>`).join('')}
          </td></tr>` : '';

  const listeBlokk = listeHtml ? `
          <tr><td colspan="3" style="padding-top:14px;">${listeHtml}</td></tr>` : '';

  const ctaHtml = ctaUrl ? `
          <tr><td colspan="3" style="padding-top:22px;">
            <a href="${ctaUrl}" style="display:inline-block;background-color:#141414;color:#ffffff;text-decoration:none;font-size:13.5px;font-weight:700;padding:13px 26px;border-radius:12px;">${eskaperHtml(ctaLabel)}&nbsp;&nbsp;→</a>
          </td></tr>` : '';
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td align="center">
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="max-width:540px;width:100%;">
      <tr><td style="padding:0 8px 14px;">
        <span style="font-size:13.5px;font-weight:800;letter-spacing:0.02em;color:#1c1917;">DigiHome<span style="color:#7c3aed;">.</span></span>
        <span style="font-size:11.5px;color:#a6a19a;">&nbsp;·&nbsp;${eskaperHtml(merkelapp)}</span>
      </td></tr>
      <tr><td style="background-color:#ffffff;border-radius:20px;padding:24px 26px;box-shadow:0 1px 3px rgba(28,25,23,0.05),0 14px 44px -22px rgba(28,25,23,0.18);">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
          <tr><td colspan="3" style="padding-bottom:16px;">
          ...
          <tr>
            ...
            <td valign="middle">
              <p style="margin:0;font-size:15px;line-height:1.35;color:#1c1917;"><b>${eskaperHtml(aktorNavn || 'DigiHome')}</b> <span style="color:#8a857d;">${eskaperHtml(hendelse)}</span></p>
              <p style="margin:3px 0 0;font-size:11.5px;color:#b3ada3;">${klokkeslett}</p>
            </td>
            <td valign="middle" align="right" style="padding-left:10px;white-space:nowrap;">
              ${ctaUrl ? `<a href="${ctaUrl}" style="display:inline-block;background-color:#f2edfc;color:#5b21b6;text-decoration:none;font-size:12px;font-weight:700;padding:9px 14px;border-radius:11px;white-space:nowrap;">Åpne&nbsp;→</a>` : ''}
            </td>
          </tr>
          ${tittel ? `
          <tr><td colspan="3" style="padding-top:16px;">
            <p style="margin:0;font-size:17.5px;font-weight:800;line-height:1.35;color:#1c1917;">${eskaperHtml(tittel)}</p>
          </td></tr>` : ''}
          ${chipsHtml}
          ${introHtml}
          ${meldingBlokk}
      ...
      ${bunntekst ? `
      <tr><td style="padding:16px 10px 0;">
        <p style="margin:0;font-size:10.5px;line-height:1.6;color:#b8b2a9;">${eskaperHtml(bunntekst)}</p>
      </td></tr>` : ''}
    </table>
  </td></tr></table>
</div>`;

  const text = `${aktorNavn ? `${aktorNavn} ` : ''}${hendelse}${tittel ? `: ${tittel}` : ''}${intro ? ` — ${intro}` : ''}${melding ? ` — «${melding}»` : ''}${ctaUrl ? ` — Åpne: ${ctaUrl}` : ''}`;
  return { html, text };
}
