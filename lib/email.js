...
// Sender individuell HTML-epost til hver mottaker (personvern: ikke synlig CC).
export async function sendHtmlEmail({ to, subject, html, text, fromName = 'DigiHome', replyTo } = {}) {
  ensureInit();
  ...
  try {
    const out = await sgMail.sendMultiple(msg);
