...
export function byggChatEpost({
  ...
  if (!kropp && (vedlegg || []).length) kropp = '<span style="color:#78716c;">Delte vedlegg med deg</span>';
  ...
  <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr><td align="center">
    <table role="presentation" cellpadding="0" cellspacing="0" border="0" style="max-width:540px;width:100%;">
      <tr><td style="padding:0 8px 14px;">
        <span style="font-size:13.5px;font-weight:800;letter-spacing:0.02em;color:#1c1917;">DigiHome</span>
        <span style="font-size:11.5px;color:#a6a19a;">&nbsp;·&nbsp;Teamchat</span>
      </td></tr>
      <tr><td style="background-color:#ffffff;border-radius:20px;padding:26px 26px 24px;">
        <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0">
          <tr>
            ...
            <td valign="middle">
              <p style="margin:0;font-size:14.5px;line-height:1.35;color:#1c1917;"><b>${eskaperHtml(avsenderNavn)}</b> <span style="color:#8a857d;">nevnte deg${erTraad ? ' i en tråd' : ' i teamchatten'}</span></p>
              <p style="margin:3px 0 0;font-size:11.5px;color:#b3ada3;">${klokkeslett}</p>
            </td>
          </tr>
          ${traadHtml}
          <tr><td colspan="2" style="padding-top:14px;">
            <div style="background-color:#f5f1fc;border-radius:4px 16px 16px 16px;padding:14px 17px;font-size:14px;line-height:1.6;color:#292524;">${kropp}</div>
          </td></tr>
          ${vedleggHtml}
          <tr><td colspan="2" style="padding-top:22px;">
            <a href="${chatUrl}" style="display:inline-block;background-color:#141414;color:#ffffff;text-decoration:none;font-size:13.5px;font-weight:700;padding:12px 24px;border-radius:12px;">${erTraad ? 'Åpne tråden' : 'Åpne chatten'}&nbsp;&nbsp;→</a>
          </td></tr>
          <tr><td colspan="2" style="padding-top:14px;">
  ...
  const text = `${avsenderNavn} nevnte deg${erTraad ? ` i tråden${traadNavn ? ` «${traadNavn}»` : ''}` : ' i teamchatten'}: ${tekst || (vedlegg || []).map((v) => v.name).join(', ')}${chatUrl ? ` — Åpne: ${chatUrl}` : ''}`;
  return { html, text };
}
...
export async function sendHtmlEmail({ to, subject, html, text, fromName = 'DigiHome', replyTo, individual = true, headers, categories, sendAt, attachments, personlig = false } = {}) {
  ensureInit();
  ...
  // «Personlig» = person-til-person-varsel (f.eks. @-tag i chatten): skru av
  // klikk-/åpningssporing. SendGrids link-omskriving og sporingspiksel er
  // markedsføringssignaler som gjør at Outlook oftere legger e-posten i
  // «Annet» i stedet for «Prioritert» — og lenkene blir renere.
  if (personlig) {
