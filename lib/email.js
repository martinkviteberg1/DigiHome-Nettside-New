...
export function byggSakEpost({
  ...
  melding = null,          // sitat/kommentar (ren tekst) → lilla boble m/ @badges
  ...
  let meldingHtml = '';
  if (melding) {
    const kropp = badgeMentionsHtml(eskaperHtml(melding).replace(/\n/g, '<br/>'), mentions);
    meldingHtml = `
          <tr><td colspan="3" style="padding-top:14px;">
            <div style="background-color:#f5f1fc;border-radius:4px 18px 18px 18px;padding:15px 18px;font-size:14.5px;line-height:1.6;color:#292524;">${kropp}</div>
          </td></tr>`;
  }
