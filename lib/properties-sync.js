...
export function applyEnrichment(doc) {
  ...
  const out = { ...doc };
  delete out.enrich;
  if (!e || !e.finnUrl) return { ...out, enriched: false, enrichedFields: [] };
  ...
  if (!doc.monthlyRentBand && e.monthlyRentBand) { out.monthlyRentBand = e.monthlyRentBand; filled.push('leie'); }
  ...
  if (e.district && doc.districtSource !== 'plattform') {
...
export async function listPublicProperties(db, { limit = 12 } = {}) {
  ...
  return props.map((p) => {
    ...
    const { finnUrl, finnCode, finnStatus, finnCheckedAt, enriched, enrichedFields, districtSource, missingFields, incomplete, postalCode, ...safe } = e;
    return safe;
  });
}
