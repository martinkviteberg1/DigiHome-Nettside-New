...
export async function listPublicProperties(db, { limit = 12 } = {}) {
  ...
  return props.map((p) => {
    ...
    const { finnUrl, finnCode, finnStatus, finnCheckedAt, finnSource, hasUnitData, enriched, enrichedFields, districtSource, missingFields, incomplete, postalCode, ...safe } = e;
    return safe;
  });
}
