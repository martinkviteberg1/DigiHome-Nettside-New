...
import { applyEditorial, normalizeEditorial } from '@/lib/property-editorial';

// Norsk tusenskille uten locale-avhengige mellomrom (nb-NO gir U+202F i noen
// ICU-versjoner, og da matcher ikke intervallet plattformens egen tekst).
const nbNum = (n) => String(Math.round(Number(n) || 0)).replace(/\B(?=(\d{3})+(?!\d))/g, ' ');

// Plattformens utleiemodus → vårt språk. Deres 'hel'/'whole' og 'fleks'/'room'
// er en binær modell; «begge» finnes bare som redaksjonell overstyring hos oss.
const PLATFORM_SCOPE = { whole: 'hele', hel: 'hele', hele: 'hele', room: 'rom', rom: 'rom', fleks: 'rom', flex: 'rom' };

// EKSAKT MÅNEDSLEIE — det som publiseres på boligsiden og i nyhetsbrevet.
// 23 000 → «23 000 kr/mnd».
//
// Historikk (viktig, ikke reverser uten å lese dette): tidligere rundet vi
// beløpet ned i en 2 000-kroners bøtte («22 000–24 000 kr/mnd») for at et
// eksakt beløp ikke skulle kunne knyttes til en identifiserbar leieavtale. Den
// regelen gjelder KONTRAKTSLEIE på en utleid bolig — og en utleid bolig
// publiseres aldri her. En LEDIG bolig har verken leietaker eller kontrakt:
// beløpet er annonsert månedsleie, akkurat slik FINN og alle andre
// utleieannonser oppgir den. Intervallet gjorde derfor bare annonsen dårligere.
//
// Fortsatt urørt: eksakt kontraktsleie eksponeres aldri (utleide boliger er
// sperret i publiseringsporten), og en redaksjonell pris berører aldri
// `rentAmount` — nøkkeltallene leser det feltet.
export function rentTextFromAmount(amount) {
  const n = Number(amount);
  if (!Number.isFinite(n) || n <= 0) return null;
  return `${nbNum(n)} kr/mnd`;
}

// Prisintervall fra et beløp — samme 2 000-kroners bøtte som plattformens
// boligeksport bruker (16 000 → «16 000–18 000 kr/mnd»). BEHOLDT for å kunne
// tolke gamle data og plattformens egne intervaller, men den brukes ikke lenger
// til å publisere pris. Se rentTextFromAmount over.
export function rentBandFromAmount(amount, step = 2000) {
  const n = Number(amount);
  if (!Number.isFinite(n) || n <= 0) return null;
  const low = Math.floor(n / step) * step;
  return `${nbNum(low)}–${nbNum(low + step)} kr/mnd`;
}
...
function normalizeUnit(u, baseUrl) {
  ...
  return {
    ...
    // ANNONSEN FRA UTLEIEMODULEN. Da vi ba om dette 3. august var feltet tomt;
    // 4. august hadde 13 av 23 enheter i produksjon ekte annonsetittel og 10
    // hadde annonsetekst (source 'rental_listing'). Nå skal ikke forvalteren
    // skrive samme tekst to steder: annonsen hun lager i utleiemodulen er
    // kilden, og redaktøren overstyrer bare når hun vil noe annet.
    // HTML strippes — teksten skal kunne rendres trygt både på nett og i e-post.
    // Taggene erstattes med MELLOMROM, ikke med tomt: fjerner man <a>-taggen
    // uten mellomrom, blir «3D Visning</a>https://…» til «Visninghttps://…»,
    // og det er nøyaktig den feilen som står i en av annonsetekstene i dag.
    adTitle: u.ad && u.ad.title ? String(u.ad.title).replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 200) : null,
    adDescription: u.ad && u.ad.description
      ? String(u.ad.description).replace(/<\s*br\s*\/?>/gi, '\n').replace(/<\/\s*(p|div|li|h[1-6])\s*>/gi, '\n\n').replace(/<[^>]*>/g, ' ')
        .replace(/&nbsp;/gi, ' ').replace(/&amp;/gi, '&').replace(/&(?:quot|#34);/gi, '"').replace(/&(?:apos|#39);/gi, "'")
        .replace(/[ \t]{2,}/g, ' ').replace(/[ \t]+\n/g, '\n').replace(/\n{3,}/g, '\n\n').trim().slice(0, 4000)
      : null,
    adSource: u.ad && u.ad.source ? String(u.ad.source).slice(0, 40) : null,
    adUpdatedAt: u.ad && u.ad.updatedAt ? String(u.ad.updatedAt).slice(0, 40) : null,
    ...
    // KILDE TIL FINN-LENKEN. Plattformen sendte tidligere lenker som stammet fra
    // registreringen i et annet system, og én av dem pekte på feil bolig
    // (Nedre Gartnergaten). Nå merker plattformen lenken med opprinnelse:
    // 'manual_ad' = annonse opprettet i utleiemodulen = godkjent kilde.
    finnSourceRaw: u.finnSource ? String(u.finnSource).slice(0, 30) : null,
    ...
    // PUBLISERINGSKLARHET fra plattformen. Samme språk som utleiemodulen bruker,
    // så admin kan vise eieren nøyaktig hva som mangler i stedet for å gjette.
    readiness: u.readiness && typeof u.readiness === 'object' ? {
      hasOwner: u.readiness.hasOwner === true,
      hasPhotos: u.readiness.hasPhotos === true,
      hasArea: u.readiness.hasArea === true,
      hasRentPrice: u.readiness.hasRentPrice === true,
      hasActiveListing: u.readiness.hasActiveListing === true,
      readyToMarket: u.readiness.readyToMarket === true,
      blockers: Array.isArray(u.readiness.blockers) ? u.readiness.blockers.map((b) => String(b).slice(0, 40)).slice(0, 8) : [],
    } : null,
    // UTLEIEENHET fra plattformen. Deres modell er binær: en enhet leies ut som
    // HEL bolig (rental_mode 'hel' → 'whole') eller ROM FOR ROM ('fleks' →
    // 'room'). De har ingen «begge»-tilstand — den finnes bare som redaksjonell
    // overstyring hos oss, for boliger som faktisk annonseres på to måter.
    // Feltene ligger i deres preview og aktiveres her i samme øyeblikk de
    // publiserer; til da er de udefinerte og vi rører ikke verdien.
    rentalScope: PLATFORM_SCOPE[String(u.rentalScope || u.rentalMode || '').toLowerCase()] || null,
    roomsTotal: Number(u.roomsTotal) > 0 ? Number(u.roomsTotal) : null,
    // Ledige rom kan legitimt være 0 (fullt bofellesskap), så her må vi skille
    // mellom «null» og «ingen ledige».
    roomsVacant: u.roomsVacant == null || u.roomsVacant === '' ? null : Number(u.roomsVacant),
...
export function applyEnrichment(doc) {
  ...
  if (e && e.finnUrl) {
    ...
    // Plattformen sender FINN-lenken selv. `finnSourceRaw='manual_ad'` betyr at
    // annonsen er opprettet i utleiemodulen — den godkjente kilden vi ba om
    // etter at én plattformlenke pekte på feil bolig (Nedre Gartnergaten).
    ...
    out.finnOrigin = unit.finnSourceRaw || null;
    out.finnVerified = unit.finnSourceRaw === 'manual_ad';
  }
  if (out.finnSource === 'manuell') out.finnVerified = true;
  ...
  if (unit) {
    ...
    out.platformReadiness = unit.readiness || null;
    out.platformBlockers = (unit.readiness && unit.readiness.blockers) || [];
    // PUBLISERT MÅNEDSLEIE FRA UTLEIEMODULEN.
    // Boligeksporten sender bare et prisintervall, og bare på 5 av 23 boliger
    // (verifisert mot plattformens API). Enhetseksporten har selve beløpet
    // (rent.amount) på 13 av 23. Det eksakte beløpet VINNER derfor over både
    // intervallet fra boligeksporten og en pris hentet fra FINN — prisen på
    // boligkortet skal alltid stamme fra utleiemodulen når den finnes.
    // isEstimate=true betyr annonsert leie på en ledig bolig (en utleid bolig
    // har faktisk kontraktsleie). Kilden merkes, slik at nøkkeltallene aldri
    // forveksler annonsert leie med realisert inntekt.
    if (unit.rentAmount > 0) {
      out.monthlyRentBand = rentTextFromAmount(unit.rentAmount);
      out.rentBandSource = unit.rentIsEstimate ? 'plattform-estimat' : 'plattform-belop';
    }

    // UTLEIEENHET OG ROMKAPASITET fra utleiemodulen. Dette var tidligere noe
    // forvalteren måtte skrive inn manuelt i «Rediger boligdata» for hver bolig.
    // Nå kommer det fra kilden, og den redaksjonelle overstyringen (som kjører
    // etterpå) vinner fortsatt — blant annet for «begge», som plattformen ikke har.
    if (unit.rentalScope) out.rentalScope = unit.rentalScope;
    if (unit.roomsTotal != null) out.roomsTotal = unit.roomsTotal;
    if (unit.roomsVacant != null) out.roomsVacant = unit.roomsVacant;
  ...
  // REDAKSJONELLE OVERSTYRINGER — SIST, fordi redaktøren skal vinne over både
  // plattform og FINN. applyEditorial tar vare på plattformens egne verdier i
  // out.platformValues først, slik at admin kan se begge og nullstille per felt.
  // Merk: en redaksjonell pris rører ALDRI out.rentAmount (som nøkkeltallene
  // leser) — den setter bare den publiserte månedsleien og merkes 'redaksjonell'.
  applyEditorial(out, ed, { rentTextFromAmount });
...
export async function listPublicProperties(db, { limit = 12 } = {}) {
  ...
  const props = await db.collection(PROPERTIES_COLL)
    .find(
      ...
      { projection: { _id: 0, id: 1, title: 1, area: 1, city: 1, type: 1, bedrooms: 1, sqm: 1, images: 1, status: 1, model: 1, monthlyRentBand: 1, availableFrom: 1, district: 1, districtSource: 1, enrich: 1, editorial: 1 } }
    )
  ...
  return props.map((p) => {
    ...
    const {
      ...
      // Redaksjonelle adminfelt — må strippes eksplisitt, ellers lekker de her.
      editorialFields, editorialValues, editorialRentAmount, platformValues, imageRights,
      rentIndicativeEditorial, sqmSource, bedroomsSource, roomsSource, typeSource,
      modelSource, areaSource, availableFromSource, descriptionSource, titleSource,
      ...safe
    } = e;
    return safe;
  });
...
export async function setPropertyEditorialFields(db, { id, fields, resetAll = false }) {
  const key = String(id || '').slice(0, 80);
  if (!key) return { ok: false, error: 'Mangler id' };
  const filter = { $or: [{ id: key }, { externalId: key }] };
  const doc = await db.collection(PROPERTIES_COLL).findOne(filter, { projection: { _id: 0, externalId: 1, editorial: 1 } });
  if (!doc) return { ok: false, error: 'Fant ikke boligen' };

  if (resetAll) {
    await db.collection(PROPERTIES_COLL).updateOne({ externalId: doc.externalId }, { $unset: { editorial: '' } });
    const fresh0 = await db.collection(PROPERTIES_COLL).findOne({ externalId: doc.externalId }, { projection: { _id: 0 } });
    return { ok: true, cleared: true, changed: [], removed: Object.keys(doc.editorial || {}), property: applyEnrichment(fresh0) };
  }

  const res = normalizeEditorial(fields || {}, doc.editorial || {});
  if (res.error) return { ok: false, error: res.error, field: res.field };
  const next = res.editorial;
  const hasAny = Object.keys(next).length > 0;
  const update = hasAny
    ? { $set: { editorial: { ...next, updatedAt: new Date().toISOString() } } }
    : { $unset: { editorial: '' } };
  await db.collection(PROPERTIES_COLL).updateOne({ externalId: doc.externalId }, update);
  const fresh = await db.collection(PROPERTIES_COLL).findOne({ externalId: doc.externalId }, { projection: { _id: 0 } });
  return {
    ...
    changed: res.changed,
    removed: res.cleared,
    property: applyEnrichment(fresh),
  };
}

export async function setPropertyEditorialTitle(db, { id, title }) {
  // Delegerer til feltmotoren. Tidligere skrev denne HELE editorial-objektet,
  // som ville slettet pris/areal/annonsetekst hver gang noen lagret en tittel.
  const clean = String(title == null ? '' : title).replace(/\s+/g, ' ').trim().slice(0, 160);
  const res = await setPropertyEditorialFields(db, { id, fields: { title: clean || null } });
  if (!res.ok) return res;
  return { ok: true, cleared: !clean, property: res.property };
}

async function _legacySetPropertyEditorialTitle(db, { id, title }) {
  const key = String(id || '').slice(0, 80);
  if (!key) return { ok: false, error: 'Mangler id' };
  const filter = { $or: [{ id: key }, { externalId: key }] };
  const doc = await db.collection(PROPERTIES_COLL).findOne(filter, { projection: { _id: 0, externalId: 1 } });
