...
// Prisintervall fra et beløp — samme 2 000-kroners bøtte som plattformen selv
// bruker (16 000 → «16 000–18 000 kr/mnd»). Vi viser aldri et eksakt beløp
// utad: en enkelt leiepris kan knyttes til en identifiserbar leieavtale.
...
export function applyEnrichment(doc) {
  ...
  if (unit) {
    ...
    // PRISINTERVALL FRA PLATTFORMENS EGET BELØP.
    // Boligeksporten sender et ferdig intervall på bare 5 av 22 boliger, men
    // enhetseksporten har selve beløpet (rent.amount). Vi bøtter det i samme
    // 2 000-kroners intervall som plattformen bruker selv, slik at prisen på
    // boligkortet ALLTID stammer fra utleiemodulen — aldri fra FINN.
    // isEstimate=true er prisantydning på en ledig bolig (en utleid bolig har
    // faktisk kontraktsleie), og det merkes i kilden så KPI-ene aldri
    // forveksler prisantydning med realisert inntekt.
    if (!out.monthlyRentBand && unit.rentAmount > 0) {
      out.monthlyRentBand = rentBandFromAmount(unit.rentAmount);
  ...
  // leser) — den lager bare prisintervallet og merkes 'redaksjonell'.
  applyEditorial(out, ed, { bandFromAmount: rentBandFromAmount });
