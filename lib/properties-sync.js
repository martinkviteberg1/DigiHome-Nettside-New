...
import { resolveDistrict, fetchStreetDistrict, normStreet, isJunkArea } from '@/lib/geo-bergen';
...
// IDENTISKE på gate + by + type + soverom + areal + bildeantall + leiemodell.
// Da er det ikke lenger en tolkning, men samme bolig registrert to ganger.
// Nyeste rad beholdes; de eldre flagges. Vi sletter aldri noe.
// ---------------------------------------------------------------------------
function markDuplicates(docs = []) {
  ...
  for (const d of docs) {
    const key = [
      ...
      d.type || '', d.bedrooms ?? '', d.sqm ?? '', (d.images || []).length, d.model || '',
    ].join('|');
  ...
  for (const [, list] of groups) {
    ...
    list.sort((a, b) => String(b.platformUpdatedAt || '').localeCompare(String(a.platformUpdatedAt || '')));
...
export async function syncPropertiesFromPlatform(db, { target, key, maxPages = 10, pageLimit = 200 } = {}) {
  ...
  for (const raw of rows) {
    ...
    // Bydel: plattformen sender den ikke, så vi slår opp gatenavnet hos
    // Kartverket (cachet). Feiler oppslaget faller vi tilbake på poststed/by —
    // aldri en gjettet bydel.
  ...
  return { ok: true, synced: rows.length, upserted, updated, withDistrict, incompleteCount, duplicateCount, staleMarked: staleRes.modifiedCount || 0, partialError: lastError || null };
}
...
export async function refreshPropertyQuality(db) {
  try {
    const docs = await db.collection(PROPERTIES_COLL)
      .find({ stale: { $ne: true } }, { projection: { _id: 0, externalId: 1, area: 1, city: 1, type: 1, bedrooms: 1, sqm: 1, images: 1, model: 1, platformUpdatedAt: 1, incomplete: 1, duplicate: 1, missingFields: 1 } })
      .limit(500).toArray();
    ...
    for (const d of docs) {
      ...
      if (!d.sqm) missing.push('areal');
      if (!d.bedrooms) missing.push('soverom');
      if (!d.area) missing.push('gateadresse');
      d._missing = missing;
      d._incomplete = !imgs && !d.sqm && !d.bedrooms;
