...
export async function runOptimization(db, { mode = 'weekly', dryRun = true, customerId } = {}) {
  ...
  const alerts = buildAlerts({ googleAds, metaAds, current: aggregate([...googleAds, ...metaAds]), previous: null, config: cfg });
  ...
  const runDoc = {
    ...
    recommendations, alerts, keywordIdeas: keywordIdeas.slice(0, 100), report, summary, autoApplied,
  };
