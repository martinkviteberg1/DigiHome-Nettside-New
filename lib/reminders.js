...
export async function runDueReminders(dbArg = null, { dryRun = false } = {}) {
  ...
  // Hent alle personer én gang (id → {name,email}).
  const personer = await db.collection('admin_users')
    .find({}, { projection: { _id: 0, id: 1, name: 1, email: 1 } }).toArray();
  ...
  for (const t of saker) {
    ...
    if (t.dueDate === imorgen) {
      ...
      for (const mid of mottakere) {
        leggTil(mid, { kind: 'sak', key: `sak:${t.id}:${imorgen}:${mid}`, title: t.title, due: imorgen, priority: t.priority });
      }
    }
    ...
    for (const s of (Array.isArray(t.subtasks) ? t.subtasks : [])) {
      if (s && !s.done && s.due === imorgen && s.assigneeId) {
        ...
        leggTil(s.assigneeId, { kind: 'deloppgave', key: `sub:${t.id}:${s.id}:${imorgen}:${s.assigneeId}`, title: s.text, due: imorgen, taskTitle: t.title });
      }
    }
  }
