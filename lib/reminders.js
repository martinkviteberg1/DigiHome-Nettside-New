...
function byggEpost({ navn, items, base }) {
  const rad = (it) => {
    const merke = it.kind === 'deloppgave'
      ? '<span style="display:inline-block;font-size:11px;font-weight:700;color:#6d28d9;background:#ede9fe;border-radius:6px;padding:1px 7px;margin-right:6px">Sjekklistepunkt</span>'
    ...
    return `<tr><td style="padding:10px 0;border-top:1px solid #f0efec">
      <div style="font-size:14px;font-weight:600;line-height:1.45">${merke}<a href="${base}/admin/saker/${encodeURIComponent(it.taskId || '')}" style="color:#111;text-decoration:none">${esc(it.title)}</a>${pri}</div>
      ${kontekst}
  ...
  <div style="background:#f6f5f3;padding:32px 16px;font-family:-apple-system,'Segoe UI',Roboto,sans-serif">
    <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:16px;overflow:hidden;border:1px solid #eee">
      ...
      <div style="padding:26px 24px">
        ...
        <a href="${base}/admin/saker" style="display:inline-block;margin-top:20px;background:#0a0a0a;color:#fff;text-decoration:none;font-size:13.5px;font-weight:600;padding:11px 20px;border-radius:99px">Åpne Saker i admin →</a>
      </div>
    </div>
  </div>`;
}
