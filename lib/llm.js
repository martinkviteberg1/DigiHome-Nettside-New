...
import { logLlmUsage, resolveModelForFeature } from '@/lib/llm-usage';
...
export async function chatLLM({ messages, model = 'gpt-4o-mini', temperature = 0.3, maxTokens = 700, retries = 4, feature = 'general' }) {
  ...
  // Modelloverstyring per funksjon (settes fra admin-UI, 60s cache i llm-usage).
  let effectiveModel = model;
  try {
    const db = await getDb();
    effectiveModel = await resolveModelForFeature(db, feature, model);
  } catch (_) { /* stille — bruk oppgitt modell */ }

  // GPT-5- og o-seriene bruker max_completion_tokens og støtter ikke egendefinert temperature.
  const isReasoningStyle = /^(gpt-5|o\d)/i.test(effectiveModel);
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (content) {
            ...
            if (data.usage) {
              getDb().then((db) => logLlmUsage(db, { model: effectiveModel, feature, usage: data.usage, provider: prov.name })).catch(() => {});
            }
            return content;
          }
