...
import { rensModellDrivere, rensModellFakta, beregnInvestorModell } from './budsjett-modell';
import {
...
export async function hentPlan(db, id) {
  ...
  return {
    ...
    ...(doc.type === 'modell' ? { drivere: rensModellDrivere(doc.drivere), fakta: rensModellFakta(doc.fakta, N) } : {}),
...
export async function lagrePlan(db, body = {}) {
  ...
  if (type === 'modell') {
    ...
    if (!fakta) {
      const eks = await db.collection(BUDSJETT_COLL).findOne({ plan: true, id }, { projection: { _id: 0, fakta: 1 } });
      fakta = eks?.fakta || null;
    }
    set.drivere = rensModellDrivere(body.drivere);
    set.fakta = { ...rensModellFakta(fakta || {}, antallMnd), oppdatertAt: fakta?.oppdatertAt || now };
    const m = beregnInvestorModell({ antallMnd, fakta: set.fakta, drivere: set.drivere });
