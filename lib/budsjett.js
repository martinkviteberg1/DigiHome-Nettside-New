...
import { rensModellDrivere, rensModellFakta, rensScenarioer, beregnInvestorModell, rensPlattformDrivere, rensFelles, beregnPlattform, beregnKonsern, rensTechDrivere, rensTechFakta, beregnTech } from './budsjett-modell';

const normPlanSelskap = (v) => (v === 'tech' ? 'tech' : 'digihome');
import {
...
import { beregnSikretSerie, beregnBortfallSerie, beregnEnhetsSerie } from './portefolje-serier';
...
// Porteføljeserier (sikret honorar, bortfall, enheter) bor i lib/portefolje-serier.js — re-eksporteres her.
export { beregnSikretSerie, beregnBortfallSerie, beregnEnhetsSerie };
...
// selskap: 'digihome' | 'tech' | undefined (alle). Planer uten feltet er Digihome AS.
export async function listPlaner(db, { kunInvestorSynlige = false, selskap } = {}) {
  const filter = kunInvestorSynlige ? { plan: true, investorSynlig: true } : { plan: true };
  if (selskap === 'tech') filter.selskap = 'tech';
  else if (selskap === 'digihome') filter.selskap = { $ne: 'tech' };
  const docs = await db.collection(BUDSJETT_COLL)
    .find(filter, { projection: { _id: 0, id: 1, navn: 1, startYm: 1, antallMnd: 1, status: 1, type: 1, selskap: 1, kobletPlanId: 1, modellSammendrag: 1, inntekter: 1, kostnader: 1, investorSynlig: 1, updatedAt: 1, updatedBy: 1, drivere: 1, fakta: 1, tech: 1 } })
    .sort({ startYm: -1, navn: 1 })
    .toArray();
  // «Nå» i planen: inneværende måneds plantall — så listen kan møte faktisk fra Økonomi.
  const naaD = new Date(); const naaYm = `${naaD.getUTCFullYear()}-${String(naaD.getUTCMonth() + 1).padStart(2, '0')}`;
  const naaIdx = (d) => { const { y, m0 } = ymDeler(d.startYm); const i = (naaD.getUTCFullYear() * 12 + naaD.getUTCMonth()) - (y * 12 + m0); return i >= 0 && i < (Number(d.antallMnd) || 12) ? i : null; };
  const planNaa = (d) => {
    const i = naaIdx(d); if (i == null || d.type !== 'modell') return null;
    try {
      const N = Math.min(36, Math.max(1, Math.round(d.antallMnd) || 12));
      if (normPlanSelskap(d.selskap) === 'tech') {
        const m = beregnTech({ antallMnd: N, drivere: d.tech || {}, fakta: d.fakta || {} });
        return { ym: naaYm, idx: i, inntekt: m.inntekt.total[i] || 0, kost: m.kostSum[i] || 0, resultat: m.resultat[i] || 0, mrr: m.saas?.mrr?.[i] || 0 };
      }
      const m = beregnInvestorModell({ antallMnd: N, fakta: rensModellFakta(d.fakta || {}, N), drivere: rensModellDrivere(d.drivere || {}), startYm: d.startYm });
      return { ym: naaYm, idx: i, inntekt: m.inntekt[i] || 0, kost: m.kostSum[i] || 0, resultat: m.resultat[i] || 0 };
    } catch (e) { return null; }
  };
  return docs.map((d) => ({
    id: d.id,
    selskap: normPlanSelskap(d.selskap),
    kobletPlanId: d.kobletPlanId || null,
    tech: d.selskap === 'tech' ? (d.modellSammendrag?.tech || null) : null,
    naa: planNaa(d),
    navn: d.navn,
    startYm: d.startYm,
    ...
    konsern: d.type === 'modell' ? (d.modellSammendrag?.konsern || null) : null,
    updatedAt: d.updatedAt || null,
    updatedBy: d.updatedBy || null,
  }));
}
...
export async function hentPlan(db, id) {
  ...
  const selskap = normPlanSelskap(doc.selskap);
  if (selskap === 'tech') {
    return {
      id: doc.id,
      selskap: 'tech',
      ...
      type: 'modell',
      tech: rensTechDrivere(doc.tech || {}),
      fakta: rensTechFakta(doc.fakta || {}, N),
      kobletPlanId: doc.kobletPlanId || null,
      notat: String(doc.notat || ''),
  ...
  return {
    ...
    ...(doc.type === 'modell' ? {
      ...
      // Konsern: plattform-segment + felleskostnader (null = ikke tatt i bruk ennå)
      plattform: doc.plattform ? rensPlattformDrivere(doc.plattform) : null,
      felles: doc.felles ? rensFelles(doc.felles) : null,
    } : {}),
...
export async function lagrePlan(db, body = {}) {
  ...
  // Selskap: låst etter opprettelse (et Tech-budsjett blir ikke et Digihome AS-budsjett).
  const eksSel = await db.collection(BUDSJETT_COLL).findOne({ plan: true, id }, { projection: { _id: 0, selskap: 1 } });
  const selskap = eksSel ? normPlanSelskap(eksSel.selskap) : normPlanSelskap(body.selskap);
  if (selskap === 'tech') return lagreTechPlan(db, { id, navn, startYm, antallMnd, status, now, body });
  ...
  if (type === 'modell') {
    ...
    const eks = await db.collection(BUDSJETT_COLL).findOne({ plan: true, id }, { projection: { _id: 0, id: 1, fakta: 1, drivere: 1, plattform: 1, felles: 1 } });
    ...
    // Utelatte drivere på en EKSISTERENDE plan betyr «behold» — aldri tilbake til standard.
    let drivereInn = body.drivere;
    if (eks && drivereInn === undefined) drivereInn = eks.drivere || {};
    if (!eks) {
      try {
    ...
    // Konsern-lagene: sendes bare når klienten redigerer dem; utelatt = behold. null = slå av.
    if (body.plattform !== undefined) set.plattform = body.plattform ? rensPlattformDrivere(body.plattform) : null;
    else if (eks?.plattform) set.plattform = rensPlattformDrivere(eks.plattform);
    if (body.felles !== undefined) set.felles = body.felles ? rensFelles(body.felles) : null;
    else if (eks?.felles) set.felles = rensFelles(eks.felles);
    ...
    if (set.plattform) {
      const pl = beregnPlattform({ antallMnd, drivere: set.plattform });
      const ko = beregnKonsern({ forvaltning: m, plattform: pl, felles: set.felles || {}, antallMnd });
      set.modellSammendrag.konsern = {
        sumInntekt: ko.sammendrag.sumInntekt, sumKost: ko.sammendrag.sumKost, resultat: ko.sammendrag.resultat,
        breakEvenIdx: ko.sammendrag.breakEvenIdx, kapitalbehov: ko.sammendrag.kapitalbehov, arrExit: ko.sammendrag.arrExit,
        andelPlattformPct: ko.sammendrag.andelPlattformPct,
      };
    }
  }
  await db.collection(BUDSJETT_COLL).updateOne(
  ...
  // Styringsregel: kun ÉN vedtatt plan per selskap og identiske periode — andre demoteres.
  if (status === 'vedtatt') await demoterAndreVedtatte(db, { id, startYm, antallMnd, selskap: 'digihome', now });
  return { ok: true, id, ...set };
}

async function demoterAndreVedtatte(db, { id, startYm, antallMnd, selskap, now }) {
  const selFilter = selskap === 'tech' ? { selskap: 'tech' } : { selskap: { $ne: 'tech' } };
  await db.collection(BUDSJETT_COLL).updateMany(
    { plan: true, ...selFilter, startYm, antallMnd, id: { $ne: id }, status: 'vedtatt' },
    { $set: { status: 'utkast', updatedAt: now } },
  );
}

// ── Digihome Tech AS — plattformbudsjett ─────────────────────────────────────
// tech (drivere) og fakta (enheter under forvaltning) sendes bare når klienten
// endrer dem; utelatt = behold. kobletPlanId peker på Digihome AS-budsjettet
// som gir lisensvolumet.
async function lagreTechPlan(db, { id, navn, startYm, antallMnd, status, now, body }) {
  const eks = await db.collection(BUDSJETT_COLL).findOne({ plan: true, id }, { projection: { _id: 0, tech: 1, fakta: 1, kobletPlanId: 1 } });
  const tech = rensTechDrivere(body.tech !== undefined ? body.tech : (eks?.tech || {}));
  let faktaInn = body.fakta !== undefined ? body.fakta : eks?.fakta;
  const kobletPlanId = body.kobletPlanId !== undefined ? (body.kobletPlanId ? String(body.kobletPlanId) : null) : (eks?.kobletPlanId || null);
  // Uten fakta men med kobling: hent enhetsserien fra Digihome AS-budsjettet nå.
  if (!faktaInn && kobletPlanId) { try { faktaInn = await lagTechFakta(db, { startYm, antallMnd, kobletPlanId }); } catch (e) { faktaInn = null; } }
  const fakta = { ...rensTechFakta(faktaInn || {}, antallMnd), oppdatertAt: faktaInn?.oppdatertAt || now };
  const m = beregnTech({ antallMnd, drivere: tech, fakta });
  const set = {
    plan: true,
    selskap: 'tech',
    type: 'modell',
    navn, startYm, antallMnd, status,
    investorSynlig: Boolean(body.investorSynlig),
    tech, fakta, kobletPlanId,
    notat: String(body.notat || '').slice(0, 2000),
    modellSammendrag: {
      sumInntekt: m.sammendrag.sumInntekt, sumKost: m.sammendrag.sumKost, resultat: m.sammendrag.resultat, breakEvenIdx: m.sammendrag.breakEvenIdx,
      tech: { andelForvaltningPct: m.sammendrag.andelForvaltningPct, andelHuseierPct: m.sammendrag.andelHuseierPct, andelBedriftPct: m.sammendrag.andelBedriftPct, kapitalbehov: m.sammendrag.kapitalbehov, arrExit: m.sammendrag.arrExit, enheterVedSlutt: m.sammendrag.enheterVedSlutt },
    },
    updatedAt: now,
    updatedBy: String(body.updatedBy || '').slice(0, 120),
  };
  await db.collection(BUDSJETT_COLL).updateOne({ plan: true, id }, { $set: set, $setOnInsert: { id, createdAt: now } }, { upsert: true });
  if (status === 'vedtatt') await demoterAndreVedtatte(db, { id, startYm, antallMnd, selskap: 'tech', now });
  return { ok: true, id, ...set };
}

// Enheter under forvaltning per måned for et Tech-budsjett:
//   · fra et Digihome AS-budsjett (planens modellerte enhetsserie, forskjøvet til
//     Tech-periodens måneder; utenfor planens periode holdes første/siste verdi)
//   · ellers fra dagens portefølje (kontraktsfestede enheter, uten vekst)
export async function lagTechFakta(db, { startYm, antallMnd, kobletPlanId = null, rows = null }) {
  const N = Math.min(36, Math.max(1, Math.round(Number(antallMnd)) || 12));
  const { y, m0 } = ymDeler(startYm);
  if (kobletPlanId) {
    const plan = await hentPlan(db, kobletPlanId);
    if (plan && plan.selskap !== 'tech' && plan.type === 'modell') {
      const m = beregnInvestorModell({ antallMnd: plan.antallMnd, fakta: plan.fakta, drivere: plan.drivere, startYm: plan.startYm });
      const { y: py, m0: pm0 } = ymDeler(plan.startYm);
      const offset = (y * 12 + m0) - (py * 12 + pm0); // Tech-mnd 0 = plan-mnd offset
      const serie = Array.from({ length: N }, (_, i) => { const idx = Math.max(0, Math.min(m.enheter.length - 1, offset + i)); return Math.round((m.enheter[idx] || 0) * 10) / 10; });
      return { enheterForvaltning: serie, kilde: 'plan', kildePlanId: plan.id, kildePlanNavn: plan.navn, kildeSystemPerEnhet: plan.drivere?.systemPerEnhet ?? null, oppdatertAt: new Date().toISOString() };
    }
  }
  if (Array.isArray(rows)) {
    const serie = beregnEnhetsSerie(rows, y, m0 + N).slice(m0, m0 + N);
    return { enheterForvaltning: serie, kilde: 'portefolje', kildePlanId: null, kildePlanNavn: null, oppdatertAt: new Date().toISOString() };
  }
  return null;
}

// Dupliser en plan → nytt utkast «(kopi)», ikke delt med investorer. Samme selskap.
export async function dupliserPlan(db, id, { updatedBy = '' } = {}) {
  const doc = await db.collection(BUDSJETT_COLL).findOne({ plan: true, id: String(id) }, { projection: { _id: 0 } });
  if (!doc) return { ok: false, error: 'Fant ikke budsjettet', status: 404 };
  const now = new Date().toISOString(); const nyId = uuidv4();
  const navn = `${String(doc.navn || 'Budsjett').replace(/\s*\(kopi( \d+)?\)$/i, '')} (kopi)`.slice(0, 80);
  await db.collection(BUDSJETT_COLL).insertOne({ ...doc, id: nyId, navn, status: 'utkast', investorSynlig: false, createdAt: now, updatedAt: now, updatedBy: String(updatedBy || '').slice(0, 120) });
  return { ok: true, id: nyId, navn };
}
