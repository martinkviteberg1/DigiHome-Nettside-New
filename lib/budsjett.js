...
export async function listPlaner(db) {
  const docs = await db.collection(BUDSJETT_COLL)
    .find({ plan: true }, { projection: { _id: 0, id: 1, navn: 1, startYm: 1, antallMnd: 1, status: 1, inntekter: 1, kostnader: 1, updatedAt: 1, updatedBy: 1 } })
  ...
  return docs.map((d) => ({
    ...
    inntekter: r0(planSum(d.inntekter)),
    kostnader: r0(planSum(d.kostnader)),
    resultat: r0(planSum(d.inntekter) - planSum(d.kostnader)),
    updatedAt: d.updatedAt || null,
    updatedBy: d.updatedBy || null,
  }));
}
...
export async function hentPlan(db, id) {
  ...
  if (!doc) return null;
  const N = Math.min(24, Math.max(1, Math.round(doc.antallMnd) || 12));
  return {
    id: doc.id,
...
export async function lagrePlan(db, body = {}) {
  ...
  if (!(antallMnd >= 3 && antallMnd <= 24)) return { ok: false, error: 'Lengden må være 3–24 måneder', status: 400 };
