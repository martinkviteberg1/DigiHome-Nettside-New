...
export async function fetchMetaCreatives({ limit = 100 } = {}) {
  ...
  return rows.map((a) => {
    ...
    return {
      ...
      cta,
      link: ld.link || c.instagram_permalink_url || '',
    };
  });
}
...
const META_LEAD_ACTIONS = ['lead', 'offsite_conversion.fb_pixel_lead', 'onsite_conversion.lead_grouped', 'leadgen.other', 'onsite_web_lead'];
function sumLeadActions(arr) {
  let n = 0;
  for (const a of arr || []) {
    const t = String(a.action_type || '');
    if (META_LEAD_ACTIONS.includes(t) || /lead/i.test(t)) n += Number(a.value) || 0;
  }
  return n;
}

// Hent alle Meta-annonser m/ ytelsesmetrikk (spend/cpc/ctr/konv.) for perioden.
export async function fetchMetaAdsWithInsights({ datePreset = 'last_30d', since, until, limit = 200 } = {}) {
  const win = (since && until) ? `time_range(${JSON.stringify({ since, until })})` : `date_preset(${datePreset})`;
  const fields = [
    'id', 'name', 'effective_status', 'status',
    'campaign{name}', 'adset{name}',
    'creative{object_story_spec}',
    `insights.${win}{spend,impressions,reach,frequency,clicks,cpc,ctr,actions,action_values}`,
  ].join(',');
  ...
  return rows.map((a) => {
    ...
    const linkUrl = ld.link || (ld.child_attachments && ld.child_attachments[0] && ld.child_attachments[0].link) || (oss.video_data && oss.video_data.call_to_action && oss.video_data.call_to_action.value && oss.video_data.call_to_action.value.link) || '';
...
export async function setMetaAdStatus(adId, status = 'PAUSED') {
