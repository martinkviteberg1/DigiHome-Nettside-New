...
export async function lagLeieforholdExcel(data, okonomi = null, meta = {}) {
  ...
  const HODER = ['Eiendom / enhet', 'Enhetstype', 'Adresse', 'Huseier', 'Leietaker', 'Status', 'Inntektstype', 'Innflytting', 'Utflytting', 'Beløp', 'Sats (eks. mva)', 'Honorar (eks. mva)', 'Netto til huseier', 'Depositum', 'Servicenivå'];
  ...
  // Datarader — sats-kolonnen (K) er ALLTID eks. mva: avtaler inkl. mva
  // omregnes (15 % inkl → 12 % eks), så L = J×K/100 stemmer for alle rader.
  rows.forEach((r, i) => {
    const radNr = forsteData + i;
    const rad = ws.getRow(radNr);
    const satsEks = r.fee_percent ? (r.vat_inclusive ? r.fee_percent / 1.25 : r.fee_percent) : 0;
    rad.values = [
      r.unit_room,
    ...
    rad.getCell(12).value = { formula: `ROUND(J${radNr}*K${radNr}/100,0)` };
    rad.getCell(13).value = { formula: `ROUND(J${radNr}-L${radNr}*1.25,0)` };
    rad.getCell(10).numFmt = VALUTA;
    rad.getCell(11).numFmt = SATS;
    // MVA-kontekst: inkl.-avtaler merkes (fyll + notat) — satsen er omregnet.
    if (r.vat_inclusive && r.fee_percent) {
      const cSats = rad.getCell(11);
      cSats.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFDF3E0' } };
      cSats.note = `Avtalt ${r.fee_percent} % INKL. mva — vist omregnet til eks. mva (${Math.round(satsEks * 10) / 10} %). Honorar (L) = leie × sats eks. mva.`;
    }
...
// CSV: samme kolonner som Leieforhold-arket + «Avtalt sats» (kontekst for
// inkl. mva-avtaler — Sats-kolonnen er alltid omregnet til eks. mva),
...
export function lagLeieforholdCsv({ rows }) {
  ...
  const linjer = [
    ['Eiendom / enhet', 'Enhetstype', 'Adresse', 'Huseier', 'Leietaker', 'Status', 'Inntektstype', 'Innflytting', 'Utflytting', 'Beløp', 'Sats (eks. mva)', 'Avtalt sats', 'Honorar (eks. mva)', 'Netto til huseier', 'Depositum', 'Servicenivå'].join(';'),
    ...(rows || []).map((r) => {
      const satsEks = r.fee_percent ? (r.vat_inclusive ? r.fee_percent / 1.25 : r.fee_percent) : 0;
      return [
        ...
        r.monthly_rent || 0, String(Math.round(satsEks * 10) / 10).replace('.', ','),
        r.fee_percent ? `${String(r.fee_percent).replace('.', ',')} % ${r.vat_inclusive ? 'inkl. mva' : 'eks. mva'}` : '',
