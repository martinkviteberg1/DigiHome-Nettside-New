...
import { radNokkel, aktiveKostnader, fordelKostnader } from './leieforhold-filter';
...
export async function lagLeieforholdExcel(data, okonomi = null, meta = {}) {
  ...
  const start0 = 6;
  // Venstre kolonne: Portefølje + Inntekter
  let neste = kpiBlokk(ws0, start0, 'A', 'B', 'Portefølje', [
    ['Enheter i utvalget', rows.length],
    ['Utleid', totals.leased || 0],
    ['Fremtidig innflytting', totals.future || 0],
    ['Under signering', totals.signing || 0],
    ['Ledig', totals.vacant || 0],
    ['Utleiegrad', (totals.occupancy_pct || 0) / 100, '0 %'],
  ]);
  kpiBlokk(ws0, neste + 1, 'A', 'B', 'Leieinntekter / mnd', [
    ['Faktisk leie (utleid)', totals.actual_rent || 0, VALUTA],
    ['Forventet · signert', totals.expected_rent || 0, VALUTA],
    ['Under signering', totals.pending_rent || 0, VALUTA],
    ['Ledig · estimat', totals.estimate_rent || 0, VALUTA],
    ['Potensial totalt', (totals.actual_rent || 0) + (totals.expected_rent || 0) + (totals.pending_rent || 0) + (totals.estimate_rent || 0), VALUTA],
  ]);
  // Høyre kolonne: DigiHome-honorar + Økonomi
  let nesteH = kpiBlokk(ws0, start0, 'D', 'E', 'DigiHome / mnd (eks. mva)', [
    ['Honorar realisert', feeReal, VALUTA],
    ['Honorar garantert (m/ signert)', totals.fee_garantert ?? feeReal, VALUTA],
    ['Honorar potensial totalt', totals.fee_total ?? feeReal, VALUTA],
    ['Netto til huseiere (realisert)', totals.net || 0, VALUTA],
  ]);
  kpiBlokk(ws0, nesteH + 1, 'D', 'E', 'Økonomi (asset-light)', [
  ...
  ws0.getCell('A22').value = 'DigiHome er asset-light: huseier bærer alle boligkostnader. Margin = honorar − fordelte faste kostnader. CAC er engangs anskaffelseskost (payback-metrikk).';
  ws0.getCell('A22').font = { size: 8.5, italic: true, color: GRAA };
