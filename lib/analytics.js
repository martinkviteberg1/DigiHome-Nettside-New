...
export function track(type, meta = {}) {
  try {
    ...
    // Fest aktive A/B-tildelinger på hver hendelse (for funnel-nedbryting pr. variant).
    let ab = {};
    try { ab = JSON.parse(window.localStorage.getItem('dh_ab') || '{}') || {}; } catch (e) {}
    const metaOut = Object.keys(ab).length ? { ...(meta || {}), ab } : (meta || {});
    send({
      type,
      visitorId: st.vid,
      sessionId: st.sid,
...
export function getLeadAttribution() {
  try {
    ...
    const src = st.sessSrc || {};
    // GA4 client-id fra _ga-cookien (format GA1.1.<cid1>.<cid2>) → korrekt server-side
    // stitching i GA4 Measurement Protocol ved «won».
    let gaClientId = '';
    try {
      const ga = readCookie('_ga');
      if (ga) { const parts = ga.split('.'); if (parts.length >= 4) gaClientId = parts.slice(-2).join('.'); }
    } catch (e) {}
    // Aktive A/B-tildelinger → festes på leadet for variant-nedbryting i admin.
    let ab;
    try { const a = JSON.parse(lsGet('dh_ab') || '{}'); if (a && Object.keys(a).length) ab = a; } catch (e) {}
    return {
      ...
      // Meta-matching (Conversions API): _fbp/_fbc-cookiene satt av pixelen.
