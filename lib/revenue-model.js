...
export async function computeRevenueModel(db, {
  lifetimeMonths = 36,
  grossMarginPct = null,      // null = vis LTV som ren omsetning
  leaseActualRule = 'signed_started',
} = {}) {
  let contracts = [];
  try { contracts = dedupeContracts(await listContracts(db)); } catch (_) { contracts = []; }
  ...
  return {
    ...
    tiers: {
      ...
      potential: { mrr: mrrPotential, arr: round2(mrrPotential * 12), count: potential.length, label: 'Potensial — forvaltningsavtale uten leiekontrakt (estimert leie)' },
    },
    ...
    revenueQualityPct,
    ownerFees,
    expiring90d: expiring.slice(0, 20),
    // Toppliste for gjennomsiktighet i UI
    breakdown: {
      actual: actual.sort((a, b) => b.fee - a.fee).slice(0, 30),
      contracted: contracted.sort((a, b) => b.fee - a.fee).slice(0, 30),
      potential: potential.sort((a, b) => b.fee - a.fee).slice(0, 30),
    },
  };
}
