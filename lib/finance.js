...
export async function upsertContract(db, body = {}) {
  ...
  const doc = {
    ...
    source: (body.source || 'manual').toString().slice(0, 40),
    externalRef: (body.externalRef || '').toString().slice(0, 120) || null,
    note: (body.note || '').toString().slice(0, 500),
    updatedAt: now,
  };
  if (body.id) { await db.collection(CONTRACTS_COLL).updateOne({ id: String(body.id) }, { $set: doc }); return { id: String(body.id), ...doc }; }
  const id = uuidv4();
...
function contractMonthlyFee(c) {
  ...
  if (c.type === 'leiekontrakt') {
    ...
    return { amount: round2(rent * pct), confidence: 'faktisk', activeFrom: c.startDate || null, endDate: c.endDate || null };
  }
  const rent = (c.estimatedMonthlyRent != null ? Number(c.estimatedMonthlyRent) : Number(c.monthlyRent)) || 0;
  return { amount: round2(rent * pct), confidence: 'forventet', activeFrom: c.expectedRentStart || c.startDate || null, endDate: c.endDate || null };
}
...
export async function computeResultat(db) {
  ...
  for (const c of contracts) {
    if ((c.status || 'active') === 'terminated' || (c.status || '') === 'expired') continue;
    ...
    if (from <= now && end >= now) {
      activeContracts++;
      if (f.confidence === 'faktisk') incomeActual += f.amount; else incomeExpected += f.amount;
    }
  }
