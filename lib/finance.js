...
function unitMrrMap(fees, mStart, mEnd) {
  ...
  for (const f of fees) {
    if (!activeInMonth(f.activeFrom, f.endDate, mStart, mEnd)) continue;
    if (f.confidence !== 'faktisk' && f.propertyId && leaseProps.has(f.propertyId)) continue;
...
export async function computeInvestorMetrics(db, { horizon = 12 } = {}) {
  const [contracts, settings] = await Promise.all([listContracts(db), getFinanceSettings(db)]);
...
export async function computeForecast(db, { months = 18, assumptions = {} } = {}) {
  ...
  const [contracts, settings, res] = await Promise.all([listContracts(db), getFinanceSettings(db), computeResultat(db)]);
  const { llmMonthly } = await autoCosts(db, settings);
  ...
  const baseOpexExMarketing = round2((res.monthly.opexManual || 0) + (llmMonthly || 0));
...
export async function computeCustomers(db) {
  ...
  for (const [key, cs] of groups) {
    ...
    cs.forEach((c, i) => {
      ...
      if (isActiveNow) {
        activeContracts++;
        const deduped = f.confidence !== 'faktisk' && c.propertyId && leaseProps.has(c.propertyId);
