...
import { normSelskap, rensAutoregler, rensPrisliste, fordelAuto, beregnLisens, konsolider, foreslaaFlytt } from '@/lib/selskap-okonomi';
import { hentLeieforhold } from '@/lib/leieforhold';
import { beregnSikretSerie, beregnEnhetsSerie } from '@/lib/portefolje-serier';

// Plattformens leieforhold-eksport (1:1 med porteføljen) — samme mål som
// Leieforhold-modulen og budsjettets fakta-lag bruker. DB-cachet 10 min.
const plattformTarget = () => ({
  url: String(process.env.DIGIHOME_API_URL_PROD || 'https://app.digihome.no').trim().replace(/\/+$/, ''),
...
export const INNTEKT_COLL = 'finance_inntektsposter'; // Tech: abonnement/lisens-inntekter som ikke (ennå) hentes automatisk
...
export async function getFinanceSettings(db) {
  ...
  return {
    ...
    // Selskapsdimensjon: regler for automatiske kostnader, Techs prisliste og
    // Techs kontantsaldo (Digihome AS bruker openingCashBalance/-Date over).
    autoregler: rensAutoregler(doc?.autoregler),
    prisliste: rensPrisliste(doc?.prisliste),
    kontantTech: { saldo: doc?.kontantTech?.saldo != null ? Number(doc.kontantTech.saldo) : null, dato: doc?.kontantTech?.dato || null },
...
export async function setFinanceSettings(db, patch = {}) {
  ...
  const next = {
    ...
    autoregler: patch.autoregler !== undefined ? rensAutoregler({ ...cur.autoregler, ...patch.autoregler }) : cur.autoregler,
    prisliste: patch.prisliste !== undefined ? rensPrisliste({ ...cur.prisliste, ...patch.prisliste }) : cur.prisliste,
    kontantTech: patch.kontantTech !== undefined
      ? { saldo: patch.kontantTech?.saldo === '' || patch.kontantTech?.saldo == null ? null : Math.max(0, Number(patch.kontantTech.saldo) || 0), dato: patch.kontantTech?.dato || null }
      : cur.kontantTech,
...
// ─────────────────────────── Kostnader (OPEX) ───────────────────────────
// Hver kostnad tilhører ETT selskap (`selskap`: 'digihome' | 'tech'). Eldre
// poster uten feltet leses som Digihome AS — ingen migrering nødvendig.
export async function listCosts(db, { selskap } = {}) {
  try {
    const alle = (await db.collection(COSTS_COLL).find({}).sort({ createdAt: -1 }).toArray()).map((c) => ({ ...strip(c), selskap: normSelskap(c.selskap) }));
    return selskap ? alle.filter((c) => c.selskap === normSelskap(selskap)) : alle;
  } catch (_) { return []; }
}
export async function upsertCost(db, body = {}) {
  ...
  // Selskap: satt eksplisitt → normaliseres og regnes som bekreftet av bruker.
  if (body.selskap !== undefined) { doc.selskap = normSelskap(body.selskap); doc.selskapBekreftet = true; }
  if (body.id) { await db.collection(COSTS_COLL).updateOne({ id: String(body.id) }, { $set: doc }); return { id: String(body.id), ...doc }; }
  const id = uuidv4();
  if (!doc.selskap) doc.selskap = 'digihome';
...
// Flytt flere kostnader til et selskap i ett grep (sorterings-assistenten).
export async function flyttKostnader(db, ids = [], selskap) {
  const liste = (Array.isArray(ids) ? ids : []).map(String).filter(Boolean).slice(0, 500);
  if (!liste.length) return { ok: false, error: 'Ingen kostnader valgt' };
  const r = await db.collection(COSTS_COLL).updateMany({ id: { $in: liste } }, { $set: { selskap: normSelskap(selskap), selskapBekreftet: true, updatedAt: new Date().toISOString() } });
  return { ok: true, flyttet: r.modifiedCount || 0, selskap: normSelskap(selskap) };
}
// «Behold der de er» — merk som bekreftet så assistenten ikke spør igjen.
export async function bekreftKostnader(db, ids = []) {
  const liste = (Array.isArray(ids) ? ids : []).map(String).filter(Boolean).slice(0, 500);
  if (!liste.length) return { ok: false, error: 'Ingen kostnader valgt' };
  const r = await db.collection(COSTS_COLL).updateMany({ id: { $in: liste } }, { $set: { selskapBekreftet: true } });
  return { ok: true, bekreftet: r.modifiedCount || 0 };
}

// ─────────────────────────── Inntektsposter (Tech) ───────────────────────────
// Abonnements-/lisensinntekter som ikke hentes automatisk ennå (selvbetjente
// huseiere, bedriftskunder). kr/mnd eks. mva, aktiv i [startDate, endDate].
export const INNTEKT_KUNDETYPER = ['huseier', 'bedrift', 'annet'];
export async function listInntektsposter(db, { selskap = 'tech' } = {}) {
  try { return (await db.collection(INNTEKT_COLL).find({ selskap: normSelskap(selskap, 'tech') }).sort({ createdAt: -1 }).toArray()).map(strip); } catch (_) { return []; }
}
export async function upsertInntektspost(db, body = {}) {
  const now = new Date().toISOString();
  const navn = (body.navn || '').toString().trim().slice(0, 120);
  const belop = Math.max(0, Math.round(Number(body.belop) || 0));
  if (!navn) return { ok: false, error: 'Posten trenger et navn (kunde/produkt)' };
  if (!belop) return { ok: false, error: 'Beløp per måned må være større enn 0' };
  const doc = {
    selskap: normSelskap(body.selskap, 'tech'),
    navn,
    kundetype: INNTEKT_KUNDETYPER.includes(body.kundetype) ? body.kundetype : 'annet',
    enheter: Math.max(0, Math.round(Number(body.enheter) || 0)),
    belop,
    startDate: body.startDate || null,
    endDate: body.endDate || null,
  ...
  if (body.id) { await db.collection(INNTEKT_COLL).updateOne({ id: String(body.id) }, { $set: doc }); return { ok: true, post: { id: String(body.id), ...doc } }; }
  const id = uuidv4();
  await db.collection(INNTEKT_COLL).insertOne({ id, createdAt: now, ...doc });
  return { ok: true, post: { id, createdAt: now, ...doc } };
}
export async function deleteInntektspost(db, id) { await db.collection(INNTEKT_COLL).deleteOne({ id: String(id) }); return true; }
...
// ─────────────────────────── Selskapsøkonomi (per juridisk enhet) ───────────────────────────
// ETT kall → alle tre visninger (Digihome AS · Digihome Tech AS · Konsern):
//   · «nå» (inneværende måned, inkl. automatiske kostnader etter regler)
//   · tidslinje 12 måneder bakover + inneværende (manuelle kostnader og
//     kontrakter på faktiske datoer; automatiske kostnader kan bare måles nå)
// Plattformlisensen (enheter under forvaltning × Techs pris) er inntekt i
// Tech, kostnad i Digihome AS og elimineres i konsernet — alltid synlig.
// Digihome AS-inntekt følger samme regler som computeResultat: faktisk =
// signerte, startede leiekontrakter; forventet = forvaltningsavtaler uten
// leiekontrakt på enheten.
export async function computeSelskap(db) {
  const settings = await getFinanceSettings(db);
  const [costs, contracts, poster, alleKostnader] = await Promise.all([listActiveCosts(db), listContractsDeduped(db), listInntektsposter(db), listCosts(db)]);
  const auto = await autoCosts(db, settings);
  const autoF = fordelAuto(auto, settings.autoregler);
  const naa = new Date();
  const Y = naa.getUTCFullYear(); const M = naa.getUTCMonth();

  // ÉN SANNHET for porteføljen: Leieforhold-radene fra plattformen (samme
  // kilde som Leieforhold-modulen, budsjettets fakta og investordecket).
  // Faller plattformen ut, brukes kontraktsregisteret (finance_contracts).
  let rader = null; let kilde = 'kontrakter'; let kildeInfo = null;
  try {
    const lf = await hentLeieforhold(plattformTarget(), { db });
    if (lf?.ok && Array.isArray(lf.rows) && lf.rows.length) { rader = lf.rows; kilde = 'leieforhold'; kildeInfo = { source: lf.source, cached: !!lf.cached, fetchedAt: lf.fetchedAt || null }; }
  } catch (_) { rader = null; }
  const serieStartAar = Y - 1; // seriene dekker [jan i fjor, des i år]
  const sikretSerie = rader ? beregnSikretSerie(rader, serieStartAar, 24) : null;
  const enhetSerie = rader ? beregnEnhetsSerie(rader, serieStartAar, 24) : null;
  const serieIdx = (y, m) => (y - serieStartAar) * 12 + m;

  const aktive = contracts.filter((c) => (c.status || 'active') !== 'terminated' && (c.status || '') !== 'expired');
  const fees = aktive.map((c) => ({ c, f: contractMonthlyFee(c) }));
  const dhMnd = (mStart, mEnd) => {
    let faktisk = 0; let forventet = 0; let avtaler = 0;
    const enheter = new Set(); const leaseProps = new Set();
    for (const { c, f } of fees) { if (c.type === 'leiekontrakt' && c.propertyId && activeInMonth(f.activeFrom, f.endDate, mStart, mEnd)) leaseProps.add(c.propertyId); }
    for (const { c, f } of fees) {
      if (!activeInMonth(f.activeFrom, f.endDate, mStart, mEnd)) continue;
      enheter.add(c.propertyId || c.id);
      if (!f.amount) continue;
      if (f.confidence === 'faktisk') { faktisk += f.amount; avtaler++; } else {
        if (c.type !== 'leiekontrakt' && c.propertyId && leaseProps.has(c.propertyId)) continue;
        forventet += f.amount; avtaler++;
      }
    }
    return { faktisk: round2(faktisk), forventet: round2(forventet), avtaler, enheter: enheter.size };
  };
  const kostMnd = (sel, mStart, mEnd) => {
    const byCat = {}; let sum = 0;
    for (const c of costs) {
      if (normSelskap(c.selskap) !== sel) continue;
      if (!activeInMonth(c.startDate, c.endDate, mStart, mEnd)) continue;
      const m = monthlyCost(c); sum += m; byCat[c.category] = round2((byCat[c.category] || 0) + m);
    }
    return { byCat, sum: round2(sum) };
  };
  const posterMnd = (mStart, mEnd) => {
    const ut = { huseier: 0, bedrift: 0, annet: 0, sum: 0, antall: 0, enheter: 0 };
    for (const p of poster) {
      if (p.paused || !activeInMonth(p.startDate, p.endDate, mStart, mEnd)) continue;
      const k = INNTEKT_KUNDETYPER.includes(p.kundetype) ? p.kundetype : 'annet';
      ut[k] += p.belop; ut.sum += p.belop; ut.antall += 1; ut.enheter += p.enheter || 0;
    }
    return ut;
  };
  const leggAuto = (k, sel) => { for (const p of autoF[sel].poster) { k.byCat[p.kategori] = round2((k.byCat[p.kategori] || 0) + p.belop); k.sum = round2(k.sum + p.belop); } };

  const bygg = (y, m, medAuto) => {
    const mStart = Date.UTC(y, m, 1); const mEnd = Date.UTC(y, m + 1, 0, 23, 59, 59);
    const dhK = dhMnd(mStart, mEnd);
    const i = serieIdx(y, m);
    // Leieforhold-kilden gir kontraktsfestet honorar (signert) og enheter under forvaltning direkte.
    const dh = rader && i >= 0 && i < 24
      ? { faktisk: round2(sikretSerie[i]), forventet: 0, avtaler: dhK.avtaler, enheter: enhetSerie[i] }
      : dhK;
    const lisens = beregnLisens(dh.enheter, settings.prisliste);
    const p = posterMnd(mStart, mEnd);
    const kD = kostMnd('digihome', mStart, mEnd); const kT = kostMnd('tech', mStart, mEnd);
    if (medAuto) { leggAuto(kD, 'digihome'); leggAuto(kT, 'tech'); }
    if (lisens) { kD.byCat['Plattformlisens (Tech)'] = lisens; kD.sum = round2(kD.sum + lisens); }
    const techInntekt = round2(lisens + p.sum);
    const digihome = {
      inntekt: dh.faktisk, inntektForventet: round2(dh.faktisk + dh.forventet), forventetTillegg: dh.forventet,
      kost: kD.sum, resultat: round2(dh.faktisk - kD.sum), resultatForventet: round2(dh.faktisk + dh.forventet - kD.sum),
      enheter: dh.enheter, avtaler: dh.avtaler, lisens, byCat: kD.byCat,
    };
    const tech = {
      inntekt: techInntekt, kost: kT.sum, resultat: round2(techInntekt - kT.sum),
      lisens, poster: { huseier: p.huseier, bedrift: p.bedrift, annet: p.annet, antall: p.antall, enheter: p.enheter }, byCat: kT.byCat,
    };
    const k = konsolider({ digihome: { inntekt: digihome.inntekt, kost: digihome.kost }, tech: { inntekt: tech.inntekt, kost: tech.kost }, lisens });
    const byCatK = { ...kT.byCat };
    for (const [cat, v] of Object.entries(kD.byCat)) { if (cat === 'Plattformlisens (Tech)') continue; byCatK[cat] = round2((byCatK[cat] || 0) + v); }
    const konsern = { ...k, inntektForventet: round2(k.inntekt + dh.forventet), resultatForventet: round2(k.resultat + dh.forventet), byCat: byCatK, enheter: dh.enheter };
    return { ym: `${y}-${String(m + 1).padStart(2, '0')}`, digihome, tech, konsern };
  };

  const naaMnd = bygg(Y, M, true);
  const tidslinje = [];
  for (let i = 12; i >= 1; i--) { const t = Y * 12 + M - i; tidslinje.push(bygg(Math.floor(t / 12), t % 12, false)); }
  tidslinje.push(naaMnd);

  const breakdown = (byCat) => Object.entries(byCat).map(([category, amount]) => ({ category, amount })).sort((a, b) => b.amount - a.amount);
  const runway = (saldo, resultat) => (saldo != null && resultat < 0 ? Math.round((saldo / -resultat) * 10) / 10 : null);
  const kontantDH = settings.openingCashBalance; const kontantTech = settings.kontantTech?.saldo ?? null;
  const forslag = foreslaaFlytt(alleKostnader.filter((c) => c.paused !== true)).map((c) => ({ id: c.id, name: c.name, category: c.category, amount: monthlyCost(c) }));
  ...
  return {
    ...
    naa: {
      ym: naaMnd.ym,
      digihome: { ...naaMnd.digihome, costBreakdown: breakdown(naaMnd.digihome.byCat), auto: autoF.digihome, kontant: kontantDH, runwayMnd: runway(kontantDH, naaMnd.digihome.resultat) },
      tech: { ...naaMnd.tech, costBreakdown: breakdown(naaMnd.tech.byCat), auto: autoF.tech, kontant: kontantTech, runwayMnd: runway(kontantTech, naaMnd.tech.resultat) },
      konsern: { ...naaMnd.konsern, costBreakdown: breakdown(naaMnd.konsern.byCat), kontant: kontantDH != null || kontantTech != null ? round2((kontantDH || 0) + (kontantTech || 0)) : null, runwayMnd: runway(kontantDH != null || kontantTech != null ? (kontantDH || 0) + (kontantTech || 0) : null, naaMnd.konsern.resultat) },
    },
    tidslinje: tidslinje.map((r) => ({ ym: r.ym, digihome: { inntekt: r.digihome.inntekt, kost: r.digihome.kost, resultat: r.digihome.resultat, enheter: r.digihome.enheter, lisens: r.digihome.lisens }, tech: { inntekt: r.tech.inntekt, kost: r.tech.kost, resultat: r.tech.resultat, lisens: r.tech.lisens, poster: r.tech.poster.sum }, konsern: { inntekt: r.konsern.inntekt, kost: r.konsern.kost, resultat: r.konsern.resultat, eliminert: r.konsern.eliminert } })),
    kilde: { portefolje: kilde, ...(kildeInfo || {}) },
    prisliste: settings.prisliste,
    autoregler: settings.autoregler,
    autoKilder: { adSpendMonthly: auto.adSpendMonthly, llmMonthly: auto.llmMonthly, extMonthly: auto.extMonthly, platformMonthly: auto.platformMonthly, adsConfigured: auto.adConfigured, platformOk: auto.platformOk },
    forslagFlytt: forslag,
    antall: { kostnader: { digihome: costs.filter((c) => normSelskap(c.selskap) === 'digihome').length, tech: costs.filter((c) => normSelskap(c.selskap) === 'tech').length }, inntektsposter: poster.length },
  };
}
